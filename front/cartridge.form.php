...
if (isset($_POST["add"])) {
    ...
    Event::log(
        ...
        sprintf(__('%s adds cartridges'), $_SESSION["zentraname"])
    );
    Html::back();
    ...
    if ($cart->delete($_POST, true)) {
        Event::log(
            ...
            "inventory",
            //TRANS: %s is the user login
            sprintf(__('%s purges a cartridge'), $_SESSION["zentraname"])
        );
    }
    Html::back();
        ...
        for ($i = 0; $i < $_POST["nbcart"]; $i++) {
            if ($cart->install($_POST["printers_id"], $_POST["cartridgeitems_id"])) {
                Event::log(
                    ...
                    "inventory",
                    //TRANS: %s is the user login
                    sprintf(__('%s installs a cartridge'), $_SESSION["zentraname"])
                );
            }
        }
    }
    ...
    if ($cart->update($_POST)) {
        Event::log(
            ...
            "inventory",
            //TRANS: %s is the user login
            sprintf(__('%s updates a cartridge'), $_SESSION["zentraname"])
        );
    }
    Html::back();
} else {
