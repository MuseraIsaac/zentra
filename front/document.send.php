...
if (isset($_GET['docid'])) {
    ...
    // public FAQ.
    // Document::canViewFile() will do appropriate checks depending on ZENTRA configuration.
    ...
    if (!file_exists(ZENTRA_DOC_DIR . "/" . $doc->fields['filepath'])) {
        ...
        if (
            $doc->fields['sha1sum']
            && $doc->fields['sha1sum'] != sha1_file(ZENTRA_DOC_DIR . "/" . $doc->fields['filepath'])
        ) {
    ...
    if (count($splitter) == 2) {
        ...
        if ($splitter[0] == "_pictures") {
            if (Document::isImage(ZENTRA_PICTURE_DIR . '/' . $splitter[1])) {
                // Can use expires header as picture file path changes when picture changes.
                $expires_headers = true;
                $send = ZENTRA_PICTURE_DIR . '/' . $splitter[1];
            }
        }

        if ($splitter[0] == "_inventory" && Session::haveRight(Conf::$rightname, READ)) {
            $iconf = new Conf();
            if ($iconf->isInventoryFile(ZENTRA_INVENTORY_DIR . '/' . $splitter[1])) {
                $send = ZENTRA_INVENTORY_DIR . '/' . $splitter[1];
