...
if (isset($_POST["add"])) {
    ...
    if ($newID = $license->add($_POST)) {
        Event::log(
            ...
            sprintf(__('%1$s adds the license %2$s'), $_SESSION["zentraname"], $newID)
        );
    ...
    Event::log(
        ...
        "inventory",
        //TRANS: %s is the user login, %2$s is the license id
        sprintf(__('%1$s deletes the license %2$s'), $_SESSION["zentraname"], $_POST["id"])
    );
    $license->redirectToList();
    ...
    Event::log(
        ...
        "inventory",
        //TRANS: %s is the user login, %2$s is the license id
        sprintf(__('%1$s purges the license %2$s'), $_SESSION["zentraname"], $_POST["id"])
    );
    $license->redirectToList();
    ...
    Event::log(
        ...
        "inventory",
        //TRANS: %s is the user login, %2$s is the license id
        sprintf(__('%1$s updates the license %2$s'), $_SESSION["zentraname"], $_POST["id"])
    );
    Html::back();
} else {
