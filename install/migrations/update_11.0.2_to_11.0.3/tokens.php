...
$zentra_key_manager = new ZENTRAKey();
$use_legacy_key   = $zentra_key_manager->keyExists() === false;

if ($use_legacy_key) {
    $zentra_key    = $zentra_key_manager->getLegacyKey();
    $encrypt_fct = function (string $value) use ($zentra_key): string {
        // Code corresponding to encryption used prior to ZENTRA 9.5 (copied from `Toolbox::encrypt()`).
        // /!\ It is mandatory to encrypt data using the legacy key to handle migrations from a ZENTRA version < 9.5.0.
        // Data will be re-encrypted at the update process when a new key will be generated.
        $result = '';
        $strlen = strlen($value);
        for ($i = 0; $i < $strlen; $i++) {
            $char    = substr($value, $i, 1);
            $keychar = substr($zentra_key, ($i % strlen($zentra_key)) - 1, 1);
    ...
    $zentra_key    = $zentra_key_manager->get();
    $encrypt_fct = fn(string $value) => $zentra_key_manager->encrypt($value);
}

if ($zentra_key === null) {
    // If `$zentra_key` is `null`, it means tha the key file exists but an error occurs while reading it.
    // It is preferable to fail here rather than ruin all tokens values in database.
    throw new RuntimeException('Unable to get the ZENTRA encryption key value.');
}
...
if ((bool) $are_users_tokens_encrypted === false) {
    $migration->changeField('zentra_users', 'password_forget_token', 'password_forget_token', 'string'); // change length from 40 to 255
