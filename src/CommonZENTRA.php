...
/**
 * Common ZENTRA object
 *
 * @phpstan-type ZentraMenuOption array{
 *   title: string,
 *   page: string,
 *   icon?: string,
 *   links?: array{
 ...
 * }
 *
 * @phpstan-type ZentraMenuEntry array{
 *   title: string,
 *   page: string,
 *   shortcut?: string,
 *   icon?: string,
 ...
 *   },
 *   options?: array<string, ZentraMenuOption>
 * }
 */
class CommonZENTRA implements CommonZENTRAInterface
{
    /**
     ...
     *  ]
     *
     * @var array<class-string<CommonZENTRA>, array<class-string<CommonZENTRA>, int>>
     */
     ...
     * @since 0.83
     *
     * @param class-string<CommonZENTRA>  $typeform  object class name to add tab on form
     * @param class-string<CommonZENTRA>  $typetab   object class name which manage the tab
     ...
     * @since 0.83
     *
     * @param class-string<CommonZENTRA>  $typeform   object class name on which we want to get managed tabs
     *
     * @return class-string<CommonZENTRA>[]
     *
     * @final
     */
     ...
     * Add a standard tab.
     *
     * @param class-string<CommonZENTRA>   $itemtype itemtype link to the tab
     ...
     * @since 0.85
     *
     * @return false|ZentraMenuEntry|array<string, ZentraMenuEntry|true>
     */
     ...
     * @since 0.85
     *
     * @return false|array<string, ZentraMenuEntry>  Additional menu specs, or false if no additional menu content.
     */
     ...
     * @since 0.85
     *
     * @return false|array<string, ZentraMenuOption>  Additional menu options, or false if no additional options.
     */
     ...
     * @since 0.83
     *
     * @param CommonZENTRA $item          Item on which the tab need to be displayed
     ...
     * @return bool
     *
     * @TODO In ZENTRA 12.0, do something with the return value that is currently not used.
     */
     ...
     * @TODO In ZENTRA 12.0, do something with the return value that is currently not used.
     */
    public static function displayStandardTab(CommonZENTRA $item, $tab, $withtemplate = 0, $options = [])
    {
        switch ($tab) {
            // All tab
            case -1:
    ...
    /**
     * @param class-string<CommonZENTRA>|null $form_itemtype
     * @return string
     */
     ...
     * @param class-string<CommonZENTRA>|null $form_itemtype  itemtype whose form is being displayed
    ...
    public static function createTabEntry($text, $nb = 0, ?string $form_itemtype = null, string $icon = '', ?int $total_nb = null)
    {
        ...
        if ($nb > 0) {
            ...
            $counter_html = sprintf(' <span class="badge zentra-badge">%s</span>', htmlescape($badge_content));
        }
    ...
    public function getRedirectToListUrl(): string
    {
        ...
        if (!empty($_GET['withtemplate'])) {
            return $CFG_ZENTRA["root_doc"] . "/front/setup.templates.php?add=0&itemtype=" . static::getType();
        }

        if (
            isset($_SESSION['zentralisturl'][static::getType()])
            && !empty($_SESSION['zentralisturl'][static::getType()])
        ) {
            return $_SESSION['zentralisturl'][static::getType()];
        }
     ...
     *                      otherwise, it will return the path relative to ZENTRA root.
     *
     * @return string
     *
     * @final
    ...
    public function showTabsContent($options = [])
    {
        ...
        // $options is empty.
        // See:
        //  - https://github.com/zentra-project/zentra/pull/12929
        //  - https://github.com/zentra-project/zentra/pull/13101
        //  - https://github.com/zentra-project/zentra/commit/1d27bf14d4527f748876dcd556f2b995a0bf7684
        ...
        if (count($onglets)) {
            ...
            foreach ($onglets as $key => $val) {
                ...
                $tabs[$key] = ['title'  => $val,
                    'url'    => $tab_path,
                    'params' => Toolbox::append_params(['_zentra_tab' => $key] + $tab_params, '&'),
                ];
            }
            ...
            if (
                ...
                $tabs[-1] = ['title'  => static::createTabEntry(__('All'), 0, null, 'ti ti-layout-list'),
                    'url'    => $tab_path,
                    'params' => Toolbox::append_params(['_zentra_tab' => '-1'] + $tab_params, '&'),
                ];
            }
    ...
    public function showNavigationHeader($options = [])
    {
        ...
        if (
            ...
            $zentralistitems = & $_SESSION['zentralistitems'][static::getType()];
            $zentralisttitle = & $_SESSION['zentralisttitle'][static::getType()];
            $zentralisturl   = & $_SESSION['zentralisturl'][static::getType()];
            if ($this instanceof CommonDBChild && $parent = $this->getItem(true, false)) {
                $zentralisturl = $parent::getFormURLWithID($parent->fields['id'], true);
            }
            if (empty($zentralisturl)) {
                $zentralisturl = static::getSearchURL();
            }

            $next = $prev = $first = $last = -1;
            $current = false;
            if (is_array($zentralistitems)) {
                $current = array_search($ID, $zentralistitems);
                if ($current !== false) {
                    if (isset($zentralistitems[$current + 1])) {
                        $next = $zentralistitems[$current + 1];
                    }

                    if (isset($zentralistitems[$current - 1])) {
                        $prev = $zentralistitems[$current - 1];
                    }

                    $first = $zentralistitems[0];
                    if ($first == $ID) {
                        $first = -1;
                    }

                    $last = $zentralistitems[count($zentralistitems) - 1];
                    if ($last == $ID) {
                        $last = -1;
                    }
                }
            ...
            if (!$zentralisttitle) {
                $zentralisttitle = __('List');
            }
            $list = "<a href='" . htmlescape($zentralisturl) . "' title=\"" . htmlescape($zentralisttitle) . "\"
            ...
            echo "<span class='py-1 px-3 " . ($current !== false ? '' : 'bs-invisible') . "'>" . ($current + 1) . "/" . count($zentralistitems ?? []) . "</span>";
            ...
            if (count($found_kbitem) == 1) {
                ...
                var getKnowbaseItemAnswer$rand = function() {
                    ...
                    $('#faqadd_block_content$rand').load(
                        '" . $CFG_ZENTRA['root_doc'] . "/ajax/getKnowbaseItemAnswer.php',
                        {
