...
class Migration
{
    ...
    /**
     * Add a new ZENTRA normalized field
     *
    ...
    public function renameTable($oldtable, $newtable)
    {
        if (!$this->db->tableExists("$newtable") && $this->db->tableExists("$oldtable")) {
            ...
            if (
                str_starts_with($oldtable, 'zentra_plugin_')
                || str_starts_with($newtable, 'zentra_plugin_')
            ) {
                return;
            }
            $message = sprintf(
     ...
     * @since 0.84
     *
     * @param array $rule     Array of fields of zentra_rules
     * @param array $criteria Array of Array of fields of zentra_rulecriterias
     * @param array $actions  Array of Array of fields of zentra_ruleactions
     *
    ...
    public function createRule(array $rule, array $criteria, array $actions)
    {
        ...
        if (countElementsInTable('zentra_rules', ['name' => $rule['name']])) {
            return 0;
        }
        $rule['comment']     = sprintf(__('Automatically generated by ZENTRA %s'), $this->version);
        $rule['description'] = '';
        ...
        $this->db->insert('zentra_rules', $values);
        ...
        foreach ($criteria as $criterion) {
            ...
            $this->db->insert('zentra_rulecriterias', $values);
        }
        ...
        foreach ($actions as $action) {
            ...
            $this->db->insert('zentra_ruleactions', $values);
        }
    ...
    public function updateDisplayPrefs($toadd = [], $todel = [], bool $only_default = false)
    {
        ...
        $this->addDebugMessage(sprintf(__('Data migration - %s'), 'zentra_displaypreferences'));
                ...
                'LEFT JOIN' => [
                    'zentra_profilerights' => [
                        'ON' => [
                            ...
                            'zentra_profiles'      => 'id',
                            [
                                'AND' => ['zentra_profilerights.name' => $name],
                            ],
                        ],
                    ],
                ],
                'WHERE'     => [
                    'zentra_profilerights.id' => null,
                ],
            ]
        );
            ...
            'JOIN'      => [
                'zentra_profilerights' => [
                    'FKEY' => [
                        ...
                        [
                            'AND' => [
                                'zentra_profilerights.name' => $name,
                            ],
                        ],
                    ],
                ],
    ...
    public function replaceRight($name, $rights, $requiredrights = ['config' => READ | UPDATE])
    {
        ...
        $i = 1;
        foreach ($requiredrights as $reqright => $reqvalue) {
            $join["zentra_profilerights as right$i"] = [
                'ON' => [
                    "right$i"       => 'profiles_id',
                    'zentra_profiles' => 'id',
                    [
    ...
    private function updateProfileLastRightsUpdate(int $profile_id): void
    {
        ...
        if (!$this->db->fieldExists('zentra_profiles', 'last_rights_update')) {
            return;
        }
        ...
        if ($old_table !== $new_table && $update_structure) {
            ...
            foreach ($fkey_column_array as $fkey_column) {
                ...
                $this->changeField(
                    ...
                    $fkey_oldname,
                    $fkey_newname,
                    "int " . $this->getDefaultPrimaryKeySignOption() . " NOT NULL DEFAULT '0'" // assume that foreign key always uses ZENTRA conventions
                );
            }
        }
        ...
        foreach ($this->search_opts as $itemtype => $changes) {
            foreach ($changes as $p) {
                ...
                if (is_a($itemtype, 'CommonITILObject', true)) {
                    $tables = [
                        'zentra_' . strtolower($itemtype) . 'templatehiddenfields',
                        'zentra_' . strtolower($itemtype) . 'templatemandatoryfields',
                        'zentra_' . strtolower($itemtype) . 'templatepredefinedfields',
                    ];
                    foreach ($tables as $table) {
                        if (!$this->db->tableExists($table)) {
                            continue;
    ...
    public function addCrontask(string $itemtype, string $name, int $frequency, ?int $param = null, array $options = []): void
    {
        ...
        $this->db->insert('zentra_crontasks', $input);
    }
