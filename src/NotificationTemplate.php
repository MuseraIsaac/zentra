...
class NotificationTemplate extends CommonDBTM
{
    ...
    /**
     * @param NotificationTarget<covariant CommonZENTRA> $target
    ...
    public function getTemplateByLanguage(
        ...
        if (!isset($this->templates_by_languages[$tid])) {
            ...
            $bak_dropdowntranslations = ($_SESSION['zentra_dropdowntranslations'] ?? null);
            $_SESSION['zentra_dropdowntranslations'] = DropdownTranslation::getAvailableTranslations($language);
            Session::loadLanguage($language);
            $bak_language = $_SESSION["zentralanguage"];
            $_SESSION["zentralanguage"] = $language;

            // set timezone from user, and reload object
            $orig_tz = null;
            if (isset($user_infos['additionnaloption']['timezone'])) {
                $orig_tz = $_SESSION['zentratimezone'] ?? '0';
                if ($orig_tz === '0') {
            ...
            $footer_string = sprintf(__('Automatically generated by %s'), $CFG_ZENTRA['app_name']);
            ...
            $_SESSION["zentralanguage"] = $bak_language;
            Session::loadLanguage();
            if ($bak_dropdowntranslations !== null) {
                $_SESSION['zentra_dropdowntranslations'] = $bak_dropdowntranslations;
            } else {
    ...
    public static function process($string, $data, bool $html_context = false)
    {
        ...
        $string = self::convertRelativeZentraLinksToAbsolute($string, $html_context);

        return $string;
    }

    /**
     * Convert relative links to ZENTRA nto absolute links.
     *
     * @param string $string
     * @return string
     */
    private static function convertRelativeZentraLinksToAbsolute(string $string, bool $html_context): string
    {
        global $CFG_ZENTRA;
    ...
    /**
     * @param NotificationTarget<covariant CommonZENTRA>  $target     Target instance
