...
class ZENTRAKey
{
    ...
    public function __construct(string $config_dir = ZENTRA_CONFIG_DIR)
    {
        $this->keyfile = $config_dir . '/zentracrypt.key';
        $this->legacykeyfile = $config_dir . '/zentra.key';
    }

    /**
     * Returns expected key path for given ZENTRA version.
     * Will return null for ZENTRA versions that was not yet handling a custom security key.
     *
     * @param string $zentra_version
     *
     * @return string|null
     */
    public function getExpectedKeyPath(string $zentra_version): ?string
    {
        if (version_compare($zentra_version, '9.4.6', '<')) {
            return null;
        } elseif (version_compare($zentra_version, '9.5.x', '<')) {
    ...
    /**
     * Check if ZENTRA security key used for decryptable passwords exists
     *
     * @return bool
     */
    ...
    {
        $zentra_key_read_errors = $this->getKeyFileReadErrors();
        if ($zentra_key_read_errors !== []) {
            TemplateRenderer::getInstance()->display(
                '/central/messages.html.twig',
                [
                    'messages' => [
                        'errors' => $zentra_key_read_errors,
                    ],
                ]
            );
        }
    ...
    /**
     * Get ZENTRA security key used for decryptable passwords
     *
     * @return string|null
     */
    ...
    /**
     * Get ZENTRA security legacy key that was used for decryptable passwords.
     * Usage of this key should only be used during migration from ZENTRA < 9.5 to ZENTRA >= 9.5.0.
     *
     * @return string|null
     */
    ...
    /**
     * Generate ZENTRA security key used for decryptable passwords
    ...
    public function decrypt(?string $string, ?string $key = null): ?string
    {
        ...
        try {
            ...
            trigger_error(
                'Unable to base64_decode the string. The string was probably not encrypted using ZENTRAKey::encrypt',
                E_USER_WARNING
            );
            return '';
        }
