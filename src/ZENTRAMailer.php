...
class ZENTRAMailer
{
    ...
    public function __construct(?TransportInterface $transport = null)
    {
        ...
        if (
            (int) $CFG_ZENTRA['smtp_mode'] === MAIL_SMTPOAUTH
        ...
        if (!empty($CFG_ZENTRA['smtp_sender'])) {
            $this->email->sender($CFG_ZENTRA['smtp_sender']);
        }
    }
    ...
    final public static function buildDsn(bool $with_clear_password): string
    {
        ...
        if ($CFG_ZENTRA['smtp_mode'] != MAIL_MAIL) {
            ...
            if ($with_clear_password) {
                if ((int) $CFG_ZENTRA['smtp_mode'] === MAIL_SMTPOAUTH) {
                    $provider = OauthConfig::getInstance()->getSmtpOauthProvider();
                    $refresh_token = (new ZENTRAKey())->decrypt($CFG_ZENTRA['smtp_oauth_refresh_token']);
                    if ($provider !== null && $refresh_token !== '') {
                        ...
                        if ($new_refresh_token !== null && $new_refresh_token !== $refresh_token) {
                            ...
                            $CFG_ZENTRA['smtp_oauth_refresh_token'] = (new ZENTRAKey())->encrypt($new_refresh_token);
                        }
                    }
                } else {
                    $password = (new ZENTRAKey())->decrypt($CFG_ZENTRA['smtp_passwd']);
                }
            }
            $dsn = sprintf(
                'smtp://%s%s:%s',
                ($CFG_ZENTRA['smtp_username'] != '' ? sprintf(
                    '%s:%s@',
                    rawurlencode($CFG_ZENTRA['smtp_username']),
                    $with_clear_password ? rawurlencode($password) : '********'
                ) : ''),
                $CFG_ZENTRA['smtp_host'],
                $CFG_ZENTRA['smtp_port']
            );

            if (!$CFG_ZENTRA['smtp_check_certificate']) {
                $dsn .= '?verify_peer=0';
            }
        }
