...
class ZENTRANetwork extends CommonZENTRA
{
    public static function getTypeName($nb = 0)
    {
        return __('ZENTRA Network');
    }

    public function getTabNameForItem(CommonZENTRA $item, $withtemplate = 0)
    {
        return self::createTabEntry('ZENTRA Network');
    }
    ...
    public static function showForConfig()
    {
        ...
        TemplateRenderer::getInstance()->display('pages/setup/general/zentranetwork_setup.html.twig', [
    ...
    /**
     * Get ZENTRA User Agent in expected format from ZENTRA Network services.
     *
     * @return string
     */
    public static function getZentraUserAgent(): string
    {
        $version = VersionParser::getNormalizedVersion(ZENTRA_VERSION, false);
        $comments = sprintf('installation-mode:%s', ZENTRA_INSTALL_MODE);
        if (!empty(ZENTRA_USER_AGENT_EXTRA_COMMENTS)) {
            // append extra comments (remove '(' and ')' chars to not break UA string)
            $comments .= '; ' . (string) preg_replace('/\(\)/', ' ', ZENTRA_USER_AGENT_EXTRA_COMMENTS);
        }
        return sprintf('ZENTRA/%s (%s)', $version, $comments);
    }

    /**
     * Get ZENTRA Network UID to pass in requests to ZENTRA Network Services.
     *
     * @return string
     */
    public static function getZentraNetworkUid(): string
    {
        return Config::getUuid('zentra_network');
    }

    /**
     * Get ZENTRA Network registration key.
     *
    ...
    {
        global $CFG_ZENTRA;
        return (new ZENTRAKey())->decrypt($CFG_ZENTRA['zentranetwork_registration_key'] ?? '');
    }

    /**
     * Get ZENTRA Network registration information.
     *
    ...
    public static function getRegistrationInformations(bool $force_refresh = false)
    {
        ...
        $registration_key = self::getRegistrationKey();
        $lang = preg_replace('/^([a-z]+)_.+$/', '$1', $_SESSION["zentralanguage"]);

        $cache_key = sprintf('registration_%s_%s_informations', sha1($registration_key), $lang);
        if (!$force_refresh && ($informations = $ZENTRA_CACHE->get($cache_key)) !== null) {
            return $informations;
        }
        ...
        $eopts = [
            CURLOPT_HTTPHEADER => [
                ...
                'User-Agent:' . self::getZentraUserAgent(),
                'X-Registration-Key:' . $registration_key,
                'X-Zentra-Network-Uid:' . self::getZentraNetworkUid(),
            ],
        ];
        ...
        $registration_response = Toolbox::callCurl(
            rtrim(ZENTRA_NETWORK_REGISTRATION_API_URL, '/') . '/info',
            $eopts,
            $error_message
        );
        ...
        $ZENTRA_CACHE->set($cache_key, $informations, new DateInterval('P1D')); // Cache for one day

        return $informations;
    }

    /**
     * Check if ZENTRA Network registration is existing and valid.
     *
     * @return bool
     */
    ...
    public static function isServicesAvailable(&$curl_error = null): bool
    {
        ...
        $content = Toolbox::callCurl(rtrim(ZENTRA_NETWORK_API_URL, '/') . '/ping', $eopts, $error_msg, $curl_error);
        return $content !== '';
    }
    ...
    public static function getOffers(bool $force_refresh = false): array
    {
        ...
        $lang = preg_replace('/^([a-z]+)_.+$/', '$1', $_SESSION["zentralanguage"]);
        $cache_key = 'zentra_network_offers_' . $lang;

        if (!$force_refresh && ($offers = $ZENTRA_CACHE->get($cache_key)) !== null) {
            return $offers;
        }
        ...
        $response = Toolbox::callCurl(
            rtrim(ZENTRA_NETWORK_REGISTRATION_API_URL, '/') . '/offers',
            $eopts,
            $error_message
        );
        ...
        $ZENTRA_CACHE->set($cache_key, $offers, HOUR_TIMESTAMP);
