...
final class AltchaManager
{
     ...
     *
     * The existing zentracrypt.key will work fine for this as it is has a length
     * of 256 bits at the time this comment was written.
     */
    private function computeHmacKey(): string
    {
        return (new ZENTRAKey())->get();
    }
    ...
    private function getMode(): AltchaMode
    {
        ...
        if (!defined('ZENTRA_ALTCHA_MODE')) {
            // @phpstan-ignore theCodingMachineSafe.function (we checked just above if it is defined)
            define('ZENTRA_ALTCHA_MODE', self::DEFAULT_MODE);
        }

        $mode = ZENTRA_ALTCHA_MODE;
    ...
    private function getComplexity(): int
    {
        // Should never happens as it is defined by SystemConfigurator.php
        // but it it still another safety to avoid failure.
        if (!defined('ZENTRA_ALTCHA_MAX_NUMBER')) {
            // @phpstan-ignore theCodingMachineSafe.function (we checked just above if it is defined)
            define('ZENTRA_ALTCHA_MAX_NUMBER', self::DEFAULT_COMPLEXITY);
        }

        $complexity = ZENTRA_ALTCHA_MAX_NUMBER;
    ...
    private function getExpiresAtInterval(): DateInterval
    {
        // Should never happens as it is defined by SystemConfigurator.php
        // but it it still another safety to avoid failure.
        if (!defined('ZENTRA_ALTCHA_EXPIRATION_INTERVAL')) {
            // @phpstan-ignore theCodingMachineSafe.function (we checked just above if it is defined)
            define('ZENTRA_ALTCHA_EXPIRATION_INTERVAL', self::DEFAULT_EXPIRATION_INTERVAL);
        }

        $interval = ZENTRA_ALTCHA_EXPIRATION_INTERVAL;
