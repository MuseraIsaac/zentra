...
use Zentra\Toolbox\MarkdownRenderer;
use ZENTRAKey;
use Html;
use Infocom;
use Item_Devices;
...
abstract class API
{
    ...
    public function initApi()
    {
        ...
        $app_tokens = array_map(
            static function (array $client): string {
                ...
                return (new ZENTRAKey())->decrypt($client['app_token']);
            },
            $found_clients
        );
    ...
    protected function cors()
    {
        ...
        if ($this->verb == "OPTIONS") {
            ...
            exit(0); // @phpstan-ignore zentra.forbidExit (API response is streamed)
        }
    }
    ...
    protected function initSession($params = [])
    {
        ...
        if (isset($params['user_token']) && !empty($params['user_token'])) {
            ...
            $noAuto = false;
        } elseif (!$CFG_ZENTRA['enable_api_login_credentials']) {
            $this->returnError(
        ...
        if (!$auth->login($params['login'], $params['password'], $noAuto, false, $params['auth'])) {
            ...
            if (
                ...
                $this->returnError(__("parameter user_token seems invalid"), 401, "ERROR_ZENTRA_LOGIN_USER_TOKEN", false);
            }
            $this->returnError($err, 401, "ERROR_ZENTRA_LOGIN", false);
        }
    ...
    /**
     * Retrieve ZENTRA Session initialised by initSession function
     * Use 'session_token' param in $this->parameters
     *
     * @return void
     */
    protected function retrieveSession()
    {
        ...
        if (
            ...
            if (file_exists(ZENTRA_ROOT . '/inc/downstream.php')) {
                include_once(ZENTRA_ROOT . '/inc/downstream.php');
            }
    ...
    protected function getMyEntities($params = [])
    {
        ...
        $myentities = [];
        foreach ($_SESSION['zentraactiveprofile']['entities'] as $entity) {
            if ($entity['is_recursive'] == 1 && $params['is_recursive'] == 1) {
                $sons = getSonsOf('zentra_entities', $entity['id']);
    ...
    {
        $actives_entities = [];
        foreach (array_values($_SESSION['zentraactiveentities']) as $active_entity) {
            $actives_entities[] = ['id' => $active_entity];
        }

        return ["active_entity" => [
            "id"                      => $_SESSION['zentraactive_entity'],
            "active_entity_recursive" => $_SESSION['zentraactive_entity_recursive'],
    ...
    protected function changeActiveProfile($params = [])
    {
        ...
        if (isset($_SESSION['zentraprofiles'][$profiles_id])) {
    ...
    {
        $myprofiles = [];
        foreach ($_SESSION['zentraprofiles'] as $profiles_id => $profile) {
    ...
    {
        return ["active_profile" => $_SESSION['zentraactiveprofile']];
    }
    ...
    /**
     * Return the current $CFG_ZENTRA
     *
     * @return array
     */
    protected function getZentraConfig()
    {
        return ['cfg_zentra' => Config::getSafeConfig()];
    }
    ...
    protected function getItem($itemtype, $id, $params = [])
    {
        ...
        if (
            ...
            && $params['with_disks']
            && in_array($itemtype, $CFG_ZENTRA['itemdeviceharddrive_types'])
        ) {
            // build query to retrive filesystems
            $fs_iterator = $DB->request([
                'SELECT'    => [
                    'zentra_filesystems.name AS fsname',
                    'zentra_items_disks.*',
                ],
        ...
        if (
            ...
            && in_array($itemtype, $CFG_ZENTRA['software_types'])
        ) {
            $fields['_softwares'] = [];
            if (!Software::canView()) {
                ...
                $soft_iterator = $DB->request([
                    'SELECT'    => [
                        ...
                        'zentra_softwares.id AS softwares_id',
                        'zentra_softwareversions.id AS softwareversions_id',
                        'zentra_items_softwareversions.is_dynamic',
                        'zentra_softwareversions.states_id',
                        'zentra_softwares.is_valid',
                    ],
                    ...
                    'WHERE'     => [
                        'zentra_items_softwareversions.items_id'   => $id,
        ...
        if (
            ...
            $fields['_connections'] = [];
            foreach ($CFG_ZENTRA["directconnect_types"] as $connect_type) {
        ...
        if (
            ...
            if (!Contract::canView()) {
                ...
                $iterator = $DB->request([
                    ...
                    'WHERE'     => [
                        'zentra_contracts_items.items_id'  => $id,
                        'zentra_contracts_items.itemtype'  => $itemtype,
                    ] + getEntitiesRestrictCriteria('zentra_contracts', '', '', true),
                    'ORDERBY'   => 'zentra_contracts.name',
                ]);
                foreach ($iterator as $data) {
        ...
        if (
            ...
            if (!Ticket::canView()) {
                ...
                $criteria['WHERE'] = [
                    'zentra_items_tickets.items_id' => $id,
        ...
        if (
            ...
            if (!Problem::canView()) {
                ...
                $criteria['WHERE'] = [
                    'zentra_items_problems.items_id' => $id,
        ...
        if (
            ...
            if (!Change::canView()) {
                ...
                $criteria['WHERE'] = [
                    'zentra_changes_items.items_id' => $id,
                    'zentra_changes_items.itemtype' => $itemtype,
    ...
    protected function getItems($itemtype, $params = [], &$totalcount = 0)
    {
        ...
        $default = ['expand_dropdowns' => false,
            'get_hateoas'       => true,
            'only_id'           => false,
            'range'             => "0-" . ($_SESSION['zentralist_limit'] - 1),
            'sort'              => "id",
            'order'             => "ASC",
            'searchText'        => null,
            'is_deleted'        => false,
        ...
        if ($item->getType() == 'Entity') {
            ...
            $entity_restrict = getEntitiesRestrictCriteria($itemtype::getTable(), '', $_SESSION['zentraactiveentities'], $item->maybeRecursive(), true);
     ...
     *    - 'with_logs':         Retrieve historical. Optional.
     *
     * @return array collection of zentra object's fields
     */
    ...
    /**
     * Expose the ZENTRA searchEngine
     *
     * @param string $itemtype itemtype (class) of object
     * @param array  $params   with those options :
     ...
     *                      Optional.
     *    - 'rawdata': boolean for displaying raws data of Search engine of zentra (like sql request, and full searchoptions)
     *
    ...
    protected function searchItems($itemtype, $params = [])
    {
        ...
        if (isset($params['range'])) {
            ...
            $params['range'] = [0, $_SESSION['zentralist_limit'] - 1];
        }
        ...
        if (is_array($input)) {
            ...
            foreach ($input as $object) {
                ...
                if (!$item->can(-1, CREATE, $object)) {
                    ...
                    if (!isset($object['entities_id'])) {
                        $object['entities_id'] = $_SESSION['zentraactive_entity'];
                    }
                    ...
                    try {
                        ...
                        $message = $this->getZentraLastMessage();
            ...
            if ($isMultiple) {
                if ($failed == count($input)) {
                    $this->returnError($idCollection, 400, "ERROR_ZENTRA_ADD", false);
                } elseif ($failed > 0) {
                    $this->returnError($idCollection, 207, "ERROR_ZENTRA_PARTIAL_ADD", false);
                }
            } else {
                if ($failed > 0) {
                    $this->returnError($idCollection[0]['message'], 400, "ERROR_ZENTRA_ADD", false);
                } else {
    ...
    protected function updateItems($itemtype, $params = [])
    {
        ...
        if (is_array($input)) {
            ...
            if ($isMultiple) {
                if ($failed == count($input)) {
                    $this->returnError($idCollection, 400, "ERROR_ZENTRA_UPDATE", false);
                } elseif ($failed > 0) {
                    $this->returnError($idCollection, 207, "ERROR_ZENTRA_PARTIAL_UPDATE", false);
                }
            } else {
                if ($failed > 0) {
                    $this->returnError($idCollection[0]['message'], 400, "ERROR_ZENTRA_UPDATE", false);
                } else {
    ...
    /**
     * Delete one or more objects in ZENTRA
     *
     * @param string $itemtype itemtype (class) of object
     * @param array  $params   with those options :
     *    - 'input' : Array of objects with fields of itemtype to be updated.
    ...
    protected function deleteItems($itemtype, $params = [])
    {
        ...
        if (is_array($input)) {
            ...
            if ($isMultiple) {
                if ($failed == count($input)) {
                    $this->returnError($idCollection, 400, "ERROR_ZENTRA_DELETE", false);
                } elseif ($failed > 0) {
                    $this->returnError($idCollection, 207, "ERROR_ZENTRA_PARTIAL_DELETE", false);
                }
            } else {
                if ($failed > 0) {
                    $this->returnError($idCollection[0]['message'], 400, "ERROR_ZENTRA_DELETE", false);
                } else {
                    return $idCollection; // Return collection, even if the request affects a single item
                }
            }
    ...
    protected function lostPassword($params = [])
    {
        ...
        if ($CFG_ZENTRA['use_notifications'] == '0' || $CFG_ZENTRA['notifications_mailing'] == '0') {
        ...
        if (!isset($params['password_forget_token'])) {
            ...
            return [
                __('If the given email address corresponds to one and only one ZENTRA user, you will receive an email containing the information required to reset your password. Please contact your administrator if you do not receive an email.'),
            ];
        } else {
    ...
    /**
     * Log usage of the api into zentra historical or log files (defined by api config)
     *
     ...
     */
    private function getZentraLastMessage()
    {
        $all_messages             = [];
    ...
    public function inlineDocumentation($file)
    {
        ...
        $documentation = file_get_contents(ZENTRA_ROOT . '/' . $file);
        // language=Twig
            ...
            <script type="module">
                import('/js/modules/Monaco/MonacoEditor.js').then(() => {
                    ...
                    lang_elements.each((index, element) => {
                        ...
                        window.ZENTRA.Monaco.colorizeText(code, lang).then((html) => {
                            el.html(html);
                        });
                    });
                });
...
TWIG, ['md' => (new MarkdownRenderer())->render($documentation)]);
        ...
        Html::nullFooter();
        exit(); // @phpstan-ignore zentra.forbidExit (API response is streamed)
    }
    ...
    public static function getHatoasClasses($itemtype)
    {
        ...
        $hclasses = [];
        if (in_array($itemtype, $CFG_ZENTRA["reservation_types"])) {
            $hclasses[] = "ReservationItem";
        }
        if (in_array($itemtype, $CFG_ZENTRA["document_types"])) {
            $hclasses[] = "Document_Item";
        }
        if (in_array($itemtype, $CFG_ZENTRA["contract_types"])) {
            $hclasses[] = "Contract_Item";
        }
        if (in_array($itemtype, $CFG_ZENTRA["infocom_types"])) {
            $hclasses[] = "Infocom";
        }
        if (in_array($itemtype, $CFG_ZENTRA["ticket_types"])) {
            $hclasses[] = "Item_Ticket";
        }if (in_array($itemtype, $CFG_ZENTRA["project_asset_types"])) {
            $hclasses[] = "Item_Project";
        }
        if (in_array($itemtype, $CFG_ZENTRA["networkport_types"])) {
            $hclasses[] = "NetworkPort";
        }
        if (in_array($itemtype, $CFG_ZENTRA["itemdevices_types"])) {
            //$hclasses[] = "Item_Devices";
            foreach ($CFG_ZENTRA['device_types'] as $device_type) {
                if (
                    (($device_type == "DeviceMemory")
                    && !in_array($itemtype, $CFG_ZENTRA["itemdevicememory_types"]))
                    || (($device_type == "DevicePowerSupply")
                    && !in_array($itemtype, $CFG_ZENTRA["itemdevicepowersupply_types"]))
                    || (($device_type == "DeviceNetworkCard")
                    && !in_array($itemtype, $CFG_ZENTRA["itemdevicenetworkcard_types"]))
                ) {
                    continue;
                }
        ...
        if (!NetworkEquipment::canView()) {
            ...
            foreach ($networkport_types as $networkport_type) {
                ...
                $netp_iterator = $DB->request([
                    ...
                    'FROM'      => 'zentra_networkports AS netp',
                    'LEFT JOIN' => [
                ...
                foreach ($netp_iterator as $data) {
                    if (isset($data['netport_id'])) {
                        ...
                        $netn_iterator = $DB->request([
                            ...
                            'FROM'      => [
                                'zentra_networknames AS netn',
                            ],
                            'LEFT JOIN' => [
                                'zentra_ipaddresses AS ipadr'               => [
                                    'ON' => [
                                        'ipadr'  => 'items_id',
                                        'netn'   => 'id',
                                        [
                                ...
                                'zentra_fqdns AS fqdn'                      => [
                                    'ON' => [
                                        'fqdn'   => 'id',
                                        'netn'   => 'fqdns_id',
                                    ],
                                ],
                                'zentra_ipaddresses_ipnetworks AS ipadnet'  => [
                                    'ON' => [
                                        'ipadnet'   => 'ipaddresses_id',
                                        'ipadr'     => 'id',
                                    ],
                                ],
                                'zentra_ipnetworks AS ipnet'                => [
                                    'ON' => [
                                        'ipnet'     => 'id',
                        ...
                        if (count($netn_iterator)) {
                            ...
                            foreach ($raw_ipadresses as $ipadress) {
                                ...
                                $ipnet_iterator = $DB->request([
                                    ...
                                    'FROM'         => 'zentra_ipnetworks AS ipnet',
                                    'INNER JOIN'   => [
                                        'zentra_ipaddresses_ipnetworks AS ipadnet' => [
                                            'ON' => [
                                                'ipadnet'   => 'ipnetworks_id',
    ...
    protected function userPicture($user_id)
    {
        ...
        if (!empty($user->fields['picture'])) {
            // Send file
            $file = ZENTRA_PICTURE_DIR . '/' . $user->fields['picture'];

            Toolbox::getFileAsResponse($file, $user->fields['picture'])->send();
        } else {
            // No content
            http_response_code(204); // @phpstan-ignore zentra.forbidHttpResponseCode (API response is streamed)
        }
        exit(); // @phpstan-ignore zentra.forbidExit (API response is streamed)
    }
    ...
    public function handleDepreciation(string $itemtype): string
    {
        ...
        if ($deprecated) {
            ...
            $class = "Zentra\Api\Deprecated\\$itemtype";
