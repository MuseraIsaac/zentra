...
class APIRest extends API
{
    ...
    public function manageUploadedFiles()
    {
        foreach (array_keys($_FILES) as $filename) {
            ...
            $upload_result
            = ZENTRAUploadHandler::uploadFiles(['name'           => $filename,
    ...
    public function call()
    {
        ...
        } elseif ($resource === "getZentraConfig") {
            // get complete php var $CFG_ZENTRA
            $this->returnResponse($this->getZentraConfig());
            ...
            switch ($this->verb) {
                ...
                case "GET": // retrieve item(s)
                    ...
                    if ($id !== false) {
                        ...
                        //add pagination headers
                        $range = [0, $_SESSION['zentralist_limit']];
    ...
    private function getItemtype($index = 0, $recursive = true, $all_assets = false)
    {
        ...
        if (isset($this->url_elements[$index])) {
            ...
            if ($all_assets || $valid_class) {
                ...
                if ($deprecated) {
                    $itemtype = "Zentra\Api\Deprecated\\$itemtype";
                }
                ...
                if ($deprecated) {
                    ...
                    $itemtype = str_replace("Zentra\Api\Deprecated\\", "", $itemtype);
                }
                return $itemtype;
            }
            $this->returnError(
    ...
    public function returnResponse($response, $httpcode = 200, $additionalheaders = [])
    {
        ...
        http_response_code($httpcode); // @phpstan-ignore zentra.forbidHttpResponseCode (API response is streamed)
    ...
    public function inlineDocumentation($file = "apirest.md")
    {
        ...
        if ($this->format == "html") {
            ...
            echo file_get_contents(ZENTRA_ROOT . '/' . $file);
        }
        exit(); // @phpstan-ignore zentra.forbidExit (API response is streamed)
    }
}
