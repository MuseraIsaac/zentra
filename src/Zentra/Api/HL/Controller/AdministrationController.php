            ...
            'User' => [
                    ...
                    'read' => static function () {
                        if (!Session::canViewAllEntities()) {
                            return [
                                ...
                                'WHERE' => [
                                    'zentra_profiles_users.entities_id' => $_SESSION['zentraactiveentities'],
                                ],
                            ];
                        }
                        return true;
                ...
                'properties' => [
                    ...
                    'authtype' => [
                        ...
                        'enum' => [Auth::DB_ZENTRA, Auth::MAIL, Auth::LDAP, Auth::EXTERNAL, Auth::CAS, Auth::X509],
                        'description' => <<<EOD
                            - 1: ZENTRA database
                            - 2: Email
                            - 3: LDAP
...
EOD,
                ...
                'pdf_font' => [
                    ...
                    'enum' => array_keys(\ZENTRAPDF::getFontList()),
                    'x-field' => 'pdffont',
                ],
...
EOT,
    ...
    private function getUserPictureResponse(string $username, ?string $picture_path): Response
    {
        if ($picture_path !== null) {
            $picture_path = ZENTRA_PICTURE_DIR . '/' . $picture_path;
        } else {
    ...
    private function getUsedOrManagedItems(int $users_id, bool $is_managed, array $request_params, string $api_version): Response
    {
        ...
        $schema = Doc\Schema::getUnionSchemaForItemtypes(
            itemtypes: array_filter($CFG_ZENTRA['assignable_types'], static function ($t) use ($is_managed) {
