...
final class ManagementController extends AbstractController
{
    ...
    protected static function getRawKnownSchemas(): array
    {
        ...
        foreach ($management_types as $m_class => $m_data) {
            ...
            if (in_array($m_class, $CFG_ZENTRA['state_types'], true)) {
                $schemas[$m_name]['properties']['status'] = self::getDropdownTypeSchema(class: State::class, full_schema: 'State');
            }

            if (in_array($m_class, $CFG_ZENTRA['location_types'], true)) {
            ...
            if (in_array($m_class, $CFG_ZENTRA['assignable_types'], true)) {
                ...
                $schemas[$m_name]['properties']['group_tech'] = [
                    ...
                    'items' => [
                        ...
                        'x-full-schema' => 'Group',
                        'x-join' => [
                            'table' => 'zentra_groups', // The table with the desired data
                            'fkey' => 'groups_id',
                            'field' => 'id',
                            'ref-join' => [
        ...
        $schemas['Document']['properties']['filepath'] = [
            'type' => Doc\Schema::TYPE_STRING,
            'x-mapped-from' => 'id',
            'x-mapper' => static fn($v) => $CFG_ZENTRA["root_doc"] . "/front/document.send.php?docid=" . $v,
            'readOnly' => true,
        ];
