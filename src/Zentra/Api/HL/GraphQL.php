...
final class GraphQL
{
    ...
    {
        /** @var CacheInterface $ZENTRA_CACHE */
        global $ZENTRA_CACHE;

        $api_version = $request->getHeaderLine('ZENTRA-API-Version') ?: Router::API_VERSION;
        ...
        if (Environment::get() === Environment::TESTING || !$ZENTRA_CACHE->has($cache_key)) {
            $parsed = Parser::parse(source: $schema_str, options: ['assumeValid' => true, 'assumeValidSDL' => true]);
            if (Environment::get() !== Environment::TESTING) {
                $ZENTRA_CACHE->set($cache_key, $parsed);
            }
        } else {
            $parsed = $ZENTRA_CACHE->get($cache_key);
        }
