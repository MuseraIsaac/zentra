...
namespace Zentra\Api\HL\Middleware;
...
class DebugResponseMiddleware extends AbstractMiddleware implements ResponseMiddlewareInterface
{
    public function process(MiddlewareInput $input, callable $next): void
    {
        ...
        $use_mode = isset($_SESSION['zentra_use_mode']) ? (int) $_SESSION['zentra_use_mode'] : Session::NORMAL_MODE;
        ...
        foreach ($outputs as $output) {
            ...
            $node = $crawler->filter('div.zentra-debug-alert');
            if ($node->count() > 0) {
