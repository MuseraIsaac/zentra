...
final class OpenAPIGenerator
{
    ...
    {
        $description = <<<EOT
The High-Level REST API documentation shown here is dynamically generated from the core ZENTRA code and any enabled plugins.
If a plugin is not enabled, its routes will not be shown here.
EOT;

        return [
            'title' => 'ZENTRA High-Level REST API',
    ...
    public function getSchema(): array
    {
        ...
        $schema = [
            ...
            'servers' => [
                [
                    'url' => $CFG_ZENTRA['url_base'] . '/api.php',
                    'description' => 'ZENTRA High-Level REST API',
                ],
            ],
            'components' => [
    ...
    private function getSecuritySchemeComponents(): array
    {
        ...
        return [
            'oauth' => [
                ...
                'flows' => [
                    'authorizationCode' => [
                        'authorizationUrl' => $CFG_ZENTRA['root_doc'] . '/api.php/authorize',
                        'tokenUrl' => $CFG_ZENTRA['root_doc'] . '/api.php/token',
                        'refreshUrl' => $CFG_ZENTRA['root_doc'] . '/api.php/token',
                        'scopes' => $scopes,
                    ],
                    'password' => [
                        'tokenUrl' => $CFG_ZENTRA['root_doc'] . '/api.php/token',
    ...
    private function getPathSchemas(RoutePath $route_path): array
    {
        ...
        foreach ($route_methods as $route_method) {
            ...
            if ($route_path->getRouteSecurityLevel() !== Route::SECURITY_NONE) {
                $path_schema['parameters']['ZENTRA-Entity'] = [
                    'name' => 'ZENTRA-Entity',
                    'in' => 'header',
                ...
                $path_schema['parameters']['ZENTRA-Profile'] = [
                    'name' => 'ZENTRA-Profile',
                    'in' => 'header',
                    'description' => 'The ID of the profile to use. If not specified, the default profile for the user is used.',
                    'schema' => ['type' => Doc\Schema::TYPE_INTEGER],
                ];
                $path_schema['parameters']['ZENTRA-Entity-Recursive'] = [
                    'name' => 'ZENTRA-Entity-Recursive',
                    'in' => 'header',
