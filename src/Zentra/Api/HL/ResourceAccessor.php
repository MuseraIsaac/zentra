...
use Zentra\Api\HL\RSQL\RSQLException;
use Zentra\Api\HL\Search\SearchContext;
...
/**
 * Class contaning methods for accessing ZENTRA resources (items) from the HL API via schemas.
 */
final class ResourceAccessor
{
    ...
    /**
     * Map the request parameters to the format required for the ZENTRA add/update methods.
    ...
    public static function updateBySchema(array $schema, array $request_attrs, array $request_params, string $field = 'id'): Response
    {
        ...
        // We should return the updated item but we NEVER return the ZENTRA item fields directly. Need to use special API methods.
    ...
    {
        if (!isset($request_params['entity']) && isset($_SESSION['zentraactive_entity'])) {
            $request_params['entity'] = $_SESSION['zentraactive_entity'];
        }
        $input = self::getInputParamsBySchema($schema, $request_params);
