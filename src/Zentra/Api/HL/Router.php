...
use Zentra\Api\HL\Controller\ComponentController;
use Zentra\Api\HL\Controller\CoreController;
use Zentra\Api\HL\Controller\CRUDControllerTrait;
use Zentra\Api\HL\Controller\CustomAssetController;
use Zentra\Api\HL\Controller\DashboardController;
use Zentra\Api\HL\Controller\DropdownController;
use Zentra\Api\HL\Controller\GraphQLController;
use Zentra\Api\HL\Controller\ITILController;
use Zentra\Api\HL\Controller\KnowbaseController;
use Zentra\Api\HL\Controller\ManagementController;
use Zentra\Api\HL\Controller\NotepadController;
use Zentra\Api\HL\Controller\ProjectController;
use Zentra\Api\HL\Controller\ReportController;
use Zentra\Api\HL\Controller\RuleController;
use Zentra\Api\HL\Controller\SetupController;
use Zentra\Api\HL\Controller\ToolController;
use Zentra\Api\HL\Middleware\AbstractMiddleware;
use Zentra\Api\HL\Middleware\AuthMiddlewareInterface;
use Zentra\Api\HL\Middleware\CookieAuthMiddleware;
use Zentra\Api\HL\Middleware\CRUDRequestMiddleware;
use Zentra\Api\HL\Middleware\DebugRequestMiddleware;
use Zentra\Api\HL\Middleware\DebugResponseMiddleware;
use Zentra\Api\HL\Middleware\InternalAuthMiddleware;
use Zentra\Api\HL\Middleware\IPRestrictionRequestMiddleware;
use Zentra\Api\HL\Middleware\MiddlewareInput;
use Zentra\Api\HL\Middleware\OAuthRequestMiddleware;
use Zentra\Api\HL\Middleware\RequestMiddlewareInterface;
use Zentra\Api\HL\Middleware\ResponseMiddlewareInterface;
use Zentra\Api\HL\Middleware\ResultFormatterMiddleware;
use Zentra\Api\HL\Middleware\RSQLRequestMiddleware;
use Zentra\Api\HL\Middleware\SecurityResponseMiddleware;
...
The low-level API which is closely tied to the ZENTRA source code.
...
EOT;
        return [
            [
                ...
                'endpoint'   => $CFG_ZENTRA['url_base'] . '/api.php/v1',
            ],
            [
                'api_version' => '2',
                'version' => '2.0.0',
                'endpoint' => $CFG_ZENTRA['url_base'] . '/api.php/v2.0',
            ],
            [
                'api_version' => '2',
                'version' => '2.1.0',
                'endpoint' => $CFG_ZENTRA['url_base'] . '/api.php/v2.1',
            ],
            [
                'api_version' => '2',
                'version' => '2.2.0',
                'endpoint' => $CFG_ZENTRA['url_base'] . '/api.php/v2.2',
            ],
        ];
    }
    ...
    private function cacheRoutes(array $routes): void
    {
        ...
        $ZENTRA_CACHE->set('hlapi_routes', $hints, 5 * MINUTE_TIMESTAMP);
    }
    ...
    private function getRoutesFromCache(): array
    {
        ...
        $routes = [];
        $hints = $ZENTRA_CACHE->get('hlapi_routes') ?? [];
        if (empty($hints)) {
    ...
    public function getAllRoutePaths(bool $force_all = false): array
    {
        ...
        $paths = [];
        $is_user_authenticated = isset($_SESSION['zentraID']);
        foreach ($routes as $route) {
    ...
    public function matchAll(Request $request): array
    {
        ...
        $api_version = self::normalizeAPIVersion($request->getHeaderLine('ZENTRA-API-Version') ?: static::API_VERSION);
    ...
    private function handleAuth(Request $request): void
    {
        if ($request->hasHeader('Authorization')) {
            ...
            if ($request->hasHeader('ZENTRA-Profile')) {
                $requested_profile = $request->getHeaderLine('ZENTRA-Profile');
                if (is_numeric($requested_profile)) {
                    Session::changeProfile((int) $requested_profile);
                }
            }
            if ($request->hasHeader('ZENTRA-Entity')) {
                $requested_entity = $request->getHeaderLine('ZENTRA-Entity');
                if (is_numeric($requested_entity)) {
                    $is_recursive = $request->hasHeader('ZENTRA-Entity-Recursive') && strtolower($request->getHeaderLine('ZENTRA-Entity-Recursive')) === 'true';
