...
class ImportMapGenerator
{
     ...
     */
    public function __construct(string $root_doc, string $zentra_root, ?CacheInterface $cache = null)
    {
    ...
    public static function getInstance(): ImportMapGenerator
    {
        ...
        if (self::$instance === null) {
            self::$instance = new self($CFG_ZENTRA['root_doc'], ZENTRA_ROOT, $ZENTRA_CACHE);
        }
    ...
    public function generate(): array
    {
        ...
        // Try to get ZENTRA core modules from cache first
        ...
        if (!$core_modules) {
            // Scan ZENTRA core directories
            $core_modules = ['imports' => []];
            $this->addModulesToImportMap($core_modules, $this->zentra_root . '/js/modules', $this->zentra_root);
            $this->addModulesToImportMap($core_modules, $this->zentra_root . '/public/js/modules', $this->zentra_root);
            $this->addModulesToImportMap($core_modules, $this->zentra_root . '/public/lib', $this->zentra_root);
            $this->addModulesToImportMap($core_modules, $this->zentra_root . '/public/build', $this->zentra_root);
    ...
    private function addModulesToImportMap(
        ...
        foreach ($iterator as $file) {
            if ($file->isFile() && $file->getExtension() === 'js') {
                ...
                // Get the path that would be used for a ZENTRA located at the server root dir (e.g. `/js/modules/Foo.js`)
