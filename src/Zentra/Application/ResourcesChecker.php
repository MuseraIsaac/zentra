...
final class ResourcesChecker
{
    ...
    public function checkResources(): void
    {
        if ($this->isSourceCodeMixedOfMultipleVersions()) {
            echo 'Source code files of a previous ZENTRA version were detected.' . PHP_EOL;
            echo 'Please update ZENTRA by following the procedure described in the installation documentation.' . PHP_EOL;
            exit(1); // @phpstan-ignore zentra.forbidExit (Script execution should be stopped to prevent further errors)
        }
        ...
        if (!$this->areDependenciesUpToDate()) {
            ...
            echo 'Run "php bin/console dependencies install" in the zentra tree to fix this.' . PHP_EOL;
            exit(1); // @phpstan-ignore zentra.forbidExit (Script execution should be stopped to prevent further errors)
        }
        if ($this->shouldLocalesBeChecked() && !$this->areLocalesUpToDate()) {
            echo 'Application locales have to be compiled.' . PHP_EOL;
            echo 'Run "php bin/console tools:locales:compile" in the zentra tree to fix this.' . PHP_EOL;
            exit(1); // @phpstan-ignore zentra.forbidExit (Script execution should be stopped to prevent further errors)
        }
    }

    /**
     * Check if the ZENTRA source code files seems to contain a mix of multiple ZENTRA versions.
     */
    ...
    private function areLocalesUpToDate(): bool
    {
        ...
        if (count($mo_files) < count($po_files)) {
            return false;
        } elseif (file_exists($this->root_dir . '/locales/zentra.pot')) {
            // Assume that `locales/zentra.pot` file only exists when installation mode is GIT
    ...
    private function shouldCheckResources(): bool
    {
        ...
        require_once($this->root_dir . '/src/Zentra/Application/Environment.php');
