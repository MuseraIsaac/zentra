...
class RoutingExtension extends AbstractExtension
{
    ...
    public function path(string $resource, array $parameters = []): string
    {
        if ($this->router) {
            try {
                // Symfony's router must take precedence over ZENTRA's router, for forwards compatibility
                return $this->router->generate($resource, $parameters, UrlGeneratorInterface::ABSOLUTE_PATH);
            } catch (RouteNotFoundException) {
                // Fallback to ZENTRA router in this case for consistency
            }
        }
