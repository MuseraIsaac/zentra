...
use Dropdown;
use Zentra\Asset\Capacity\CapacityInterface;
use Zentra\Asset\CustomFieldType\TypeInterface;
use Zentra\CustomObject\AbstractDefinition;
use Zentra\CustomObject\AbstractDefinitionManager;
...
final class AssetDefinitionManager extends AbstractDefinitionManager
{
    ...
    private function __construct()
    {
        ...
        foreach ($directory_iterator as $file) {
            $classname = $file->getExtension() === 'php'
                ? 'Zentra\\Asset\\Capacity\\' . $file->getBasename('.php')
                : null;
            if (
                $classname !== null
        ...
        if ($this->custom_field_types === null) {
            ...
            foreach ($directory_iterator as $file) {
                ...
                $classname = $file->getExtension() === 'php'
                    ? 'Zentra\\Asset\\CustomFieldType\\' . $file->getBasename('.php')
                    : null;
    ...
    public function bootstrapDefinition(AbstractDefinition $definition): void
    {
        ...
        foreach ($config_keys as $config_key) {
            if (!in_array($asset_class_name, $CFG_ZENTRA[$config_key], true)) {
                $CFG_ZENTRA[$config_key][] = $asset_class_name;
            }
        }

        // Add type and model to dictionnary config entry
        if (!in_array($definition->getAssetTypeClassName(), $CFG_ZENTRA['dictionnary_types'], true)) {
            $CFG_ZENTRA['dictionnary_types'][] = $definition->getAssetTypeClassName();
        }
        if (!in_array($definition->getAssetModelClassName(), $CFG_ZENTRA['dictionnary_types'], true)) {
            $CFG_ZENTRA['dictionnary_types'][] = $definition->getAssetModelClassName();
        }
        ...
        CommonZENTRA::registerStandardTab(
            $asset_class_name,
            Item_Ticket::class,
            51
        );
        ...
        if ($this->allowed_dropdown_itemtypes === null) {
            $this->allowed_dropdown_itemtypes = [
                _n('Asset', "Assets", Session::getPluralNumber()) => array_combine(
                    $CFG_ZENTRA['asset_types'],
                    array_map(static fn($t) => $t::getTypeName(1), $CFG_ZENTRA['asset_types'])
                ),
            ];
...
use Zentra\\Asset\\RuleDictionaryModel;
...
use Zentra\\Asset\\RuleDictionaryType;
...
use Zentra\\Asset\\RuleDictionaryModelCollection;
...
use Zentra\\Asset\\RuleDictionaryTypeCollection;
