...
namespace Zentra\CalDAV\Backend;

use CommonDBTM;
use Zentra\CalDAV\Contracts\CalDAVCompatibleItemInterface;
use Zentra\CalDAV\Node\Property;
use Zentra\CalDAV\Traits\CalDAVUriUtilTrait;
use Group;
use Planning;
...
class Calendar extends AbstractBackend
{
        ...
        foreach ($calendars_params as $key => $calendar_data) {
            $calendars[] = [
                ...
                Property::CAL_SUPPORTED_COMPONENTS => new SupportedCalendarComponentSet(
                    $CFG_ZENTRA['caldav_supported_components']
                ),
    ...
    private function convertVCalendarToCalendarObject(VCalendar $vcalendar)
    {
        ...
        $vcalendar->PRODID = '-//zentra-project.org//ZENTRA CalDAV server//EN';
    ...
    private function storeCalendarObject($calendarId, $calendarData, (CalDAVCompatibleItemInterface&CommonDBTM)|null $item = null)
    {
        ...
        if (!in_array($vcomponent->name, $CFG_ZENTRA['caldav_supported_components'])) {
        ...
        if ($item->isNewItem()) {
            ...
            if (
                ...
                && array_key_exists('zentraactive_entity', $_SESSION)
            ) {
