...
namespace Zentra\Console;

use DBmysql;
use Zentra\Application\Environment;
use Zentra\Console\Command\ConfigurationCommandInterface;
use Zentra\Console\Command\ZentraCommandInterface;
use Zentra\Console\Exception\EarlyExitException;
use Zentra\Error\ErrorDisplayHandler\ConsoleErrorDisplayHandler;
use Zentra\Kernel\Kernel;
use Zentra\System\Requirement\RequirementInterface;
...
class Application extends BaseApplication
{
    ...
    public function __construct(private Kernel $kernel)
    {
        ...
        parent::__construct('ZENTRA CLI', ZENTRA_VERSION);

        $this->kernel->boot();
        $this->setDispatcher($this->kernel->getContainer()->get('event_dispatcher'));

        $this->db = $DB;
        $this->config = &$CFG_ZENTRA;

        // Force the current "username"
        $_SESSION['zentraname'] = 'cli';
    ...
    protected function getDefaultInputDefinition(): InputDefinition
    {
        ...
        $definition = [
            ...
            new InputOption(
                ...
                null,
                InputOption::VALUE_OPTIONAL,
                __('Output language (default value is existing ZENTRA "language" configuration or "en_GB")')
            ),
        ];
    ...
    protected function configureIO(InputInterface $input, OutputInterface $output)
    {
        ...
        if (null !== $lang && !array_key_exists($lang, $CFG_ZENTRA['languages'])) {
    ...
    protected function getCommandName(InputInterface $input): ?string
    {
        ...
        if ($name !== null) {
            // strip `zentra:` prefix that was used before ZENTRA 10.0.6
            // FIXME Deprecate usage of `zentra:` prefix in ZENTRA 11.0.
            $name = preg_replace('/^zentra:/', '', $name);
        }
        return $name;
    }
    ...
    protected function doRunCommand(Command $command, InputInterface $input, OutputInterface $output)
    {
        ...
        if (
            $is_db_available
            && ZENTRA_SKIP_UPDATES
            && (!($command instanceof ZentraCommandInterface) || $command->requiresUpToDateDb())
            && !Update::isDbUpToDate()
        ) {
            $output->writeln(
            ...
            $is_db_available
            && $command instanceof ZentraCommandInterface
            && $command->requiresUpToDateDb()
            && !Update::isDbUpToDate()
        ) {
            $output->writeln(
                '<error>'
                . __('The ZENTRA codebase has been updated. The update of the ZENTRA database is necessary.')
                . '</error>'
                . PHP_EOL
                . '<error>'
        ...
        if (
            $command instanceof ZentraCommandInterface && $command->mustCheckMandatoryRequirements()
    ...
    private function computeAndLoadOutputLang()
    {
        ...
        // 2. Check in ZENTRA configuration
        if (
        ...
        if ($lang !== $_SESSION['zentralanguage']) {
    ...
    private function checkConfigWriteAccess(Command $command, InputInterface $input): bool
    {
        ...
        $config_files_to_update = array_map(
            fn($path) => ZENTRA_CONFIG_DIR . DIRECTORY_SEPARATOR . $path,
