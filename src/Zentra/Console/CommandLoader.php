...
class CommandLoader implements CommandLoaderInterface
{
     ...
     */
    public function __construct($include_plugins = true, $rootdir = ZENTRA_ROOT, ?Plugin $plugin = null)
    {
            ...
            foreach ($names as $name) {
                foreach (['tools', 'plugins'] as $protected_namespaces) {
                    if (preg_match('/^' . $protected_namespaces . ':/', $name) === 1) {
                        ...
                        trigger_error(
                            sprintf(
                                'ZENTRA command `%s` must be moved outside the protected `%s` namespace.',
                                $name,
        ...
        foreach ($this->plugin->getPlugins() as $plugin_key) {
            ...
            foreach (['inc', 'src'] as $source_dir) {
                ...
                foreach ($plugin_files as $file) {
                    ...
                    // Prefixes can be:
                    // - ZentraPlugin\Myplugin if class is namespaced
     ...
     * @param string      $basedir   Directory containing classes (eg ZENTRA_ROOT . '/inc')
     * @param array       $prefixes  Possible prefixes to add to classname (eg 'PluginExample', 'ZentraPlugin\Example')
     *
     * @return null|Command
     */
    private function getCommandFromFile(SplFileInfo $file, $basedir, array $prefixes = []): ?Command
    {
        ...
        foreach ($prefixes as $prefix) {
            ...
            if (!class_exists($classname_to_check, false)) {
                ...
                // Needed as a file located in root source dir of Zentra can be either namespaced either not.
                continue;
            }
