...
class CheckSchemaIntegrityCommand extends AbstractCommand
{
    ...
    /**
     * Error code returned when the ZENTRA version cannot be found.
     */
    ...
    protected function configure()
    {
        ...
        $this->addOption(
            ...
            __('Plugin to check. If option is not used, checks will be done on ZENTRA core database tables.')
        );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($plugin_key !== null) {
            ...
            $context = 'core';
            $installed_version = $CFG_ZENTRA['dbversion'] ?? $CFG_ZENTRA['version'] ?? null; // `dbversion` has been added in ZENTRA 9.2

            if ($installed_version === null) {
                throw new EarlyExitException(
                    '<error>' . __('Unable to fetch ZENTRA version.') . '</error>',
        ...
        if (!$checker->hasTables($context)) {
            $message = $plugin_key === null
                ? __('The database contains no ZENTRA tables.')
