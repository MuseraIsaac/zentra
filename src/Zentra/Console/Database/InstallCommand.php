...
class InstallCommand extends AbstractConfigureCommand implements ConfigurationCommandInterface
{
    ...
    /**
     * Error code returned when trying to install and having a DB already containing zentra_* tables.
     *
     * @var int
     */
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!$this->isDbAlreadyConfigured() || $input->getOption('reconfigure')) {
            ...
            // Ensure global $DB is updated (used by ZENTRAKey)
            global $DB;
            $DB = $this->db;
        ...
        $tables_result = $mysqli->query(
          ...
          WHERE table_schema = '" . $mysqli->real_escape_string($db_name) . "'
             ...
             AND table_name LIKE 'zentra\_%'"
        );
        if (!$tables_result) {
            throw new RuntimeException('Unable to check ZENTRA tables existence.');
        }
        if ($tables_result->fetch_array()[0] > 0 && !$force) {
            $output->writeln(
                '<error>' . __('Database already contains "zentra_*" tables. Use --force option to override existing database.') . '</error>'
            );
    ...
    public function getConfigurationFilesToUpdate(InputInterface $input): array
    {
        $config_files_to_update = [
            'zentracrypt.key', // key is regenerated everytime
        ];
