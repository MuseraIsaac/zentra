...
namespace Zentra\Console\Database;

use DBmysql;
use Zentra\Cache\CacheManager;
use Zentra\Console\AbstractCommand;
use Zentra\Console\Command\ConfigurationCommandInterface;
use Zentra\Console\Exception\EarlyExitException;
use Zentra\Console\Traits\TelemetryActivationTrait;
use Zentra\Progress\ConsoleProgressIndicator;
...
class UpdateCommand extends AbstractCommand implements ConfigurationCommandInterface
{
    ...
    /**
     * Error code returned when database is not a valid ZENTRA database.
     *
     * @var int
     */
    ...
    protected function configure()
    {
        ...
        $this->addOption(
            ...
            'f',
            InputOption::VALUE_NONE,
            __('Force execution of update from v-1 version of ZENTRA even if schema did not changed')
        );
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        // Initialize entities
        $_SESSION['zentradefault_entity'] = 0;
        ...
        if ($current_version === null) {
            $msg = sprintf(
                __('Current ZENTRA version not found for database named "%s". Update cannot be done.'),
                $this->db->dbdefault
            );
        ...
        $informations->addRow([__('ZENTRA version'), $current_version, ZENTRA_VERSION]);
        $informations->addRow([
            __('ZENTRA database version'),
            $this->getPrettyDbVersion($current_db_version),
            $this->getPrettyDbVersion(ZENTRA_SCHEMA_VERSION),
        ]);
        $informations->render();
        ...
        if (VersionParser::isStableRelease($current_version) && !VersionParser::isStableRelease(ZENTRA_VERSION) && !$allow_unstable) {
        ...
        if ($this->input->getOption('skip-db-checks')) {
            $this->output->writeln(
                [
                    ...
                    '<comment>' . sprintf(
                        __('It is recommended to run the "%s" command to validate that the database schema is consistent with the current ZENTRA version.'),
            ...
            // Many old ZENTRA instances will likely have differences, and people will have to fix them manually.
            //
            ...
            $this->output->writeln(
                [
                    '<error>' . __('The database schema is not consistent with the current ZENTRA version.') . '</error>',
                    '<error>' . sprintf(
    ...
    private function checkSchemaIntegrity(string $installed_version): void
    {
        ...
        try {
            ...
            if (count($differences) > 0) {
                ...
                $error = sprintf(__('The database schema is not consistent with the installed ZENTRA version (%s).'), $install_version_nohash)
                    . ' '
