...
namespace Zentra\Console\Diagnostic;
...
class CheckSourceCodeIntegrityCommand extends AbstractCommand
{
    ...
    protected function initialize(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($input->getOption('diff')) {
            // Generating diff would require to load the ZENTRA archive in memory, and that requires about 400MB of memory.
    ...
    protected function configure()
    {
        ...
        $this->setDescription(__('Check ZENTRA source code file integrity.'));
        $this->addOption(
            'diff',
            'd',
        ...
        $this->addOption(
            ...
            null,
            InputOption::VALUE_NONE,
            __('Allow downloading the ZENTRA release if needed'),
        );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        try {
            ...
            $output->writeln('<error>' . sprintf(__('Failed to validate ZENTRA source code integrity. Error was: %s'), $e->getMessage()) . '</error>');
            return 1;
        }
        ...
        if ($all_ok) {
            $output->writeln('<info>' . __('ZENTRA source code integrity is validated.') . '</info>');
            return 0;
        }

        $output->writeln('<error>' . __('ZENTRA source code integrity is not validated.') . '</error>');
        ...
        if ($diff) {
            $errors = [];
            if (VersionParser::isDevVersion(ZENTRA_VERSION)) {
    ...
    protected function interact(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($input->getOption('diff') && !$input->getOption('allow-download')) {
            ...
            $question = new ConfirmationQuestion(
                __('Generating the source code diff could require downloading the ZENTRA release archive. Do you want to allow this operation?'),
                false
            );
