...
class SynchronizeUsersCommand extends AbstractCommand
{
    ...
    protected function configure()
    {
        ...
        $cfg_value_user = $CFG_ZENTRA['user_deleted_ldap_user'] ?? __('unknown');
        $cfg_value_groups = $CFG_ZENTRA['user_deleted_ldap_groups'] ?? __('unknown');
        $cfg_value_authorizations = $CFG_ZENTRA['user_deleted_ldap_authorizations'] ?? __('unknown');
        $description = sprintf(
        ...
        $description = sprintf(
            ...
            ($CFG_ZENTRA['user_restored_ldap'] ?? __('unknown'))
        );
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($only_create) {
            ...
            if (null !== $deleted_user_strategies) {
                ...
                $CFG_ZENTRA['user_deleted_ldap_user'] = $deleted_user_strategies[0];
                $CFG_ZENTRA['user_deleted_ldap_groups'] = $deleted_user_strategies[1];
                $CFG_ZENTRA['user_deleted_ldap_authorizations'] = $deleted_user_strategies[2];
            }
            $restored_user_strategy = $input->getOption('restored-user-strategy');
            if (null !== $restored_user_strategy) {
                $CFG_ZENTRA['user_restored_ldap'] = $restored_user_strategy;
            }
        }
