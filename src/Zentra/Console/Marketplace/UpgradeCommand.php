...
use Zentra\Marketplace\Api\Plugins;
...
class UpgradeCommand extends AbstractCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($local_versions as $plugin_key => $local_version) {
            if (!\file_exists(ZENTRA_MARKETPLACE_DIR . '/' . $plugin_key)) {
                $msg = '<comment>'
            ...
            if ($latest_version === null) {
                $msg = '<comment>'
                    . sprintf(__('Plugin "%s" is not available for your ZENTRA version.'), $plugin_key)
                    . '</comment>'
                ;
                $output->writeln($msg);
                continue;
