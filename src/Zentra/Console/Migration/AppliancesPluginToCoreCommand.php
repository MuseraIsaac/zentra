...
class AppliancesPluginToCoreCommand extends AbstractCommand
{
    ...
    public const PLUGIN_APPLIANCE_TABLES = [
        "zentra_plugin_appliances_appliances"       => [
            "id",
            "entities_id",
            "is_recursive",
            "name",
        ...
        "zentra_plugin_appliances_appliancetypes"   => ["id","entities_id","is_recursive","name","comment"],
        "zentra_plugin_appliances_appliances_items" => ["id", "plugin_appliances_appliances_id","items_id","itemtype"],
        "zentra_plugin_appliances_environments"     => ["id","name","comment" ],
        "zentra_plugin_appliances_relations"        => ["id","plugin_appliances_appliances_items_id","relations_id"],
    ];
    ...
    protected function configure()
    {
        ...
        $this->setDescription(sprintf(__('Migrate %s plugin data into ZENTRA core tables'), 'Appliances'));
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!$no_interaction) {
            ...
            $output->writeln([
                __('You are about to launch migration of Appliances plugin data into ZENTRA core tables.'),
    ...
    {
        $this->output->writeln(
            '<comment>' . __('Updating ZENTRA itemtypes...') . '</comment>',
            OutputInterface::VERBOSITY_NORMAL
        );
        $itemtypes_tables = [
    ...
    private function createApplianceItems(): bool
    {
        ...
        $iterator = $this->db->request([
            'FROM' => 'zentra_plugin_appliances_appliances_items',
        ]);
    ...
    private function createApplianceEnvironments(): bool
    {
        ...
        $iterator = $this->db->request([
            'FROM' => 'zentra_plugin_appliances_environments',
        ]);
    ...
    private function createAppliances(): bool
    {
        ...
        $iterator = $this->db->request([
            'FROM' => 'zentra_plugin_appliances_appliances',
        ]);
    ...
    private function createApplianceTypes(): bool
    {
        ...
        $iterator = $this->db->request([
            'FROM' => 'zentra_plugin_appliances_appliancetypes',
        ]);
    ...
    private function createApplianceRelations(): bool
    {
        ...
        $iterator = $this->db->request([
            ...
            'FROM'         => 'zentra_plugin_appliances_relations AS rel',
            'INNER JOIN'   => [
                'zentra_plugin_appliances_appliances_items AS items' => [
                    'ON'  => [
                        'items' => 'id',
                        'rel'   => 'plugin_appliances_appliances_items_id',
                    ],
                ],
                'zentra_plugin_appliances_appliances AS app' => [
                    'ON'  => [
                        'app'   => 'id',
