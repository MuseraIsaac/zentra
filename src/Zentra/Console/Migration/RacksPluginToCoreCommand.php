...
class RacksPluginToCoreCommand extends AbstractCommand
{
    ...
    protected function configure()
    {
        ...
        $this->setDescription(sprintf(__('Migrate %s plugin data into ZENTRA core tables'), 'Racks'));
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!$no_interaction) {
            ...
            $output->writeln(
                [
                    __('You are about to launch migration of Racks plugin data into ZENTRA core tables.'),
    ...
    private function checkPlugin()
    {
        ...
        if ($check_version) {
            ...
            if (!$plugin->getFromDBbyDir('racks')) {
                $message  = __('Racks plugin is not part of ZENTRA plugin list. It has never been installed or has been cleaned.')
                 . ' '
                 . sprintf(
        ...
        $rack_tables = [
            'zentra_plugin_racks_itemspecifications',
            'zentra_plugin_racks_others',
            'zentra_plugin_racks_othermodels',
            'zentra_plugin_racks_racks',
            'zentra_plugin_racks_racks_items',
            'zentra_plugin_racks_rackmodels',
            'zentra_plugin_racks_racktypes',
            'zentra_plugin_racks_rackstates',
            'zentra_plugin_racks_roomlocations',
        ];
    ...
    private function importOtherElements()
    {
        ...
        $othermodels_iterator = $this->db->request(
            [
                'FROM' => 'zentra_plugin_racks_othermodels',
            ]
        );
        ...
        if ($count_othermodels = $othermodels_iterator->count()) {
            $this->output->writeln(
                [
                    '<comment>' . __('Other items do not exist in ZENTRA core.') . '</comment>',
                    sprintf(
            ...
            foreach ($othermodels_iterator as $othermodel) {
                ...
                $otheritems_iterator = $this->db->request(
                    [
                        'FROM'  => 'zentra_plugin_racks_others',
                        'WHERE' => [
    ...
    private function importItemsSpecifications()
    {
        ...
        $specs_iterator = $this->db->request(
            [
                'FROM'  => 'zentra_plugin_racks_itemspecifications',
                'ORDER' => 'id ASC',
            ]
        );
    ...
    private function importRackModels()
    {
        ...
        $models_iterator = $this->db->request(
            [
                'FROM' => 'zentra_plugin_racks_rackmodels',
            ]
        );
    ...
    private function importRackTypes()
    {
        ...
        $types_iterator = $this->db->request(
            [
                'FROM' => 'zentra_plugin_racks_racktypes',
            ]
        );
    ...
    private function importRackStates()
    {
        ...
        $states_iterator = $this->db->request(
            [
                'FROM' => 'zentra_plugin_racks_rackstates',
            ]
        );
    ...
    private function importRooms()
    {
        ...
        $rooms_iterator = $this->db->request(
            [
                'FROM' => 'zentra_plugin_racks_roomlocations',
            ]
        );
    ...
    private function importRacks()
    {
        ...
        $racks_iterator = $this->db->request(
            [
                'FROM' => 'zentra_plugin_racks_racks',
            ]
        );
    ...
    private function importRackItems()
    {
        ...
        $items_iterator = $this->db->request(
            [
                'FROM'  => 'zentra_plugin_racks_racks_items',
                'ORDER' => 'id',
            ]
        );
