...
trait TelemetryActivationTrait
{
    ...
    protected function registerTelemetryActivationOptions(InputDefinition $definition): void
    {
        $definition->addOption(
            new InputOption(
                ...
                sprintf(__('Allow usage statistics sending to Telemetry service (%s)'), ZENTRA_TELEMETRY_URI)
            )
        );
        ...
        $definition->addOption(
            new InputOption(
                ...
                null,
                InputOption::VALUE_NONE,
                sprintf(__('Disallow usage statistics sending to Telemetry service (%s)'), ZENTRA_TELEMETRY_URI)
            )
        );
    }
    ...
    protected function handTelemetryActivation(InputInterface $input, OutputInterface $output): void
    {
        ...
        if (
            ...
            $output->writeln(
                [
                    '<comment>' . __('We need your help to improve ZENTRA and the plugins ecosystem!') . '</comment>',
                    '<comment>' . __('Since ZENTRA 9.2, we’ve introduced a new statistics feature called “Telemetry”, that anonymously with your permission, sends data to our telemetry website.') . '</comment>',
                    '<comment>' . __('Once sent, usage statistics are aggregated and made available to a broad range of ZENTRA developers.') . '</comment>',
                    '<comment>' . __('Let us know your usage to improve future versions of ZENTRA and its plugins!') . '</comment>',
                ],
