...
class ErrorController extends AbstractController
{
    public function __invoke(Request $request, ?Throwable $exception = null): Response
    {
        ...
        if ($exception instanceof HttpExceptionInterface) {
            ...
            if (
                $exception instanceof \Zentra\Exception\Http\HttpExceptionInterface
                && ($custom_message = $exception->getMessageToDisplay()) !== null
            ) {
                $message = $custom_message;
            }
            if ($exception instanceof \Zentra\Exception\Http\HttpExceptionInterface) {
        ...
        if (
            (
                ...
                || isset($_SESSION['zentra_use_mode']) && $_SESSION['zentra_use_mode'] == Session::DEBUG_MODE
            )
        ) {
            $trace = sprintf(
                "%s\nIn %s(%s)",
