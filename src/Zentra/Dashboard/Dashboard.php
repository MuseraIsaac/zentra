...
class Dashboard extends CommonDBTM
{
    ...
    public static function checkRights(array $rights = []): bool
    {
        ...
        if (
            count(array_intersect($rights['entities_id'], $_SESSION['zentraactiveentities']))
            || in_array($_SESSION["zentraactiveprofile"]['id'], $rights['profiles_id'])
            || in_array($_SESSION['zentraID'], $rights['users_id'])
            || count(array_intersect($rights['groups_id'], $_SESSION['zentragroups']))
        ) {
            return true;
        }
