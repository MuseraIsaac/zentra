...
class Grid
{
     ...
     * A define.php constant (ZENTRA_AJAX_DASHBOARD) exists to control how the cards should be loaded
...
HTML;
        ...
        if (!$mini) {
            $params = [
                'is_placeholder' => $CFG_ZENTRA['is_demo_dashboards'] ?? 0,
                'messages' => [
                                ...
                                <div class="mt-2">
                                    ...
                                    <script>
                                        $('button.disable-dashboard-demo').on('click', function() {
                                            $.post(CFG_ZENTRA.root_doc + '/ajax/dashboard.php', {
...
TWIG, $params);
        ...
        $ajax_cards = ZENTRA_AJAX_DASHBOARD;
        $cache_key  = sha1($_SESSION['zentraactiveentities_string'] ?? "");
        ...
        $twig_params = [
            ...
            'js_params' => [
                ...
                'is_recursive'  => $_SESSION['zentraactive_entity_recursive'] ? 1 : 0,
            ],
        ];
        // language=Twig
        echo TemplateRenderer::getInstance()->renderFromStringTemplate(<<<TWIG
            ...
            <script type="module">
                import('/js/modules/Dashboard/Dashboard.js').then((m) => {
                    new m.ZENTRADashboard({{ js_params|json_encode|raw }});
                });
            </script>
TWIG, $twig_params);
    }
    ...
    public function initEmbedSession(array $params = [])
    {
        ...
        Session::start();
        $_SESSION["zentraactive_entity"]           = $params['entities_id'];
        $_SESSION["zentraactive_entity_recursive"] = $params['is_recursive'];
        $_SESSION["zentraname"]                    = 'embed_dashboard';
        $_SESSION['zentragroups']                  = [];
        if ($params['is_recursive']) {
            $entities = getSonsOf("zentra_entities", $params['entities_id']);
        } else {
...
HTML;
    ...
    public function getCardHtml(string $card_id = "", array $card_options = []): string
    {
        ...
        try {
            ...
            if (isset($card['provider'])) {
                ...
                if (($CFG_ZENTRA['is_demo_dashboards'] ?? 0) && $provider_func[0] === Provider::class) {
    ...
    public function getAllDasboardCards($force = false): array
    {
        ...
        $cards = $ZENTRA_CACHE->get(self::getAllDashboardCardsCacheKey());
        ...
        if ($cards === null || $force) {
            ...
            foreach ($menu_itemtypes as $firstlvl => $itemtypes) {
                foreach ($itemtypes as $itemtype) {
                    ...
                    $cards["bn_count_$clean_itemtype"] = [
                        ...
                        'provider'   => "Zentra\\Dashboard\\Provider::bigNumber$itemtype",
            ...
            foreach (Item_Devices::getDeviceTypes() as $itemtype) {
                ...
                $cards["count_" . $itemtype . "_" . $fk_itemtype] = [
                    ...
                    'label'      => $label,
                    'provider'   => "Zentra\\Dashboard\\Provider::multipleNumber" . $itemtype . "By" . $fk_itemtype,
                    'filters'    => Filter::getAppliableFilters($itemtype::getTable()),
                ];
            ...
            foreach ($CFG_ZENTRA['device_types'] as $itemtype) {
                $clean_itemtype = str_replace('\\', '_', $itemtype);
                $cards["bn_count_$clean_itemtype"] = [
                    'widgettype' => ["bigNumber"],
                    'group'      => _n('Device', 'Devices', 1),
            ...
            $assets = array_merge($CFG_ZENTRA['asset_types'], ['Software']);
            ...
            foreach ($tickets_cases as $case => $label) {
                $cards["bn_count_tickets_$case"] = [
                    ...
                    'provider'   => "Zentra\\Dashboard\\Provider::nbTicketsGeneric",
                    'args'       => [
                        'case'   => $case,
                        'params' => [
            ...
            $cards["nb_opened_ticket"] = [
                ...
                'provider'   => "Zentra\\Dashboard\\Provider::ticketsOpened",
                'filters'    => Filter::getAppliableFilters(Ticket::getTable()),
            ];
            ...
            $cards["ticket_evolution"] = [
                ...
                'provider'   => "Zentra\\Dashboard\\Provider::getTicketsEvolution",
                'filters'    => Filter::getAppliableFilters(Ticket::getTable()),
            ];
            ...
            $cards["ticket_status"] = [
                ...
                'provider'   => "Zentra\\Dashboard\\Provider::getTicketsStatus",
                'filters'    => Filter::getAppliableFilters(Ticket::getTable()),
            ];
            ...
            $cards["ticket_times"] = [
                ...
                'provider'   => "Zentra\\Dashboard\\Provider::averageTicketTimes",
                'filters'    => Filter::getAppliableFilters(Ticket::getTable()),
            ];
            ...
            $cards["tickets_summary"] = [
                ...
                'provider'   => "Zentra\\Dashboard\\Provider::getTicketSummary",
                'filters'    => Filter::getAppliableFilters(Ticket::getTable()),
            ];
            ...
            $cards["bn_count_tickets_expired_by_tech"] = [
                ...
                'provider'   => "Zentra\\Dashboard\\Provider::nbTicketsByAgreementStatusAndTechnician",
                'filters'    => Filter::getAppliableFilters(Ticket::getTable()),
            ];
            ...
            $cards["bn_count_tickets_expired_by_tech_group"] = [
                ...
                'provider'   => "Zentra\\Dashboard\\Provider::nbTicketsByAgreementStatusAndTechnicianGroup",
                'filters'    => Filter::getAppliableFilters(Ticket::getTable()),
            ];
            ...
            foreach (
                ...
                $cards["top_ticket_$itemtype"] = [
                    ...
                    'group'      => __('Assistance'),
                    'label'      => $label,
                    'provider'   => "Zentra\\Dashboard\\Provider::multipleNumberTicketBy$itemtype",
                    'filters'    => Filter::getAppliableFilters($itemtype::getTable()),
                ];
            }
            ...
            foreach (
                ...
                $cards["top_ticket_$type"] = [
                    ...
                    'group'      => __('Assistance'),
                    'label'      => $label,
                    'provider'   => "Zentra\\Dashboard\\Provider::nbTicketsActor",
                    'args'       => [
                        'case' => $type,
                    ],
                    'filters'    => Filter::getAppliableFilters(Ticket::getTable()),
            ...
            $cards["RemindersList"] = [
                ...
                'group'      => __('Tools'),
                'provider'   => "Zentra\\Dashboard\\Provider::getArticleListReminder",
            ...
            if (!Environment::get()->shouldExpectResourcesToChange()) {
                ...
                $ZENTRA_CACHE->set(self::getAllDashboardCardsCacheKey(), $cards);
            }
        }
    ...
    public function restoreLastDashboard(): string
    {
        ...
        if (isset($_SESSION['last_dashboards']) && $target !== '') {
            $target = preg_replace('/^' . preg_quote($CFG_ZENTRA['root_doc'], '/') . '/', '', $target);
            if (!isset($_SESSION['last_dashboards'][$target])) {
                return "";
            }
    ...
    public static function getDefaultDashboardForMenu(string $menu = "", bool $strict = false): string
    {
        ...
        $default    = $_SESSION["zentra$config_key"] ?? "";
        if (strlen($default)) {
