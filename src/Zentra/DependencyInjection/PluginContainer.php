...
namespace Zentra\DependencyInjection;

use Closure;
use Zentra\Kernel\Kernel;
use Zentra\Routing\PluginRoutesLoader;
use Plugin;
...
class PluginContainer implements ContainerInterface
{
    ...
    private function configureContainerServices(ContainerBuilder $container): void
    {
        ...
        // Plugins routing
        $router = $services->set('zentra_plugin_router', Router::class)->public();
        $router->bind('$loader', new Reference(PluginRoutesLoader::class));
        $router->bind('$resource', 'plugins_routes');
        $services->alias(UrlGeneratorInterface::class, 'zentra_plugin_router');
