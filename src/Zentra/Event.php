...
class Event extends CommonDBTM
{
     ...
     * Log an event.
     *
     * Log the event $event on the zentra_event table with all the others args, if
    ...
    public static function log($items_id, $type, $level, $service, $event)
    {
        ...
        if ($CFG_ZENTRA["event_loglevel"] < $level) {
            return;
        }
    ...
    public static function displayItemLogID($type, $items_id)
    {
        ...
        if ($items_id < 0 || ($type !== Entity::class && $items_id == 0)) {
            ...
            switch ($type) {
                case "rules":
                    echo "<a href=\"" . \htmlescape($CFG_ZENTRA["root_doc"]) . "/front/rule.generic.form.php?id="
                     . $items_id . "\">" . $items_id . "</a>";
                    break;
                ...
                case "reservationitem":
                    echo "<a href=\"" . \htmlescape($CFG_ZENTRA["root_doc"]) . "/front/reservation.php?reservationitems_id="
                     . $items_id . "\">" . $items_id . "</a>";
                    break;
     ...
     *
     * Print a great tab to present lasts events occurred on zentra
     *
        ...
        $twig_params = [
            ...
            'header_rows'  => [
                [
                    [
                        'colspan'   => 5,
                        'content'   => "<a href=\"" . \htmlescape($CFG_ZENTRA["root_doc"]) . "/front/event.php\">"
                            . \htmlescape(sprintf(__('Last %d events'), $_SESSION['zentralist_limit']))
                            . "</a>",
                    ],
                ],
                [
     ...
     * the `type` column in zentra_events.
    ...
    private static function getTypeValuesForDropdown(): array
    {
        ...
        foreach (self::getUsedItemtypes() as $value) {
            ...
            if (is_a($itemtype, CommonZENTRA::class, true)) {
    ...
    /**
     * Get all itemtypes referenced in the `type` columns of zentra_events
     * Note that these values are not real itemtypes but strings like "users".
     * You need to call self::getItemtypeFromType() to get a valid ZENTRA itemtype
     *
     * @return array
     */
    ...
    {
        if (is_a($type, CommonZENTRA::class, true)) {
            return $type;
        }
        ...
        if (is_a($fallback_type, CommonZENTRA::class, true)) {
