...
trait AssignableItem
{
    ...
    public function canViewItem(): bool
    {
        ...
        $is_assigned = $this->fields['users_id_tech'] === $_SESSION['zentraID']
            || count(array_intersect($this->fields['groups_id_tech'] ?? [], $_SESSION['zentragroups'] ?? [])) > 0;
        $is_owned = $this->fields['users_id'] === $_SESSION['zentraID']
            || count(array_intersect($this->fields['groups_id'] ?? [], $_SESSION['zentragroups'] ?? [])) > 0;
    ...
    private static function getAssignableVisiblityCriteriaForCentral(
        ...
        if (Session::haveRight(static::$rightname, READ_ASSIGNED)) {
            $or[] = [
                $item_table . '.users_id_tech' => $_SESSION['zentraID'],
            ];
                ...
                $or[] = [
                    $item_table . '.id' => new QuerySubQuery([
                        ...
                        'WHERE' => [
                            ...
                            'groups_id' => $_SESSION['zentragroups'],
    ...
    private static function getOwnAssetsCriteria(
        ...
        $or = [$item_table . '.users_id' => $_SESSION['zentraID']];
        if (count($_SESSION['zentragroups']) > 0) {
            $or[] = [
                $item_table . '.id' => new QuerySubQuery([
                    'SELECT'     => $relation_table . '.items_id',
