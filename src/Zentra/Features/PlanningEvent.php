...
trait PlanningEvent
{
    ...
    public static function populatePlanning($options = []): array
    {
        ...
        if ($options['genical']) {
            $_SESSION["zentraactiveprofile"][static::$rightname] = READ;
        }
        $visibility_criteria = [];
        ...
        if ($whogroup > 0) {
            if ($itemtype == 'Reminder') {
                $ngrouppriv = ["zentra_groups_reminders.groups_id" => $whogroup];
            } else {
            ...
            foreach ($iterator as $data) {
                if ($event_obj->getFromDB($data["id"]) && $event_obj->canViewItem()) {
                    ...
                    $events[$key] = [
                     ...
                     : '',
                        'ajaxurl'          => $CFG_ZENTRA["root_doc"] . "/ajax/planning.php"
    ...
    public static function displayPlanningItem(array $val, $who, $type = "", $complete = false)
    {
        ...
        $html .= "<img src='" . htmlescape($CFG_ZENTRA["root_doc"] . "/pics/" . $img) . "' alt='' title=\""
