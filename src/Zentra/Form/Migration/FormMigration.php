...
use Zentra\Form\Destination\CommonITILField\ITILActorField;
...
use Zentra\Form\QuestionType\QuestionTypeDropdownExtraDataConfig;
...
use Zentra\ItemTranslation\CldrLanguage;
...
class FormMigration extends AbstractPluginMigration
{
    ...
    {
        return 'zentra_11_form_migration';
    }

    #[Override]
    protected function getMainPluginTables(): array
    {
        return ['zentra_plugin_formcreator_forms'];
    }
    ...
    private function getFormWhereCriteria(): array
    {
        ...
        if ($this->specificFormsIds !== []) {
            $criteria[] = [
                'zentra_plugin_formcreator_forms.id' => $this->specificFormsIds,
            ];
        } else {
            $criteria[] = [
                // Exclude orphan forms that have no associated entity
                'zentra_plugin_formcreator_forms.entities_id' => new QuerySubQuery([
                    'SELECT' => 'id',
    ...
    {
        $formcreator_schema = [
            'zentra_plugin_formcreator_categories' => [
            ...
            'zentra_plugin_formcreator_targetproblems' => [
            ...
            'zentra_plugin_formcreator_targetchanges' => [
    ...
    protected function processMigration(): bool
    {
        ...
        $counts = [
            'categories' => $this->countRecords('zentra_plugin_formcreator_categories'),
            'forms' => $this->countRecords('zentra_plugin_formcreator_forms'),
            'sections' => $this->countRecords('zentra_plugin_formcreator_sections'),
            'questions' => $this->countRecords('zentra_plugin_formcreator_questions', ['NOT' => ['fieldtype' => 'description']]),
            'comments' => $this->countRecords('zentra_plugin_formcreator_questions', ['fieldtype' => 'description']),
            // Each target type is processed twice: once for destinations, once for fields
            'targets_ticket' => $this->countRecords('zentra_plugin_formcreator_targettickets') * 2,
            'targets_problem' => $this->countRecords('zentra_plugin_formcreator_targetproblems') * 2,
            'targets_change' => $this->countRecords('zentra_plugin_formcreator_targetchanges') * 2,
            'translations' => $this->countRecords('zentra_plugin_formcreator_forms_languages'),
            'conditions' => $this->countRecords('zentra_plugin_formcreator_conditions'),
            'configs' => $this->countRecords('zentra_plugin_formcreator_entityconfigs', [
                'replace_helpdesk' => [-2, 2],
            ]),
            'items_tickets_relations' => $this->countRecords('zentra_items_tickets', [
                'itemtype' => 'PluginFormcreatorFormAnswer',
            ]),
            'changes_items_relations' => $this->countRecords('zentra_changes_items', [
                'itemtype' => 'PluginFormcreatorFormAnswer',
            ]),
            'items_problems_relations' => $this->countRecords('zentra_items_problems', [
                'itemtype' => 'PluginFormcreatorFormAnswer',
            ]),
        ];
    ...
    private function processMigrationOfFormCategories(): void
    {
        ...
        if ($this->specificFormsIds !== []) {
            $additional_criteria[] = [
                'zentra_plugin_formcreator_categories.id' => new QuerySubQuery([
                    'SELECT' => 'zentra_plugin_formcreator_forms.plugin_formcreator_categories_id',
                    'FROM'   => 'zentra_plugin_formcreator_forms',
                    'WHERE'  => ['zentra_plugin_formcreator_forms.id' => $this->specificFormsIds],
                ]),
            ];
        }

        // Retrieve data from zentra_plugin_formcreator_categories table
        $raw_form_categories = $this->db->request([
            'SELECT' => ['id', 'name', 'plugin_formcreator_categories_id'],
            'FROM'   => 'zentra_plugin_formcreator_categories',
    ...
    private function processMigrationOfBasicProperties(): void
    {
        ...
        // Retrieve data from zentra_plugin_formcreator_forms table
    ...
    private function processMigrationOfSections(): void
    {
        ...
        // Retrieve data from zentra_plugin_formcreator_sections table
        $raw_sections = $this->db->request([
            'SELECT' => ['id', 'name', 'plugin_formcreator_forms_id', 'order', 'uuid'],
            'FROM'   => 'zentra_plugin_formcreator_sections',
        ]);
        ...
        foreach ($raw_questions as $raw_question) {
            ...
            if (
                ...
                if ($plugin_hint !== null && !isset($hinted_types[$fieldtype])) {
                    ...
                    $url = "https://plugins.zentra-project.org/#/plugin/$plugin_hint";
                    $this->result->addMessage(
                        MessageType::Notice,
    ...
    private function processMigrationOfComments(): void
    {
        ...
        // Retrieve data from zentra_plugin_formcreator_questions table
    ...
    private function processMigrationOfAccessControls(): void
    {
        ...
        $raw_form_access_rights = $this->db->request([
            'SELECT' => [
                'access_rights',
                new QueryExpression('zentra_plugin_formcreator_forms.id', 'forms_id'),
            ...
            'LEFT JOIN'   => [
                'zentra_plugin_formcreator_forms_users' => [
                    'ON' => [
                        'zentra_plugin_formcreator_forms_users' => 'plugin_formcreator_forms_id',
                        'zentra_plugin_formcreator_forms'       => 'id',
                    ],
                ],
                'zentra_plugin_formcreator_forms_groups' => [
                    'ON' => [
                        'zentra_plugin_formcreator_forms_groups' => 'plugin_formcreator_forms_id',
                        'zentra_plugin_formcreator_forms'        => 'id',
                    ],
                ],
                'zentra_plugin_formcreator_forms_profiles' => [
                    'ON' => [
                        'zentra_plugin_formcreator_forms_profiles' => 'plugin_formcreator_forms_id',
                        'zentra_plugin_formcreator_forms'          => 'id',
                    ],
                ],
            ],
    ...
    private function processMigrationOfFormTargets(): void
    {
        ...
        $this->processMigrationOfDestinationType(
            ...
            $this->getBasicTargetsQuery('zentra_plugin_formcreator_targetproblems')
        );
        ...
        $this->processMigrationOfDestinationType(
            ...
            $this->getBasicTargetsQuery('zentra_plugin_formcreator_targetchanges')
        );
    ...
    private function processMigrationOfITILActorsFields(
        ...
        $raw_targets_actors = $this->db->request([
            ...
            'FROM' => 'zentra_plugin_formcreator_targets_actors',
            'WHERE' => [
    ...
    private function processMigrationOfTranslations(): void
    {
        ...
        // Retrieve data from zentra_plugin_formcreator_forms_languages table
        $raw_languages = $this->db->request([
            'SELECT' => [
                'plugin_formcreator_forms_id',
                'zentra_plugin_formcreator_forms_languages.name',
            ],
            'FROM'   => 'zentra_plugin_formcreator_forms_languages',
            'LEFT JOIN'   => [
                'zentra_plugin_formcreator_forms' => [
                    'ON' => [
                        'zentra_plugin_formcreator_forms_languages' => 'plugin_formcreator_forms_id',
                        'zentra_plugin_formcreator_forms'           => 'id',
                    ],
                ],
            ],
    ...
    private function processMigrationOfVisibilityConditions(): void
    {
        ...
        // Retrieve data from zentra_plugin_formcreator_conditions table
        $raw_conditions = $this->db->request([
            'SELECT' => [
                'zentra_plugin_formcreator_conditions.itemtype',
                'zentra_plugin_formcreator_conditions.items_id',
                'zentra_plugin_formcreator_conditions.plugin_formcreator_questions_id',
                'zentra_plugin_formcreator_conditions.show_condition',
                'zentra_plugin_formcreator_conditions.show_value',
                'zentra_plugin_formcreator_conditions.show_logic',
                'zentra_plugin_formcreator_conditions.order',
                new QueryExpression(
                    'COALESCE(zentra_plugin_formcreator_forms.show_rule, zentra_plugin_formcreator_questions.show_rule, zentra_plugin_formcreator_sections.show_rule, zentra_plugin_formcreator_targettickets.show_rule, zentra_plugin_formcreator_targetchanges.show_rule, zentra_plugin_formcreator_targetproblems.show_rule)',
                    'show_rule'
                ),
            ],
            'FROM'   => 'zentra_plugin_formcreator_conditions',
            'JOIN' => [
                'zentra_plugin_formcreator_forms' => [
                    'ON' => [
                        'zentra_plugin_formcreator_conditions' => 'items_id',
                        'zentra_plugin_formcreator_forms'      => 'id',
                        ['AND' => ['zentra_plugin_formcreator_conditions.itemtype' => 'PluginFormcreatorForm']],
                    ],
                ],
                'zentra_plugin_formcreator_questions' => [
                    'ON' => [
                        'zentra_plugin_formcreator_conditions' => 'items_id',
                        'zentra_plugin_formcreator_questions'  => 'id',
                        ['AND' => ['zentra_plugin_formcreator_conditions.itemtype' => 'PluginFormcreatorQuestion']],
                    ],
                ],
                'zentra_plugin_formcreator_sections' => [
                    'ON' => [
                        'zentra_plugin_formcreator_conditions' => 'items_id',
                        'zentra_plugin_formcreator_sections'   => 'id',
                        ['AND' => ['zentra_plugin_formcreator_conditions.itemtype' => 'PluginFormcreatorSection']],
                    ],
                ],
                'zentra_plugin_formcreator_targettickets' => [
                    'ON' => [
                        'zentra_plugin_formcreator_conditions'    => 'items_id',
                        'zentra_plugin_formcreator_targettickets' => 'id',
                        ['AND' => ['zentra_plugin_formcreator_conditions.itemtype' => 'PluginFormcreatorTargetTicket']],
                    ],
                ],
                'zentra_plugin_formcreator_targetchanges' => [
                    'ON' => [
                        'zentra_plugin_formcreator_conditions'    => 'items_id',
                        'zentra_plugin_formcreator_targetchanges' => 'id',
                        ['AND' => ['zentra_plugin_formcreator_conditions.itemtype' => 'PluginFormcreatorTargetChange']],
                    ],
                ],
                'zentra_plugin_formcreator_targetproblems' => [
                    'ON' => [
                        'zentra_plugin_formcreator_conditions'     => 'items_id',
                        'zentra_plugin_formcreator_targetproblems' => 'id',
                        ['AND' => ['zentra_plugin_formcreator_conditions.itemtype' => 'PluginFormcreatorTargetProblem']],
                    ],
                ],
            ],
            'HAVING'  => [
    ...
    private function displayWarningForNonMigratedItems(): void
    {
        ...
        if ($this->specificFormsIds !== []) {
            $criteria['zentra_plugin_formcreator_forms.id'] = $this->specificFormsIds;
        }
    ...
    private function processMigrationOfValidationConditions(): void
    {
        ...
        // Retrieve data from zentra_plugin_formcreator_questionranges and zentra_plugin_formcreator_questionregexes tables
        $raw_conditions = $this->db->request([
            'SELECT' => [
                'zentra_plugin_formcreator_questions.id',
                'zentra_plugin_formcreator_questionranges.range_min',
                'zentra_plugin_formcreator_questionranges.range_max',
                'zentra_plugin_formcreator_questionregexes.regex',
            ],
            'FROM'   => 'zentra_plugin_formcreator_questions',
            'LEFT JOIN' => [
                'zentra_plugin_formcreator_questionranges' => [
                    'ON' => [
                        'zentra_plugin_formcreator_questions' => 'id',
                        'zentra_plugin_formcreator_questionranges' => 'plugin_formcreator_questions_id',
                    ],
                ],
                'zentra_plugin_formcreator_questionregexes' => [
                    'ON' => [
                        'zentra_plugin_formcreator_questions' => 'id',
                        'zentra_plugin_formcreator_questionregexes' => 'plugin_formcreator_questions_id',
                    ],
                ],
            ],
            'WHERE' => [
                'OR' => [
                    'NOT' => [
                        'zentra_plugin_formcreator_questionranges.range_min' => 'NULL',
                        'zentra_plugin_formcreator_questionranges.range_max' => 'NULL',
                        'zentra_plugin_formcreator_questionregexes.regex'    => 'NULL',
                    ],
                ],
            ],
        ]);
    ...
    private function getTicketTargetsQuery(): array
    {
        return [
                ...
                new QueryExpression(
                    'COALESCE(
                        (
                            ...
                            FROM zentra_plugin_formcreator_items_targettickets t2
                            WHERE t2.plugin_formcreator_targettickets_id = zentra_plugin_formcreator_targettickets.id
                                AND t2.link = 0
                ...
                new QueryExpression(
                    'COALESCE(
                        (
                            ...
                            FROM zentra_plugin_formcreator_items_targettickets t3
                            WHERE t3.plugin_formcreator_targettickets_id = zentra_plugin_formcreator_targettickets.id
                                AND t3.link > 0
            ...
            'FROM' => 'zentra_plugin_formcreator_targettickets',
        ];
    }
    ...
    private function processMigrationOfConfigs(): void
    {
        $entity_configs = $this->db->request([
            ...
            'FROM'   => 'zentra_plugin_formcreator_entityconfigs',
        ]);
        foreach ($entity_configs as $entity_config) {
            $this->db->update('zentra_entities', [
                'show_tickets_properties_on_helpdesk' => match ($entity_config['replace_helpdesk']) {
                    ...
                    default => 1, // Helpdesk view was not replaced or Full service catalog -> Use ZENTRA default of showing ticket properties
                },
    ...
    /**
     * Migrate ITIL object relations from FormCreator to ZENTRA forms
     *
     * @param string $table The source table name (e.g., 'zentra_items_tickets')
    ...
    private function migrateITILObjectRelations(
        ...
        $formanswers_links = $this->db->request([
            'SELECT' => [
                "$table.id",
                "$table.$itil_fk",
                'zentra_plugin_formcreator_formanswers.plugin_formcreator_forms_id',
            ],
            'FROM' => $table,
            'JOIN' => [
                'zentra_plugin_formcreator_formanswers' => [
                    'ON' => [
                        $table                                => 'items_id',
                        'zentra_plugin_formcreator_formanswers' => 'id',
                    ],
                ],
                'zentra_plugin_formcreator_forms' => [
                    'ON' => [
    ...
    private function getSourceItemtypeForTargetTable(string $targetTable): string
    {
        return match ($targetTable) {
            'zentra_plugin_formcreator_targettickets'  => 'PluginFormcreatorTargetTicket',
            'zentra_plugin_formcreator_targetproblems' => 'PluginFormcreatorTargetProblem',
            'zentra_plugin_formcreator_targetchanges'  => 'PluginFormcreatorTargetChange',
