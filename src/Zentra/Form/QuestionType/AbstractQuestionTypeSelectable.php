...
use Zentra\Form\Condition\ConditionValueTransformerInterface;
use Zentra\Form\Migration\FormQuestionDataConverterInterface;
        ...
        return <<<JS
            {
                "extractDefaultValue": function (question) {
                    const options = question.find('[data-zentra-form-editor-selectable-question-options]')
                ...
                "convertDefaultValue": function (question, value) {
                    ...
                    setTimeout(() => {
                        question.find('[data-zentra-form-editor-selectable-question-options]')
        ...
        $js = <<<TWIG
            import("/js/modules/Forms/QuestionSelectable.js").then((m) => {
                {% if question is not null %}
                    const container = $('div[data-zentra-form-editor-selectable-question-options="{{ rand }}"]');
                    container.data(
                        'manager',
                        new m.ZentraFormQuestionTypeSelectable('{{ input_type|escape('js') }}', container)
                    );
                {% else %}
                    $(document).on('zentra-form-editor-question-type-changed', function(e, question, type) {
                        if (type === '{{ question_type|escape('js') }}') {
                            const container = question.find('div[data-zentra-form-editor-selectable-question-options]');
                            container.data(
                                'manager',
                                new m.ZentraFormQuestionTypeSelectable('{{ input_type|escape('js') }}', container, true)
                            );
                        }
                    });

                    $(document).on('zentra-form-editor-question-duplicated', function(e, question, new_question) {
                        const question_type = question.find('input[data-zentra-form-editor-original-name="type"]').val();
                        if (question_type === '{{ question_type|escape('js') }}') {
                            const container = new_question.find('div[data-zentra-form-editor-selectable-question-options]');
                            container.data(
                                'manager',
                                new m.ZentraFormQuestionTypeSelectable('{{ input_type|escape('js') }}', container, true)
                            );
    ...
    public function renderAdministrationTemplate(?Question $question): string
    {
        ...
        {% macro addOption(input_type, checked, value, translations, uuid = null, order, extra_details = false, disabled = false, hide_default_value_input = false, extra_input_attributes = []) %}
            ...
            <div
                ...
                data-zentra-form-selectable-question-option
                {{ extra_details ? 'data-zentra-form-editor-question-extra-details' : '' }}
            >
                <i
                    role="button"
                    aria-label="{{ translations.move_option }}"
                    data-zentra-form-editor-question-extra-details
                    data-zentra-form-editor-question-option-handle
                ...
                <input
                    ...
                    value="{{ order }}"
                    data-zentra-form-editor-specific-question-extra-data
                    data-zentra-form-editor-question-option-order
                >
                <button
                    type="button"
                    ...
                    data-zentra-form-editor-question-extra-details
                    data-zentra-form-editor-question-option-remove
                >
                    <i class="ti ti-x"></i>
                </button>
            </div>
        ...
        <div class="{{ selectable_question_options_class|default('') }}">
            ...
            <div
                data-zentra-form-editor-selectable-question-options="{{ rand }}"
                {{ hide_container_when_unfocused ? 'data-zentra-form-editor-question-extra-details' : '' }}
            >
                {% for value in values %}
        ...
        <script>
            ...
            // the right time.
            $("[data-zentra-form-editor-container]").on('initialized', () => {
                ...
                <label class="form-check {{ loop.last ? 'mb-0' : '' }}">
                    ...
                    <span class="form-check-label">
                        {{ translate_form_item_key(
                            question,
                            '%s-%s'|format(
                                constant('Zentra\\\\Form\\\\QuestionType\\\\AbstractQuestionTypeSelectable::TRANSLATION_KEY_OPTION'),
                                value.uuid
                            )
                        ) }}
                    </span>
