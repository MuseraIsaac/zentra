    ...
    private function loadCoreQuestionsTypes(): void
    {
        ...
        while ($directory_iterator->valid()) {
            ...
            $classname = $directory_iterator->getExtension() === 'php'
                ? 'Zentra\\Form\\QuestionType\\' . $directory_iterator->getBasename('.php')
                : null;

            // Validate that the class is a valid question type
            if ($this->isClassAValidQuestionType($classname)) {
                // @phpstan-ignore zentra.forbidDynamicInstantiation (Type is checked by `self::isClassAValidQuestionType()`)
