...
final class Firewall
{
    ...
    /**
     * @param ?string $zentra_root             ZENTRA root directory on filesystem
     * @param ?array  $plugin_directories         ZENTRA plugins root directories on filesystem
     */
    public function __construct(?string $zentra_root = null, ?array $plugin_directories = null)
    {
        $this->zentra_root = $zentra_root ?? ZENTRA_ROOT;
        $this->plugin_directories = $plugin_directories ?? ZENTRA_PLUGINS_DIRECTORIES;
    }
    ...
    /**
     * Compute the fallback strategy for ZENTRA resources.
     */
    private function computeFallbackStrategyForCore(string $path): string
    {
        if (!file_exists($this->zentra_root . $path)) {
            // Modern controllers
