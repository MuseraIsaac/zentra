...
{
    /**
     * ZENTRA root directory.
     */
    protected string $zentra_root;

    /**
     * ZENTRA plugins directories.
     * @var string[]
     */
    ...
    protected function getTargetFile(string $path): ?string
    {
        ...
        if (preg_match(Plugin::PLUGIN_RESOURCE_PATTERN, $path, $path_matches) === 1) {
            ...
            && \is_file($this->zentra_root . $path)
        ) {
            $filename = $this->zentra_root . $path;
        } else {
            // preprend `/public` to expose the public resources only
            $filename = $this->zentra_root . '/public' . $path;
        }
    ...
    {
        // Get URI path relative to ZENTRA (i.e. without alias directory prefix).
        $path = $request->getPathInfo();

        if ($path === '/') {
            // ZENTRA index, no need to normalize
            return $path;
        }
        ...
        if (\str_starts_with($path, '/marketplace/')) {
            // /!\ `/marketplace/` URLs were massively used prior to ZENTRA 11.0.
            //
            // To not break URLs than can be found in the wild (in e-mail, forums, external apps configuration, ...),
            // please do not remove this behaviour before, at least, 2030 (about 5 years after ZENTRA 11.0.0 release).
            $path = preg_replace(
                '#^/marketplace/#',
                '/plugins/',
                $path
