...
abstract class InventoryAsset
{
    ...
    public function handleLinks()
    {
        ...
        foreach ($data as $key => &$value) {
            ...
            // Extract the asset class name (e.g., "Monitor" from "Zentra\Inventory\Asset\Monitor")
            ...
            if ($temp_item = getItemForItemtype($asset_itemtype)) {
                if ($temp_item->isField('autoupdatesystems_id') && isset($this->item->fields['autoupdatesystems_id'])) {
                    if ($this->item->fields['autoupdatesystems_id'] == 0) {
                        ...
                        $value->autoupdatesystems_id = Dropdown::getDropdownName('zentra_autoupdatesystems', $this->item->fields['autoupdatesystems_id']);
                    }
                }
            }
            ...
            if (in_array($key, $locks)) {
                if (isset($this->raw_links[$known_key])) {
                    if (isset($this->known_links[$known_key])) {
                        ...
                        // For more details, see: https://github.com/zentra-project/zentra/pull/19426
