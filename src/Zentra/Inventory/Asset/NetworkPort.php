...
class NetworkPort extends InventoryAsset
{
        ...
        if ($mainasset->isStackedSwitch()) {
            ...
            foreach ($this->ports as $k => $val) {
                ...
                if (
                    ...
                    //ZENTRA compute stack_id by starting with 1 (see: NetworkEquipment->getStackedSwitches)
    ...
    public function handleHub($found_macs, $netports_id)
    {
        ...
        foreach ($dbports as $dbport) {
            ...
            if ($id) {
                $zentra_ports[$id] = $dbport['id'];
            }
        }

        foreach ($found_macs as $ports_id) {
            if (!isset($zentra_ports[$ports_id])) {
                // Connect port (port found in ZENTRA)
    ...
    public function checkConf(Conf $conf): bool
    {
        global $CFG_ZENTRA;
        $this->conf = $conf;
        return in_array($this->item::class, $CFG_ZENTRA['networkport_types']);
    }
