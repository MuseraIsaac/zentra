...
namespace Zentra\Inventory\Asset;
...
class Software extends InventoryAsset
{
    ...
    public function handle()
    {
        ...
        if (isset($this->extra_data[OperatingSystem::class])) {
            ...
            // tests/functional/Zentra/Inventory/Assets/SoftwareTest.php -> testHandle()
            if (isset($this->main_asset)) {
                // When $this->extra_data['\Zentra\Inventory\Asset\OperatingSystem'] is not set,
        ...
        $iterator = $DB->request([
            'SELECT' => [
                'zentra_items_softwareversions.id as item_soft_version_id',
                'zentra_items_softwareversions.date_install as item_soft_version_date_install',
                'zentra_softwares.id as softid',
                'zentra_softwares.name',
                'zentra_softwareversions.id AS versionid',
                'zentra_softwareversions.name AS version',
                'zentra_softwareversions.arch',
                'zentra_softwares.entities_id',
                'zentra_softwarecategories.name AS softwarecategories_id',
                'zentra_softwares.is_recursive',
                'zentra_softwareversions.operatingsystems_id',
                'zentra_manufacturers.name AS manufacturers_id',
            ],
            ...
            'WHERE'     => [
                'zentra_items_softwareversions.items_id' => $this->item->fields['id'],
                'zentra_items_softwareversions.itemtype'    => $this->item->getType(),
                'zentra_items_softwareversions.is_dynamic'  => 1,
            ],
        ]);
     ...
     * @return string
     *
     * @FIXME Remove this method in ZENTRA 11.0.
     */
    ...
    private function storeSoftware()
    {
        ...
        foreach ($this->data as $val) {
            ...
            if (!isset($this->softwares[$skey])) {
                ...
                //set create date
                $stmt_columns['date_creation'] = $_SESSION["zentra_currenttime"];
                $stmt_columns['is_helpdesk_visible'] = $CFG_ZENTRA["default_software_helpdesk_visible"];
    ...
    {
        global $CFG_ZENTRA;
        return $conf->import_software == 1 && in_array($this->item::class, $CFG_ZENTRA['software_types']);
    }
