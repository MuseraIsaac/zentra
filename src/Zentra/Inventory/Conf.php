...
use Entity;
use finfo;
use Zentra\Agent\Communication\AbstractRequest;
 ...
 */
class Conf extends CommonZENTRA
{
    ...
    public function showConfigForm()
    {
        ...
        if ($canedit) {
            echo "<form name='form' action='" . htmlescape($CFG_ZENTRA['root_doc']) . "/Inventory/Configuration/Store' method='post'>";
        }
        ...
        if ($config['enabled_inventory'] == false) {
            ...
            echo Html::input("basic_auth_password", [
                "value" => (new ZENTRAKey())->decrypt($config['basic_auth_password']),
                "type" => "password",
            ]);
            echo "</td>";
            echo "</tr>";
            ...
            echo "<td width='20%'>";
            $condition = [];
            foreach ($CFG_ZENTRA['inventory_types'] as $inv_type) {
    ...
    public function saveConf(array $values)
    {
        ...
        unset($values['_zentra_csrf_token']);
        ...
        if (isset($values['auth_required']) && $values['auth_required'] === Conf::BASIC_AUTH) {
            if (
                ...
                $values['basic_auth_password'] = (new ZENTRAKey())->encrypt($values['basic_auth_password']);
            } else {
                Session::addMessageAfterRedirect(
