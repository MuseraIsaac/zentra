...
use Zentra\Inventory\Asset\Antivirus;
use Zentra\Inventory\Asset\Battery;
use Zentra\Inventory\Asset\Bios;
use Zentra\Inventory\Asset\Camera;
use Zentra\Inventory\Asset\Cartridge;
use Zentra\Inventory\Asset\Controller;
use Zentra\Inventory\Asset\DatabaseInstance;
use Zentra\Inventory\Asset\Drive;
use Zentra\Inventory\Asset\Environment;
use Zentra\Inventory\Asset\Firmware;
use Zentra\Inventory\Asset\GraphicCard;
use Zentra\Inventory\Asset\InventoryAsset;
use Zentra\Inventory\Asset\Memory;
use Zentra\Inventory\Asset\Monitor;
use Zentra\Inventory\Asset\NetworkCard;
use Zentra\Inventory\Asset\NetworkPort;
use Zentra\Inventory\Asset\OperatingSystem;
use Zentra\Inventory\Asset\Peripheral;
use Zentra\Inventory\Asset\PowerSupply;
use Zentra\Inventory\Asset\Printer;
use Zentra\Inventory\Asset\Process;
use Zentra\Inventory\Asset\Processor;
use Zentra\Inventory\Asset\RemoteManagement;
use Zentra\Inventory\Asset\Sensor;
use Zentra\Inventory\Asset\Simcard;
use Zentra\Inventory\Asset\Software;
use Zentra\Inventory\Asset\SoundCard;
use Zentra\Inventory\Asset\VirtualMachine;
use Zentra\Inventory\Asset\Volume;
use Zentra\Inventory\MainAsset\Itemtype;
...
class Inventory
{
    ...
    public function setData($data, $format = Request::JSON_MODE): bool
    {
        ...
        if (!is_dir(ZENTRA_INVENTORY_DIR)) {
            mkdir(ZENTRA_INVENTORY_DIR);
        }
        ...
        try {
            $this->inventory_tmpfile = tempnam(ZENTRA_INVENTORY_DIR, $tempnam_ext);
        ...
        try {
            ...
            $this->errors[] = preg_replace(
                '|\$ref\[file~2//.*/vendor/zentra-project/inventory_format/inventory.schema.json\]|',
    ...
    public function doInventory($test_rules = false)
    {
        ...
        if (!isset($_SESSION['zentrainventoryuserrunning'])) {
            $_SESSION['zentrainventoryuserrunning'] = 'inventory';
        }

        if (!isset($_SESSION['zentraname'])) {
            $_SESSION['zentraname'] = $_SESSION['zentrainventoryuserrunning'];
        }
        ...
        try {
            ...
            unset($_SESSION['zentrainventoryuserrunning']);
    ...
    private function handleInventoryFile()
    {
        if (isset($this->mainasset)) {
            ...
            foreach ($items as $item) {
                ...
                $filename = ZENTRA_INVENTORY_DIR . '/' . $this->conf->buildInventoryFileName($itemtype, $id, $ext);
    ...
    {
        $class_ns = '\Zentra\Inventory\MainAsset\\';
    ...
    {
        $conf = new Conf();
        $temp_files = glob(ZENTRA_INVENTORY_DIR . '/*.{' . implode(',', $conf->knownInventoryExtensions()) . '}', GLOB_BRACE);
    ...
    public static function cronCleanorphans($task)
    {
        ...
        $conf = new Conf();
        $existing_types = glob(ZENTRA_INVENTORY_DIR . '/*', GLOB_ONLYDIR);

        foreach ($existing_types as $existing_type) {
            /** @var class-string<CommonDBTM> $itemtype */
            $itemtype = str_replace(ZENTRA_INVENTORY_DIR . '/', '', $existing_type);
