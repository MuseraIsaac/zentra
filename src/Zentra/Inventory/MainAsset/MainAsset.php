...
namespace Zentra\Inventory\MainAsset;
...
use Zentra\Inventory\Asset\InventoryNetworkPort;
...
abstract class MainAsset extends InventoryAsset
{
    ...
    public function prepare(): array
    {
        ...
        foreach ($raw_data as $entry) {
            ...
            $val->last_inventory_update = $_SESSION["zentra_currenttime"];
            $val->is_deleted = 0;
    ...
    public function rulepassed($items_id, $itemtype, $rules_id, $ports_id = [])
    {
        ...
        $orig_zentraactive_entity = $_SESSION['zentraactive_entity'] ?? null;
        $orig_zentraactiveentities = $_SESSION['zentraactiveentities'] ?? null;
        $orig_zentraactiveentities_string = $_SESSION['zentraactiveentities_string'] ?? null;

        //set entity in session
        $_SESSION['zentraactiveentities']        = [$entities_id];
        $_SESSION['zentraactiveentities_string'] = $entities_id;
        ...
        if ($items_id == 0) {
            ...
            foreach ($this->item->fields as $onadd_key => $onadd_value) {
                ...
                // set value computed on add by ZENTRA (and rules) to $val
        ...
        if ($entities_id != $this->item->fields['entities_id']) {
            ...
            if ($doTransfer > 0 && $transfer->getFromDB($doTransfer)) {
                ...
                $val->entities_id = $this->item->fields['entities_id']; //for ZENTRA item
        ...
        if (in_array($itemtype, $CFG_ZENTRA['agent_types'])) {
        ...
        if ($this->is_discovery === true && !$this->isNew()) {
            ...
            if (
                ...
                && !(\Zentra\Inventory\MainAsset\NetworkEquipment::needToBeUpdatedFromDiscovery($this->item, $val))
            ) {
        ...
        if ($orig_zentraactive_entity !== null) {
            $_SESSION['zentraactive_entity'] = $orig_zentraactive_entity;
        }

        if ($orig_zentraactiveentities !== null) {
            $_SESSION['zentraactiveentities'] = $orig_zentraactiveentities;
        }

        if ($orig_zentraactiveentities_string !== null) {
            $_SESSION['zentraactiveentities_string'] = $orig_zentraactiveentities_string;
        }
    }
