    ...
    public function onPostBoot(): void
    {
        ...
        if ($this->php_sapi !== 'cli') {
            ...
            if (Session::canWriteSessionFiles()) {
                ...
                \trigger_error(
                    sprintf('Unable to write session files on `%s`.', ZENTRA_SESSION_DIR),
                    E_USER_WARNING
                );
            }
        }
        ...
        if (!$is_stateless) {
            ...
            // Indeed, the ZENTRA code often refers to the `$_SESSION` variable
        ...
        foreach ($CFG_ZENTRA['user_pref_field'] as $field) {
            if (!\array_key_exists("zentra$field", $_SESSION) && \array_key_exists($field, $CFG_ZENTRA)) {
