...
use Zentra\Controller\DropdownFormController;
use Zentra\Controller\GenericFormController;
use Zentra\Controller\GenericListController;
...
final readonly class LegacyItemtypeRouteListener implements EventSubscriberInterface
{
    ...
    /**
     * @phpstan-return class-string<CommonZENTRA>|null
     */
    ...
    private function findGenericClass(string $path_info): ?string
    {
        ...
        $namespaced_itemtype = preg_replace_callback(
            ...
            'Zentra\\' . \str_replace('/', '\\', $itemtype)
        );

        $namespaced_item = \getItemForItemtype($namespaced_itemtype);

        if ($namespaced_item instanceof CommonZENTRA) {
    ...
    private function findPluginClass(string $path_info): ?string
    {
        ...
        if ($legacy_item instanceof CommonZENTRA) {
            return $legacy_item::class;
        }

        // ZentraPlugin\MyPlugin\Item -> /plugins/myplugin/front/item.php
        $namespaced_item = \getItemForItemtype(\sprintf('ZentraPlugin\%s\%s', ucfirst($plugin), ucfirst($itemtype)));
        if ($namespaced_item instanceof CommonZENTRA) {
            return $namespaced_item::class;
        }
