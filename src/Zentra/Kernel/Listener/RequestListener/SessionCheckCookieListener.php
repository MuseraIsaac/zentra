...
class SessionCheckCookieListener implements EventSubscriberInterface
{
    ...
    public function onKernelRequest(RequestEvent $event): void
    {
        ...
        if ($event->getRequest()->isSecure() === false && $cookie_secure === true) {
            ...
            $exception->setMessageToDisplay(__('The web server is configured to allow session cookies only on secured context (https). Therefore, you must access ZENTRA on a secured context to be able to use it.'));
            throw $exception;
        }
    }
}
