...
namespace Zentra\Mail\SMTP;

use Config;
use Zentra\Mail\SMTP\OauthProvider\Azure;
use Zentra\Mail\SMTP\OauthProvider\Google;
use Zentra\Mail\SMTP\OauthProvider\ProviderInterface;
use ZENTRAKey;
...
final class OauthConfig
{
    ...
    public function getSmtpOauthProvider(): (ProviderInterface&AbstractProvider)|null
    {
        ...
        $client_secret    = (new ZENTRAKey())->decrypt($config['smtp_oauth_client_secret']);
        $provider_options = [];
        try {
        ...
        if ($config['proxy_name'] !== '' && !in_array(self::class, $CFG_ZENTRA['proxy_exclusions'])) {
            // Connection using proxy
            $provider_options['proxy'] = $config['proxy_user'] !== ''
                ? sprintf(
                    '%s:%s@%s:%s',
                    rawurlencode($config['proxy_user']),
                    rawurlencode((new ZENTRAKey())->decrypt($config['proxy_passwd'])),
                    $config['proxy_name'],
                    $config['proxy_port']
                )
                : sprintf(
