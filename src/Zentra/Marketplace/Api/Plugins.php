...
namespace Zentra\Marketplace\Api;
...
class Plugins
{
    ...
    public function __construct()
    {
        ...
        $eopts = [
            'base_uri'        => ZENTRA_MARKETPLACE_PLUGINS_API_URI,
        ];
    ...
    private function request(
        ...
        $options['headers'] = array_merge_recursive(
            [
                ...
                'User-Agent'         => ZENTRANetwork::getZentraUserAgent(),
                'X-Registration-Key' => ZENTRANetwork::getRegistrationKey(),
                'X-Zentra-Network-Uid' => ZENTRANetwork::getZentraNetworkUid(),
            ],
            $options['headers'] ?? []
        );
    ...
    public function getAllPlugins(
        ...
        if (self::$plugins === null) {
            $plugins_colct = !$force_refresh
                ? $ZENTRA_CACHE->get($cache_key, null)
                : null;
            ...
            if ($plugins_colct === null) {
                ...
                if ($this->last_error === null) {
                    ...
                    $ZENTRA_CACHE->set($cache_key, $plugins_colct, HOUR_TIMESTAMP);
                }
            }

            // Filter versions.
            // Done after caching process to be able to handle change of "ZENTRA_MARKETPLACE_PRERELEASES"
            // without having to purge the cache manually.
            foreach ($plugins_colct as &$plugin) {
                if (!ZENTRA_MARKETPLACE_PRERELEASES) {
    ...
    public function getTopTags(): array
    {
        ...
        $response  = $this->request('tags/top', [
            'headers' => [
                'X-Lang' => $CFG_ZENTRA['languages'][$_SESSION['zentralanguage']][2],
            ],
        ]);
    ...
    public function getPluginsForTag(string $tag = "", bool $force_refresh = false): array
    {
        ...
        $plugins_colct = !$force_refresh ? $ZENTRA_CACHE->get($cache_key, []) : [];
    ...
    {
        return $item_key . '_' . ZENTRANetwork::getRegistrationKey();
    }
}
