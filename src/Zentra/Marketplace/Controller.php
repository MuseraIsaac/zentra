...
/**
 * Nota: `CommonZENTRA` is required here to be able to provide a displayable name for its crons and notifications.
 */
class Controller extends CommonZENTRA
{
    /** @var string */
    ...
    {
        return (ZENTRA_MARKETPLACE_ENABLE & self::MP_MODE_CLI_ONLY) === self::MP_MODE_CLI_ONLY;
    }
    ...
    {
        return (ZENTRA_MARKETPLACE_ENABLE & self::MP_MODE_WEB_ONLY) === self::MP_MODE_WEB_ONLY;
    }
    ...
    public function downloadPlugin($auto_install = true, ?string $version = null): bool
    {
        ...
        $dest = $url !== null ? ZENTRA_TMP_DIR . '/' . basename(parse_url($url, PHP_URL_PATH)) : null;
        ...
        Toolbox::deleteDir(ZENTRA_MARKETPLACE_DIR . "/{$this->plugin_key}");

        $archive = new $driver($dest, $format);
        try {
            $archive->extractArchive(ZENTRA_MARKETPLACE_DIR);
    ...
    public function proxifyPluginArchive(): Response
    {
        ...
        $dest     = ZENTRA_TMP_DIR . '/' . mt_rand() . '.' . $filename;
    ...
    {
        $found_in_marketplace_dir  = file_exists(ZENTRA_MARKETPLACE_DIR . '/' . $this->plugin_key . '/setup.php');

        // Compute marketplace dir priority
        $marketplace_priority = null;
        foreach (ZENTRA_PLUGINS_DIRECTORIES as $position => $base_dir) {
            try {
                if (realpath($base_dir) === realpath(ZENTRA_MARKETPLACE_DIR)) {
        ...
        if ($found_outside_marketplace) {
            if ($found_dir_priority > $marketplace_priority) {
                ...
                return is_writable(ZENTRA_MARKETPLACE_DIR . '/' . $this->plugin_key) && !self::hasVcsDirectory($this->plugin_key);
            } else {
                // Plugin has been found outside marketplace and does not exist in marketplace
                // -> allow plugin update unless ZENTRA_MARKETPLACE_ALLOW_OVERRIDE is false.
                return ZENTRA_MARKETPLACE_ALLOW_OVERRIDE && self::hasWriteAccess() && !self::hasVcsDirectory($this->plugin_key);
            }
        }
    ...
    /**
     * Do the current plugin requires some Zentra Network offers
     *
    ...
    public function getRequiredOffers(): array
    {
        $api        = self::getAPI();
        $api_plugin = $api->getPlugin($this->plugin_key);
        $offers     = array_column(ZENTRANetwork::getOffers(), 'title', 'offer_reference');
    ...
    /**
     * Check if a given plugin has available versions for current ZENTRA instance.
     *
     * @return bool
     */
    ...
    public function requiresHigherOffer(): bool
    {
        ...
        $registration_informations = ZENTRANetwork::getRegistrationInformations();
        if (
    ...
    {
        return is_dir(ZENTRA_MARKETPLACE_DIR) && is_writable(ZENTRA_MARKETPLACE_DIR);
    }
    ...
    public static function hasVcsDirectory(string $plugin_key): bool
    {
        ...
        foreach ($vcs_dirs as $vcs_dir) {
            if (is_dir(ZENTRA_MARKETPLACE_DIR . '/' . $plugin_key . '/' . $vcs_dir)) {
                return true;
            }
        }
