...
namespace Zentra\Marketplace;

use CommonZENTRA;
use Config;
use Document;
use Zentra\Application\View\TemplateRenderer;
use Zentra\Marketplace\Api\Plugins as PluginsApi;
use ZENTRANetwork;
use Html;
use Plugin;
use Toolbox;
...
class View extends CommonZENTRA
{
    ...
    public static function checkRegistrationStatus(bool $force = false)
    {
        ...
        if (!ZENTRANetwork::isServicesAvailable()) {
            $messages[] = sprintf(__s('%1$s services website seems not available from your network or offline'), 'ZENTRA Network');
            $messages[] = "<a href='" . htmlescape($CFG_ZENTRA['root_doc']) . "/front/config.form.php?forcetab=Config$5'>"
                . __s("Maybe you could setup a proxy")
                . "</a> "
                . __s("or please check later");
        } else {
            $registration_info = ZENTRANetwork::getRegistrationInformations();
            if (!$registration_info['is_valid']) {
                $valid = false;

                $config_url = $CFG_ZENTRA['root_doc'] . "/front/config.form.php?forcetab="
                        . urlencode('ZENTRANetwork$1');
                $messages[] = sprintf(__s('Your %1$s registration is not valid.'), 'ZENTRA Network');
                $messages[] = __s('A registration, at least a free one, is required to use marketplace!');
                $messages[] = "<a href='" . htmlescape(ZENTRA_NETWORK_SERVICES) . "'>" . sprintf(__s('Register on %1$s'), 'ZENTRA Network') . "</a> "
                    . __s('and') . " "
                    . "<a href='" . htmlescape($config_url) . "'>" . __s("fill your registration key in setup.") . "</a>";
            } elseif (!$registration_info['subscription']['is_running']) {
                $valid = false;
                $messages[] = sprintf(__s('Your %1$s subscription has been terminated.'), 'ZENTRA Network');
                $messages[] = "<a href='" . htmlescape(ZENTRA_NETWORK_SERVICES) . "'>" . sprintf(__s('Renew it on %1$s.'), 'ZENTRA Network') . "</a> ";
            } else {
                $valid = true;
            }
        }
    ...
    public static function installed(
        ...
        foreach ($installed as $plugin) {
            ...
            if (Document::isImage(\sprintf('%s/logo.png', Plugin::getPhpDir($key)))) {
                ...
                $logo_url = sprintf('%s/Plugin/%s/Logo', $CFG_ZENTRA['root_doc'], $key);
            }
    ...
    public static function discover(
        ...
        header("X-ZENTRA-Marketplace-Total: $nb_plugins");
     ...
     * - \Plugin::getList
     * - \Zentra\Marketplace\Api\Plugins::getPaginatedPlugins
    ...
    public static function displayList(
        ...
        if ($is_list_truncated) {
            if (count($plugins) === 0) {
                $msg = sprintf(__('Unable to fetch plugin list due to %s services website unavailability. Please try again later.'), 'ZENTRA Network');
            } else {
                // Not completely offline. Do not treat as fully offline.
                $msg = sprintf(__('Plugin list may be truncated due to %s services website unavailability. Please try again later.'), 'ZENTRA Network');
            }
        ...
        if (!$only_lis) {
            ...
            if (!Controller::hasWriteAccess()) {
                echo "<div class='alert alert-warning'><i class='ti ti-alert-triangle fs-5x'></i>"
                      . htmlescape(sprintf(__("We can't write on the markeplace directory (%s)."), ZENTRA_MARKETPLACE_DIR))
                      . "<br>"
...
HTML;
            ...
            $networkmail  = htmlescape(ZENTRA_NETWORK_MAIL);
...
JS;
    ...
    public static function getButtons(string $plugin_key = ""): string
    {
        ...
        if ($must_be_cleaned) {
            ...
            if (!$is_execution_suspended && !$can_run_local_install) {
                ...
                Html::showToolTip(
                    __s('This plugin is not available for your ZENTRA version.'),
                    [
            ...
            || ($has_web_update && !$can_be_overwritten && ZENTRA_MARKETPLACE_MANUAL_DOWNLOADS)
        ) {
            ...
            if (array_key_exists('installation_url', $plugin_data) && $can_be_downloaded) {
                ...
                if (!Controller::hasVcsDirectory($plugin_key)) {
                    ...
                    $warning .= sprintf(
                        ...
                        \htmlescape(ZENTRA_ROOT . '/plugins')
                    );

                    // Use "marketplace.download.php" proxy if archive is downloadable from ZENTRA marketplace plugins API
                    // as this API will refuse to serve the archive if registration key is not set in headers.
                    $download_url = parse_url($plugin_data['installation_url'], PHP_URL_HOST) === parse_url(ZENTRA_MARKETPLACE_PLUGINS_API_URI, PHP_URL_HOST)
                        ? $CFG_ZENTRA['root_doc'] . '/front/marketplace.download.php?key=' . $plugin_key
        ...
        if (!static::$offline_mode && $mk_controller->requiresHigherOffer()) {
            $warning = sprintf(
                __s("You need a superior ZENTRA-Network offer to access to this plugin (%s)"),
                htmlescape(implode(', ', $required_offers))
            );

            $buttons .= "<a href='" . ZENTRA_NETWORK_SERVICES . "' target='_blank'>
        ...
        if (!$is_execution_suspended && $is_installed) {
            ...
            if (!strlen($error) && $is_actived && $config_page) {
                $config_url = "{$CFG_ZENTRA['root_doc']}/plugins/{$plugin_key}/{$config_page}";
    ...
    /**
     * Return HTML part for Zentra Network information for a given plugin
     * @param array $plugin data of the plugin.
    ...
    public static function getNetworkInformations(array $plugin = []): string
    {
        ...
        if (count($require_offers)) {
            ...
            $html = "<div class='offers'>
                    <a href='" . htmlescape(ZENTRA_NETWORK_SERVICES) . "' target='_blank'
                       class='badge zentra-network'
                       title='" . sprintf(__s("You must have a %s subscription to get this plugin"), 'ZENTRA Network') . "'>
                        <i class='ti ti-star-filled'></i>ZENTRA Network
                    </a>
                    <a href='" . htmlescape(ZENTRA_NETWORK_SERVICES) . "' target='_blank'
    ...
    public static function getLocalizedDescription(array $plugin = [], string $version = 'short_description'): string
    {
        ...
        $userlang = $CFG_ZENTRA['languages'][$_SESSION['zentralanguage']][3] ?? "en";
    ...
    public static function showFeatureSwitchDialog()
    {
        ...
        if (isset($_POST['marketplace_replace'])) {
            ...
            if ($mp_value === Controller::MP_REPLACE_YES) {
                Html::redirect($CFG_ZENTRA["root_doc"] . "/front/marketplace.php");
            }
        ...
        if (
            ...
            && ZENTRA_INSTALL_MODE !== 'CLOUD'
        ) {
            echo "<div class='card mb-4'>";
            echo "<div class='card-header card-title'>" . __s("Switch to marketplace") . "</div>";
            echo "<div class='card-body'>";
            echo "<form id='marketplace_dialog' method='POST'>";
            echo Html::image($CFG_ZENTRA['root_doc'] . "/pics/screenshots/marketplace.png", [
                'style' => 'width: 600px',
            ]);
            echo "<br><br>";
            echo __s("ZENTRA provides a new marketplace to download and install plugins.");
            echo "<br><br>";
