...
use Zentra\Asset\AssetModel;
use Zentra\Asset\AssetType;
use Zentra\Asset\Capacity\AbstractCapacity;
use Zentra\Asset\Capacity\AllowedInGlobalSearchCapacity;
use Zentra\Asset\Capacity\HasContractsCapacity;
use Zentra\Asset\Capacity\HasDevicesCapacity;
use Zentra\Asset\Capacity\HasDocumentsCapacity;
use Zentra\Asset\Capacity\HasHistoryCapacity;
use Zentra\Asset\Capacity\HasInfocomCapacity;
use Zentra\Asset\Capacity\HasLinksCapacity;
use Zentra\Asset\Capacity\HasNetworkPortCapacity;
use Zentra\Asset\Capacity\HasNotepadCapacity;
use Zentra\Asset\Capacity\HasPeripheralAssetsCapacity;
use Zentra\Asset\Capacity\IsProjectAssetCapacity;
use Zentra\Asset\Capacity\IsReservableCapacity;
use Zentra\Asset\CustomFieldDefinition;
use Zentra\Asset\CustomFieldType\AbstractType;
use Zentra\Asset\CustomFieldType\BooleanType;
use Zentra\Asset\CustomFieldType\DateTimeType;
use Zentra\Asset\CustomFieldType\DateType;
use Zentra\Asset\CustomFieldType\DropdownType;
use Zentra\Asset\CustomFieldType\NumberType;
use Zentra\Asset\CustomFieldType\StringType;
use Zentra\Asset\CustomFieldType\TextType;
use Zentra\Asset\CustomFieldType\URLType;
use Zentra\Dropdown\Dropdown;
...
class GenericobjectPluginMigration extends AbstractPluginMigration
{
    ...
    {
        return 'zentra_11_assets_migration';
    }

    #[Override]
    protected function getMainPluginTables(): array
    {
        return ['zentra_plugin_genericobject_objects'];
    }

    protected function validatePrerequisites(): bool
    {
        $required_fields = [
            'zentra_plugin_genericobject_types' => [
                'id',
                'itemtype',
                'name',
                'comment',
                ...
                // Not handled fields (features are not existing in ZENTRA yet)
                ...
                // Plugin capacity fields (will not be handled in ZENTRA itself)
                // 'use_plugin_simcard',
                // 'use_plugin_treeview',
        ...
        // Add required table fields depending on the zentra_plugin_genericobject_types entries
        if (
            $this->db->tableExists('zentra_plugin_genericobject_types')
            && $this->db->fieldExists('zentra_plugin_genericobject_types', 'itemtype')
        ) {
    ...
    protected function processMigration(): bool
    {
        ...
        $plugin_tables = \array_column(
            \iterator_to_array($this->db->listTables('zentra\_plugin\_genericobject\_%')),
            'TABLE_NAME'
        );
    ...
    {
        if ($this->db->tableExists('zentra_plugin_genericobject_typefamilies')) {
            $families_count = $this->db->request(['COUNT' => 'count', 'FROM' => 'zentra_plugin_genericobject_typefamilies'])->current()['count'];

            if ($families_count > 0) {
                $this->result->addMessage(
                    MessageType::Notice,
                    __('The object types families have not been imported as they are not handled by ZENTRA.')
                );
            }
    ...
    private function importDropdownDefinitions(): bool
    {
        ...
        $plugin_custom_tables = \array_column(
            \iterator_to_array(
                $this->db->listTables(
                    'zentra\_plugin\_genericobject\_%',
                    [
                        'NOT' => [
                            'TABLE_NAME' => [
                                'zentra_plugin_genericobject_types',
                                'zentra_plugin_genericobject_typefamilies',
                            ],
                        ],
                    ]
                )
        ...
        foreach ($this->getGenericobjectTypesIterator() as $type_data) {
            ...
            if (
                ...
                $this->result->addMessage(
                    MessageType::Debug,
                    sprintf(
                        __('%s "%s" (%d) is most recent on ZENTRA side, its update has been skipped.'),
            ...
            $profilerights_iterator = $this->db->request([
                ...
                'WHERE' => [
                    ...
                    'name' => preg_replace('/^zentra_/', '', $this->getExpectedTableForPluginClassName($plugin_itemtype)),
                ],
            ]);
                ...
                foreach ($assets_iterator as $asset_data) {
                    ...
                    foreach ($asset_data as $field => $value) {
                        ...
                        if ($value > 0 && $this->isAGenericObjectFkeyField($field)) {
                            ...
                            // ZENTRA foreign key fields are most of the time not nullable and takes a `0` value when
    ...
    private function getGenericobjectTypesIterator(): iterable
    {
        if ($this->types_iterator === null) {
            $this->types_iterator = $this->db->request(['FROM' => 'zentra_plugin_genericobject_types']);
        }
     ...
     *
     * @param array<string, mixed> $type_data A row from the `zentra_plugin_genericobject_types` table.
    ...
    {
        $table_matches = [];
        if (preg_match('/^zentra_plugin_genericobject_(?<itemtype_chunk>.+)$/', $table, $table_matches) !== 1) {
    ...
    private function getExpectedTableForPluginClassName(string $classname): string
    {
        ...
        if (!$this->db->tableExists($expected_table)) {
            ...
            // (e.g. `zentra_plugin_genericobject_item_states` instead of `zentra_plugin_genericobject_items_states`).
            // It means that `\getTableForItemType(\getItemTypeForTable($table)))` result differs
            // from the original `$table` value.
            $fallback_table = 'zentra_plugin_genericobject_' . \strtolower(\getPlural($classname_matches['itemtype_chunk']));
            ...
            $fallback_table = 'zentra_plugin_genericobject_' . \strtolower(\getSingular($classname_matches['itemtype_chunk']));
    ...
    private function getGenericObjectFieldsDefinition(string $itemtype): array
    {
        ...
        $constant_files = [
            sprintf('%s/genericobject/fields/field.constant.php', ZENTRA_PLUGIN_DOC_DIR),
            ...
            sprintf('%s/genericobject/fields/%s.constant.php', ZENTRA_PLUGIN_DOC_DIR, $system_name),
            sprintf('%s/genericobject/fields/%s.constant.php', ZENTRA_PLUGIN_DOC_DIR, \lcfirst($system_name)),
        ];
