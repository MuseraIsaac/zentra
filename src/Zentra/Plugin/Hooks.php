...
namespace Zentra\Plugin;

/**
 * @link https://zentra-developer-documentation.readthedocs.io/en/master/plugins/hooks.html
...
class Hooks
{
     ...
     *
     * On-demand JS files are loaded based on the `$CFG_ZENTRA['javascript']` array.
    ...
    /**
     * Register a function to redefine the ZENTRA menu.
     ...
     * - login field => Login field value where 'login field' is the name of the login field (usually samaccountname or uid) set in the LDAP config in ZENTRA.
     * - sync field => Sync field value where 'sync field' is the name of the sync field (usually objectguid or entryuuid) set in the LDAP config in ZENTRA
     */
     ...
     * The function is called with an array containing several properties including:
     * - 'context' => The context of the config option ('core' for core ZENTRA configs)
     ...
     * Core ZENTRA itemtypes with display preferences set in `install/empty_data.php` will never use this hook.
     * The function is called with an array containing several properties including:
     ...
     * The function is expected to return a URL starting with a '/' relative to the ZENTRA root directory, or an empty string.
     */
    public const SET_ITEM_IMPACT_ICON    = 'set_item_impact_icon'; // (keys: itemtype, items_id)

    /**
     * An array of database columns (example: zentra_mytable.myfield) that are stored using ZENTRA encrypting methods.
     * This allows plugin fields to be handled by the `zentra:security:changekey` command.
     * @since 9.4.6
     */
    public const SECURED_FIELDS  = 'secured_fields';

    /**
     * An array of configuration keys that are stored using ZENTRA encrypting methods.
     * This allows plugin configuration values to be handled by the `zentra:security:changekey` command.
     * @since 9.4.6
     */
     ...
     * Wakeonlan related is outdated and not supported in zentra-agent.
     ...
     *   - 'mode' => The response mode. See the `Zentra\Agent\CommunicationAgent::*_MODE` constants.
     *   - 'deviceid' => The device ID string assigned to the agent.
     *   - 'response' => An array containing the PROLOG response data which may differ based on the type of agent (ZENTRA Agent or an older type of agent).
     *
     * If the agent is a ZENTRA Agent, the response array will contain the following properties:
     * - 'expiration' => The inventory frequency in seconds.
     * - 'status' => Always 'ok'.
     * If the agent is not a ZENTRA Agent, the response array will contain the following properties (backwards compatibility with older types of agents):
     ...
     * The function is called with the following parameters:
     * - 'mode' => The response mode. See the `Zentra\Agent\CommunicationAgent::*_MODE` constants.
     * - 'inventory' => An `Zentra\Inventory\Inventory` object containing the discovery data.
     * - 'deviceid' => The device ID string assigned to the agent.
     ...
     * The function is called with the following parameters:
     * - 'mode' => The response mode. See the `Zentra\Agent\CommunicationAgent::*_MODE` constants.
     * - 'inventory' => An `Zentra\Inventory\Inventory` object containing the inventory data.
     * - 'deviceid' => The device ID string assigned to the agent.
     * - 'response' => An array that can be filled with data to be sent back to the agent. This will be empty unless modified by another plugin.
     * - 'errors' => An array that can be filled with errors to be sent back to the agent. This may not exist unless added by another plugin.
     ...
     * An example of this usage can be found in the `databaseinventory` plugin which responds to the ZENTRA Agent's request for database credentials to allow it to collect database information.
     * The ZENTRA Agent will only ask for these parameters if the server indicates that it has them available in the inventory task response.
     *
     * The function is called with the following parameters:
     * - 'options' => An array containing the following properties:
     ...
     *       - 'server' => 'zentra' to indicate that ZENTRA natively supports the inventory task.
     *       - 'version' => The ZENTRA server version.
     * - 'item' => The Agent item
     * The function is expected to modify the given array as needed and return it.
     ...
     * The hook is expected to be an array of classes which extend {@link Zentra\Dashboard\Filters\AbstractFilter}.
     */
     ...
     * The middleware classes should extend {@link \Zentra\Api\HL\Middleware\AbstractMiddleware} and
     * implement either {@link \Zentra\Api\HL\Middleware\RequestMiddlewareInterface{ or {@link \Zentra\Api\HL\Middleware\ResponseMiddlewareInterface}.
    ...
    /**
     * Automatic hook function to report status information through the ZENTRA status feature.
     ...
     * @link https://zentra-user-documentation.readthedocs.io/fr/latest/advanced/status.html
     */
