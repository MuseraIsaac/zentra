...
final class RichText
{
    ...
    {
        $html_tags = [
            // Most common inlined tag (handle manual HTML input, usefull for $CFG_ZENTRA['text_login'])
            'a',
            'b',
            'em',
            'i',
    ...
    private static function normalizeHtmlContent(string $content)
    {
        if (self::isRichTextHtmlContent($content)) {
            ...
            // - from "simple text mode" from ZENTRA prior to 9.4.0;
    ...
    public static function getEnhancedHtml(?string $content, array $params = []): string
    {
        $p = [
            ...
            'user_mentions'  => true,
            'images_lazy'    => true,
            'text_maxsize'   => ZENTRA_TEXT_MAXSIZE,
        ];
...
HTML;
    ...
    /**
     * Ensure current ZENTRA URL prefix (`$CFG_ZENTRA["root_doc"]`) is used in images URLs.
     * It permits to fix path to images that are broken when ZENTRA URL prefix is changed.
     *
     * @param string $content
     *
     * @return string
    ...
    private static function fixImagesPath(string $content): string
    {
        ...
        foreach ($patterns as $pattern) {
            $result = preg_replace(
                $pattern,
                sprintf(' $1="%s/front/document.send.php$2" ', $CFG_ZENTRA["root_doc"]),
                $content
            );
            if ($result === null) {
                $log_msg = sprintf(
    ...
    private static function replaceImagesByGallery(string $content): string
    {
        ...
        foreach ($image_matches as $image_match) {
            ...
            if ($document->getFromDB($docid)) {
                $docpath = ZENTRA_DOC_DIR . '/' . $document->fields['filepath'];
