...
final class QueryBuilder implements SearchInputInterface
{
    ...
    public static function displayCriteria($request = [])
    {
        ...
        if ($CFG_ZENTRA['allow_search_view'] == 2 && !isset($request['from_meta'])) {
        ...
        if ($CFG_ZENTRA['allow_search_view'] == 1 && !isset($request['from_meta'])) {
            $values['view'] = __('Items seen');
        }
        if ($CFG_ZENTRA['allow_search_all'] && !isset($request['from_meta'])) {
            $values['all'] = __('All');
        }
        $value = '';
    ...
    public static function manageParams($itemtype, $params = [], $usesession = true, $forcebookmark = false): array
    {
        ...
        if (
            ...
            && ( // Is an asset
                in_array($itemtype, $CFG_ZENTRA['asset_types'])
        ...
        if (
            ...
            || ($usesession
                && !isset($params["reset"])
                && !isset($_SESSION['zentrasearch'][$itemtype]))
        ) {
            if (!isset($user_default_values)) {
                $user_default_values = SavedSearch_User::getDefault(Session::getLoginUserID(), $itemtype);
            }
            if ($user_default_values) {
                $_SESSION['zentrasearch'][$itemtype] = [];
                ...
                if ($forcebookmark) {
                    ...
                    $_SESSION['zentra_loaded_savedsearch'] = $user_default_values['savedsearches_id'];
                }
            }
        }
        ...
        if (
            ...
            if (isset($_SESSION['zentrasearch'][$itemtype])) {
                unset($_SESSION['zentrasearch'][$itemtype]);
            }
            ...
            if (!isset($params['savedsearches_id'])) {
                unset($_SESSION['zentra_loaded_savedsearch']);
            }
        }
        ...
        if (
            ...
            foreach ($params as $key => $val) {
                $_SESSION['zentrasearch'][$itemtype][$key] = $val;
            }
        }
        ...
        foreach ($default_values as $key => $val) {
            if (!isset($params[$key])) {
                if (
                    ...
                    && isset($_SESSION['zentrasearch'][$itemtype][$key])
                ) {
                    $params[$key] = $_SESSION['zentrasearch'][$itemtype][$key];
                } else {
                    $params[$key]                    = $val;
                    $_SESSION['zentrasearch'][$itemtype][$key] = $val;
                }
    ...
    public static function getDefaultCriteria($itemtype): array
    {
        ...
        if ($CFG_ZENTRA['allow_search_view'] == 2) {
            $field = 'view';
        } else {
     ...
     *
     * @param class-string<CommonDBTM> $itemtype which zentra type we must search in session
    ...
    {
        if (!isset($_SESSION['zentrasearch'][$itemtype]['criteria'])) {
            return false;
        }
        $criteria = &$_SESSION['zentrasearch'][$itemtype]['criteria'];
