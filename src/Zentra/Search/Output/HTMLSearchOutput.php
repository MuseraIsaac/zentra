...
class HTMLSearchOutput extends AbstractSearchOutput
{
    ...
    public function displayData(array $data, array $params = [])
    {
        ...
        if (isset($_SESSION['zentra_loaded_savedsearch'])) {
            $savedsearch = new SavedSearch();
            if ($savedsearch->getFromDB($_SESSION['zentra_loaded_savedsearch'])) {
        ...
        TemplateRenderer::getInstance()->display('components/search/display_data.html.twig', [
            ...
            'data'                => $data,
            'union_search_type'   => $CFG_ZENTRA["union_search_type"],
            'rand'                => $rand,
            ...
            'massiveactionparams' => $data['search']['massiveactionparams'] + [
                'num_displayed' => min($_SESSION['zentralist_limit'], $count),
    ...
    public static function showItem($value, &$num, $row, $extraparam = ''): string
    {
        ...
        if (
            count($values) > 1
            && Toolbox::strlen($value) > $CFG_ZENTRA['cut']
        ) {
            $value = '';
            foreach ($values as $v) {
