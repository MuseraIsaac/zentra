...
namespace Zentra\Search\Provider;
...
final class SQLProvider implements SearchProviderInterface
{
    ...
    private static function buildSelect(array $data, string $itemtable): string
    {
        ...
        $SELECT = "SELECT DISTINCT `$itemtable`.`id` AS id, " . $DB->quote($_SESSION['zentraname'] ?? '') . " AS currentuser,
        ...
        if ($as_map && $data['itemtype'] !== 'Entity') {
            $SELECT .= ' `zentra_locations`.`id` AS loc_id, ';
        }
    ...
    public static function getDefaultSelectCriteria(string $itemtype): array
    {
        ...
        switch ($itemtype) {
            case 'FieldUnicity':
                $ret[] = "`zentra_fieldunicities`.`itemtype` AS ITEMTYPE";
                break;
    ...
    public static function getSelectCriteria(string $itemtype, int $ID, bool $meta = false, string $meta_type = '')
    {
        ...
        if (
            ((($is_fkey_composite_on_self || $table != $orig_table)
                    && (!isset($CFG_ZENTRA["union_search_type"][$itemtype])
                        || ($CFG_ZENTRA["union_search_type"][$itemtype] != $table)))
                || !empty($complexjoin))
            && ($opt["linkfield"] != getForeignKeyFieldForTable($table))
        ) {
        ...
        switch ($table . "." . $field) {
                ...
                if ($itemtype !== User::class) {
                    if ($opt->isForceGroupBy()) {
                        ...
                        if (
                            ...
                            && in_array($opt['joinparams']['beforejoin']['table'], ['zentra_tickets_users', 'zentra_changes_users', 'zentra_problems_users'])
                        ) { // For tickets_users
            ...
            case "zentra_softwarelicenses.number":
                    ...
                    $SELECT = [
                        QueryFunction::groupConcat(
                            expression: QueryFunction::concat([
                                ...
                                new QueryExpression($DB::quoteValue(Search::SHORTSEP)),
                                "zentra_profiles_users{$addtable2}.entities_id",
                                new QueryExpression($DB::quoteValue(Search::SHORTSEP)),
                                "zentra_profiles_users{$addtable2}.is_recursive",
                                new QueryExpression($DB::quoteValue(Search::SHORTSEP)),
                                "zentra_profiles_users{$addtable2}.is_dynamic",
                            ]),
                            distinct: true,
                            separator: Search::LONGSEP,
                            alias: $NAME
            ...
            case "zentra_entities.completename":
            ...
            case "zentra_softwareversions.name":
            ...
            case "zentra_softwareversions.comment":
                $_table = ($meta && $meta_type === Software::class) ? 'zentra_softwares' : ($table . $addtable);
                    ...
                    $SELECT = [
                        QueryFunction::groupConcat(
                            expression: QueryFunction::concat([
                                "zentra_softwares.name",
                                new QueryExpression($DB::quoteValue(" - ")),
                                "zentra_softwareversions{$addtable}.name",
                                new QueryExpression($DB::quoteValue(" - ")),
                                "{$table}{$addtable2}.{$field}",
                                new QueryExpression($DB::quoteValue(Search::SHORTSEP)),
                                "{$table}{$addtable2}.id",
            ...
            case "zentra_itilfollowups.content":
            case "zentra_tickettasks.content":
            case "zentra_changetasks.content":
            case "zentra_problemtasks.content":
            case "zentra_tickettasks.state":
            case "zentra_changetasks.state":
            case "zentra_problemtasks.state":
            case "zentra_tickettasks.is_private":
            case "zentra_changetasks.is_private":
            case "zentra_problemtasks.is_private":
            case "zentra_tickettasks.actiontime":
            case "zentra_changetasks.actiontime":
            case "zentra_problemtasks.actiontime":
            case "zentra_tickettasks.begin":
            case "zentra_changetasks.begin":
            case "zentra_problemtasks.begin":
            case "zentra_tickettasks.end":
            case "zentra_changetasks.end":
            case "zentra_problemtasks.end":
        ...
        $plugin_table_pattern = "/^zentra_plugin_([a-z0-9]+)/";
    ...
    public static function getDefaultWhereCriteria(string $itemtype): array
    {
        ...
        switch ($itemtype) {
            ...
            case Notification::class:
                if (!Config::canView()) {
                    $criteria = [
                        'NOT' => ['zentra_notifications.itemtype' => ['CronTask', 'DBConnection']],
                    ];
                }
                break;
            ...
            case User::class:
                ...
                if (!Session::canViewAllEntities()) {
                    $criteria = getEntitiesRestrictCriteria("zentra_profiles_users", '', '', true);
                }
                break;
            ...
            case ProjectTask::class:
                if (!Session::haveRightsOr('project', [Project::READALL, Project::READMY])) {
                    ...
                    $teamtable = 'zentra_projecttaskteams';
                    $user_criteria = [
            ...
            case Project::class:
                if (!Session::haveRight("project", Project::READALL)) {
                    ...
                    if (count($_SESSION['zentragroups'])) {
                        ...
                        $or_criteria[] = ["zentra_projects.groups_id" => $_SESSION['zentragroups']];
                        $or_criteria[] = $group_criteria;
                    }
                    $criteria = [
                        "OR" => $or_criteria,
                ...
                if (!Session::haveRight("ticket", Ticket::READALL)) {
                    ...
                    if (Session::haveRight("ticket", Ticket::READGROUP)) {
                        if (count($_SESSION['zentragroups'])) {
                            $criteria['OR'][] = [
                                'OR' => [
                                    "$requestergroup_table.groups_id" => $_SESSION['zentragroups'],
                                    "$observergroup_table.groups_id" => $_SESSION['zentragroups'],
                                ],
                            ];
                        }
                    }
                    ...
                    if (Session::haveRight("ticket", Ticket::READASSIGN)) { // assign to me
                        ...
                        if (count($_SESSION['zentragroups'])) {
                            $criteria['OR'][] = [
                                "$assigngroup_table.groups_id" => $_SESSION['zentragroups'],
                            ];
                        }
                    }
                    ...
                    if (
                        ...
                        if (count($_SESSION['zentragroups'])) {
                            $criteria['OR'][] = [
                                'AND' => [
                                    "`zentra_ticketvalidations`.`itemtype_target`" => Group::class,
                                    "`zentra_ticketvalidations`.`items_id_target`" => $_SESSION['zentragroups'],
                                ],
                            ];
                        }
                    }
            ...
            case Problem::class:
                if ($itemtype === Change::class) {
                    $right       = 'change';
                    $table       = 'changes';
                    $groupetable = "`zentra_changes_groups_";
                } elseif ($itemtype === Problem::class) {
                    $right       = 'problem';
                    $table       = 'problems';
                    $groupetable = "`zentra_groups_problems_";
                }
                // Same structure in addDefaultJoin
                $condition = '';
                if (!Session::haveRight("$right", $itemtype::READALL)) {
                    ...
                    if (Session::haveRight("$right", $itemtype::READMY)) {
                        $requester_table      = '`zentra_' . $table . '_users_'
                        ...
                        $observer_table       = '`zentra_' . $table . '_users_'
                        ...
                        $assign_table         = '`zentra_' . $table . '_users_'
                        ...
                        $criteria['OR'][] = [
                            'OR' => [
                                ...
                                "$observer_table.users_id" => Session::getLoginUserID(),
                                "$assign_table.users_id" => Session::getLoginUserID(),
                                "zentra_" . $table . ".users_id_recipient" => Session::getLoginUserID(),
                            ],
                        ];
                        if (count($_SESSION['zentragroups'])) {
                            $criteria['OR'][] = [
            ...
            case TicketTask::class:
                ...
                $criteria = [
                    'OR' => [
                        'zentra_tickettasks.is_private' => $allowed_is_private,
                        // Check for assigned or created tasks
                        'zentra_tickettasks.users_id' => Session::getLoginUserID(),
                        'zentra_tickettasks.users_id_tech' => Session::getLoginUserID(),
                    ],
                ];
            ...
            case ITILFollowup::class:
                ...
                $criteria = [
                    'zentra_itilfollowups.is_private' => $allowed_is_private,
                    'OR' => [
                        ...
                        new QueryExpression(ITILFollowup::buildParentCondition(
                            Change::getType(),
                            'changes_id',
                            "zentra_changes_users",
                            "zentra_changes_groups"
                        )),
                        new QueryExpression(ITILFollowup::buildParentCondition(
                            Problem::getType(),
                            'problems_id',
                ...
                // Entity restrictions
                $entity_restrictions = [];
                foreach ($CFG_ZENTRA['itil_types'] as $itil_itemtype) {
    ...
    public static function getWhereCriteria($nott, $itemtype, $ID, $searchtype, $val, $meta = false): ?array
    {
        ...
        // See https://github.com/zentra-project/zentra/pull/13684 for detailed examples
        ...
        switch ($inittable . "." . $field) {
            // case "zentra_users_validation.name" :

            case "zentra_users.name":
                if ($val === 'myself') {
                    switch ($searchtype) {
                        case 'equals':
                            $SEARCH = ['=', $_SESSION['zentraID']];
                            break;
                ...
                if ($itemtype === User::class) { // zentra_users case / not link table
                    $criteria = [
                        'OR' => [],
                    ];
                ...
                if ($_SESSION["zentranames_format"] == User::FIRSTNAME_BEFORE) {
                    $name1 = 'firstname';
                    $name2 = 'realname';
                } else {
                    $name1 = 'realname';
                ...
                if (is_a($itemtype, CommonITILObject::class, true)) {
                    $itil_user_tables = ['zentra_tickets_users', 'zentra_changes_users', 'zentra_problems_users'];
                ...
                // no break
            case "zentra_groups.completename":
                if ($val === 'mygroups') {
                    switch ($searchtype) {
                        case 'equals':
                            if (count($_SESSION['zentragroups']) === 0) {
                                return [];
                            } else {
                                $SEARCH = ['IN', $_SESSION['zentragroups']];
                            }
                            break;
                        ...
                        case 'notequals':
                            if (count($_SESSION['zentragroups']) === 0) {
                                ...
                                if ($use_subquery_on_id_search) {
                                    ...
                                    $SEARCH = ['NOT IN', $_SESSION['zentragroups']];
                                }
                            }
                            break;
                        ...
                        case 'under':
                            ...
                            foreach ($_SESSION['zentragroups'] as $g) {
            ...
            case "zentra_tickets.status":
            case "zentra_problems.status":
            case "zentra_changes.status":
                $tocheck = [];
                $item = getItemForItemtype($itemtype);
            ...
            case "zentra_tickets_tickets.tickets_id_1":
                $tmplink = 'OR';
                $compare = '=';
                if ($nott) {
                    $tmplink = 'AND';
            ...
            case "zentra_tickets.priority":
            case "zentra_tickets.impact":
            case "zentra_tickets.urgency":
            case "zentra_problems.priority":
            case "zentra_problems.impact":
            case "zentra_problems.urgency":
            case "zentra_changes.priority":
            case "zentra_changes.impact":
            case "zentra_changes.urgency":
            case "zentra_projects.priority":
                if (is_numeric($val)) {
                    if ($val > 0) {
                        $compare = ($nott ? '<>' : '=');
                        return [
            ...
            case "zentra_tickets.global_validation":
            case "zentra_ticketvalidations.status":
            case "zentra_changes.global_validation":
            case "zentra_changevalidations.status":
            ...
            case "zentra_notifications.event":
        ...
        // Link with plugin tables
        if (preg_match("/^zentra_plugin_([a-z0-9]+)/", $inittable, $matches)) {
            if (count($matches) == 2) {
                $plug     = $matches[1];
                $out = Plugin::doOneHook(
                    $plug,
        ...
        if ($use_subquery_on_id_search || $use_subquery_on_text_search) {
            ...
            if ($use_subquery_on_id_search) {
                ...
                //
                // AND `zentra_tickets`.`id` NOT IN (
                //     SELECT `tickets_id`
                //     FROM `zentra_groups_tickets`
                //     WHERE `groups_id` = '4' AND `zentra_groups_tickets`.`type` = '3'
                // )
                ...
                //      ) AND `zentra_groups_tickets`.`type` = '3'
                // )
        ...
        switch ($itemtype) {
            ...
            case Problem::class:
                if ($itemtype === Change::class) {
                    $right       = 'change';
                    $table       = 'changes';
                    $groupetable = "zentra_changes_groups";
                    $linkfield   = "changes_groups_id";
                } elseif ($itemtype === Problem::class) {
                    $right       = 'problem';
                    $table       = 'problems';
                    $groupetable = "zentra_groups_problems";
    ...
    public static function getLeftJoinCriteria(
        ...
        if ($tocheck === 'zentra_groups' && ($itemtype === AllAssets::class || $itemtype === ReservationItem::class)) {
            $already_link_tables[] = 'zentra_groups_items';
            return [
                'LEFT JOIN' => [
                    'zentra_groups_items' => [
                        'ON' => [
                            'zentra_groups_items' => 'items_id',
                            $rt => 'id', [
                                'AND' => [
                                    'zentra_groups_items.itemtype' => $rt . '_TYPE', // Placeholder to be replaced at the end of the SQL construction during union case handling
                                    'zentra_groups_items.type' => Group_Item::GROUP_TYPE_NORMAL,
                                ],
                            ],
                        ],
                    ],
        ...
        if (
            $specific_leftjoin_criteria === []
            && preg_match("/^zentra_plugin_([a-z0-9]+)/", $new_table, $matches)
        ) {
            if (count($matches) == 2) {
    ...
    public static function getMetaLeftJoinCriteria(string $from_type, string $to_type, array &$already_link_tables2, array $joinparams = []): array
    {
        ...
        // Specific JOIN
        if ($from_referencetype === Software::class && in_array($to_type, $CFG_ZENTRA['software_types'], true)) {
            // From Software to software_types
            $softwareversions_table = "zentra_softwareversions{$alias_suffix}";
            if (!in_array($softwareversions_table, $already_link_tables2, true)) {
                $already_link_tables2[] = $softwareversions_table;
                $joins['LEFT JOIN']["`zentra_softwareversions` AS `$softwareversions_table`"] = [
                    'ON' => [
            ...
            $items_softwareversions_table = "zentra_items_softwareversions_{$alias_suffix}";
            if (!in_array($items_softwareversions_table, $already_link_tables2, true)) {
                $already_link_tables2[] = $items_softwareversions_table;
                $joins['LEFT JOIN']["`zentra_items_softwareversions` AS `$items_softwareversions_table`"] = [
                    'ON' => [
        ...
        if ($to_type === Software::class && in_array($from_referencetype, $CFG_ZENTRA['software_types'], true)) {
            // From software_types to Software
            $items_softwareversions_table = "zentra_items_softwareversions{$alias_suffix}";
            if (!in_array($items_softwareversions_table, $already_link_tables2, true)) {
                $already_link_tables2[] = $items_softwareversions_table;
                $joins['LEFT JOIN']["`zentra_items_softwareversions` AS `$items_softwareversions_table`"] = [
                    'ON' => [
            ...
            $softwarelicenses_table = "zentra_softwarelicenses{$alias_suffix}";
            if (!in_array($softwarelicenses_table, $already_link_tables2, true)) {
                $already_link_tables2[] = $softwarelicenses_table;
                $joins['LEFT JOIN']["`zentra_softwarelicenses` AS `$softwarelicenses_table`"] = [
                    'ON' => [
                        $to_table_alias => 'id',
        ...
        if ($from_referencetype === Budget::class && in_array($to_type, $CFG_ZENTRA['infocom_types'], true)) {
            // From Budget to infocom_types
            $infocom_alias = "zentra_infocoms{$alias_suffix}";
            if (!in_array($infocom_alias, $already_link_tables2, true)) {
                $already_link_tables2[] = $infocom_alias;
                $joins['LEFT JOIN']["`zentra_infocoms` AS `$infocom_alias`"] = [
                    'ON' => [
                        $from_table => 'id',
        ...
        if ($to_type === Budget::class && in_array($from_referencetype, $CFG_ZENTRA['infocom_types'], true)) {
        ...
        if ($from_referencetype === Reservation::class && in_array($to_type, $CFG_ZENTRA['reservation_types'], true)) {
            // From Reservation to reservation_types
            $reservationitems_alias = "zentra_reservationitems{$alias_suffix}";
            if (!in_array($reservationitems_alias, $already_link_tables2, true)) {
                $already_link_tables2[] = $reservationitems_alias;
                $joins['LEFT JOIN']["`zentra_reservationitems` AS `$reservationitems_alias`"] = [
                    'ON' => [
        ...
        if ($to_type === Reservation::class && in_array($from_referencetype, $CFG_ZENTRA['reservation_types'], true)) {
        ...
        if (
            (
                in_array($to_type, $CFG_ZENTRA['directconnect_types'])
                && in_array($from_referencetype, Asset_PeripheralAsset::getPeripheralHostItemtypes(), true)
            )
            || (
                in_array($from_referencetype, $CFG_ZENTRA['directconnect_types'])
                && in_array($to_type, Asset_PeripheralAsset::getPeripheralHostItemtypes(), true)
            )
        ) {
            $asset_itemtype = in_array($to_type, $CFG_ZENTRA['directconnect_types']) ? $from_referencetype : $to_type;
        ...
        if ($to_type === Group::class && in_array($from_referencetype, $CFG_ZENTRA['assignable_types'], true)) {
            $relation_table_alias = 'zentra_groups_items' . $alias_suffix;
            if (!in_array($relation_table_alias, $already_link_tables2, true)) {
                $already_link_tables2[] = $relation_table_alias;
                $joins['LEFT JOIN']["`zentra_groups_items` AS `$relation_table_alias`"] = [
                    'ON' => [
        ...
        if ($from_referencetype === Group::class && in_array($to_type, $CFG_ZENTRA['assignable_types'], true)) {
            $relation_table_alias = 'zentra_groups_items' . $alias_suffix;
            if (!in_array($relation_table_alias, $already_link_tables2, true)) {
                $already_link_tables2[] = $relation_table_alias;
                $joins['LEFT JOIN']["`zentra_groups_items` AS `$relation_table_alias`"] = [
        ...
        if ($from_obj && $from_obj->isField($to_fk)) {
            ...
            // zentra_$from_items table exists and has a foreign key corresponding to $to_table
            ...
            // zentra_$to_items table exists and has a foreign key corresponding to $from_table
    ...
    public static function getDropdownTranslationJoinCriteria($alias, $table, $itemtype, $field): array
    {
        ...
        return [
            'LEFT JOIN' => [
                "zentra_dropdowntranslations AS $alias" => [
                    'ON' => [
                        $alias => 'itemtype',
                        ...
                        [
                            'AND' => [
                                ...
                                "$alias.language" => $_SESSION['zentralanguage'],
                                "$alias.field"    => $field,
                            ],
                        ],
                    ],
    ...
    public static function getHavingCriteria(string $LINK, bool $NOT, string $itemtype, int $ID, string $searchtype, string $val): array
    {
        ...
        //// Default cases
        // Link with plugin tables
        if (preg_match("/^zentra_plugin_([a-z0-9]+)/", $table, $matches)) {
            if (count($matches) === 2) {
                $plug     = $matches[1];
                $out = Plugin::doOneHook(
                    $plug,
    ...
    public static function getOrderByCriteria(string $itemtype, array $sort_fields): array
    {
        ...
        foreach ($sort_fields as $sort_field) {
            ...
            if (isset($CFG_ZENTRA["union_search_type"][$itemtype])) {
                    ...
                    case "zentra_users.name":
                        if ($itemtype != User::class) {
                            ...
                            if (
                                ...
                                && in_array($searchopt[$ID]['joinparams']['beforejoin']['table'], ['zentra_tickets_users', 'zentra_changes_users', 'zentra_problems_users'])
                            ) { // For tickets_users
            ...
            // Link with plugin tables
            if ($criterion === null && preg_match("/^zentra_plugin_([a-z0-9]+)/", $table, $matches)) {
                if (count($matches) == 2) {
                    $plug = $matches[1];
                    $out = Plugin::doOneHook(
                        $plug,
    ...
    public static function constructSQL(array &$data)
    {
        ...
        if (isset($CFG_ZENTRA['union_search_type'][$data['itemtype']])) {
            $itemtable          = $CFG_ZENTRA['union_search_type'][$data['itemtype']];
        ...
        if ($entity_restrict) {
            ...
            } elseif (isset($CFG_ZENTRA["union_search_type"][$data['itemtype']])) {
        ...
        if ($data['search']['no_search']) {
            ...
            $query_num = "SELECT $count,
                              " . $DB->quote($_SESSION['zentraname']) . " AS currentuser
                       FROM `$itemtable`"
                . $COMMONLEFTJOIN;
            ...
            if (isset($CFG_ZENTRA["union_search_type"][$data['itemtype']])) {
                ...
                foreach ($CFG_ZENTRA[$CFG_ZENTRA["union_search_type"][$data['itemtype']]] as $ctype) {
                    ...
                    if (
                        ...
                        if ($data['itemtype'] == AllAssets::getType()) {
                            $query_num  = str_replace(
                                $CFG_ZENTRA["union_search_type"][$data['itemtype']],
                                $ctable,
                                $tmpquery
                            );
        ...
        if (isset($CFG_ZENTRA["union_search_type"][$data['itemtype']])) {
            ...
            foreach ($CFG_ZENTRA[$CFG_ZENTRA["union_search_type"][$data['itemtype']]] as $ctype) {
                ...
                if (
                    ...
                    if ($data['itemtype'] == AllAssets::getType()) {
                        ...
                        $tmpquery = str_replace(
                            $CFG_ZENTRA["union_search_type"][$data['itemtype']] . '_TYPE',
                            $ctype,
                            $tmpquery
                        );
                        $tmpquery = str_replace(
                    ...
                    // SOFTWARE HACK
                    if ($ctype == 'Software') {
                        $tmpquery = str_replace("`zentra_softwares`.`serial`", "''", $tmpquery);
                        $tmpquery = str_replace("`zentra_softwares`.`otherserial`", "''", $tmpquery);
                    }
            ...
            $QUERY .= str_replace($CFG_ZENTRA["union_search_type"][$data['itemtype']] . ".", "", $ORDER)
                . $LIMIT;
        } else {
            $system_criteria_sql = self::getMainItemtypeSystemSQLCriteria($data['itemtype']);
            if ($system_criteria_sql !== '') {
    ...
    public static function giveItem(
        ...
        if (
            isset($CFG_ZENTRA["union_search_type"][$itemtype])
            && ($CFG_ZENTRA["union_search_type"][$itemtype] == $searchopt[$ID]["table"])
        ) {
            $oparams = [];
            if (
            ...
            switch ($table . '.' . $field) {
                ...
                case "zentra_entities.completename":
                    if ($itemtype == User::class) {
                        ...
                        for ($k = 0; $k < $data[$ID]['count']; $k++) {
                            ...
                            if (
                                ...
                                $text = sprintf(
                                    ...
                                    htmlescape(Dropdown::getDropdownName('zentra_profiles', $data[$ID][$k]['profiles_id']))
                                );
                                $comp = '';
                                if ($data[$ID][$k]['is_recursive']) {
                                    $comp = __('R');
                        ...
                        if ($html_output) {
                            if (!$_SESSION['zentrause_flat_dropdowntree_on_search_result']) {
                    ...
                    if (($so["datatype"] ?? "") != "itemlink") {
                        ...
                        for ($k = 0; $k < $data[$ID]['count']; $k++) {
                            ...
                            if (
                                ...
                                && !$_SESSION['zentrause_flat_dropdowntree_on_search_result'] //user doesn't want the completename
                            ) {
                                $split_name = explode(">", $completename);
                ...
                case "zentra_documenttypes.icon":
                    if (!empty($data[$ID][0]['name'])) {
                        return "<img class='middle' alt='' src='" . htmlescape($CFG_ZENTRA["typedoc_icon_dir"] . "/" . $data[$ID][0]['name']) . "'>";
                    }
                    return '';

                case "zentra_documents.filename":
                    $doc = new Document();
                ...
                case "zentra_tickets_tickets.tickets_id_1":
                    ...
                    for ($k = 0; $k < $data[$ID]['count']; $k++) {
                        ...
                        if ((is_int($linkid) && $linkid > 0) && !isset($displayed[$linkid])) {
                            $link_text = Dropdown::getDropdownName('zentra_tickets', $linkid);
                            if ($_SESSION["zentrais_ids_visible"] || empty($link_text)) {
                ...
                case "zentra_problems.id":
                    if ($so["datatype"] == 'count') {
                        if (
                            ...
                            $out .= "href=\"" . \htmlescape($CFG_ZENTRA["root_doc"] . "/front/problem.php?" . Toolbox::append_params($options, '&')) . "\">";
                ...
                case "zentra_tickets.id":
                    if ($so["datatype"] == 'count') {
                        if (
                            ...
                            $out .= "href=\"" . \htmlescape($CFG_ZENTRA["root_doc"] . "/front/ticket.php?" . Toolbox::append_params($options, '&')) . "\">";
                            $out .= \htmlescape($data[$ID][0]['name']) . "</a>";
                            return $out;
                        }
                    }
                ...
                case "zentra_tickets.time_to_resolve":
                case "zentra_problems.time_to_resolve":
                case "zentra_changes.time_to_resolve":
                case "zentra_tickets.time_to_own":
                case "zentra_tickets.internal_time_to_own":
                case "zentra_tickets.internal_time_to_resolve":
                    // Due date + progress
                    if (in_array($orig_id, [151, 158, 181, 186])) {
                        ...
                        if (
                            ...
                            if (
                                $table . '.' . $field == "zentra_tickets.time_to_resolve"
                                || $table . '.' . $field == "zentra_tickets.internal_time_to_resolve"
                            ) {
                                return '';
                            } else {
                                $color = '#AAAAAA';
                        ...
                        $less_warn_limit = 0;
                        $less_warn       = 0;
                        if ($_SESSION['zentraduedatewarning_unit'] == '%') {
                            $less_warn_limit = $_SESSION['zentraduedatewarning_less'];
                            $less_warn       = (100 - $percentage);
                        } elseif ($_SESSION['zentraduedatewarning_unit'] == 'hour') {
                            $less_warn_limit = $_SESSION['zentraduedatewarning_less'] * HOUR_TIMESTAMP;
                            $less_warn       = ($totaltime - $currenttime);
                        } elseif ($_SESSION['zentraduedatewarning_unit'] == 'day') {
                            $less_warn_limit = $_SESSION['zentraduedatewarning_less'] * DAY_TIMESTAMP;
                            $less_warn       = ($totaltime - $currenttime);
                        }

                        $less_crit_limit = 0;
                        $less_crit       = 0;
                        if ($_SESSION['zentraduedatecritical_unit'] == '%') {
                            $less_crit_limit = $_SESSION['zentraduedatecritical_less'];
                            $less_crit       = (100 - $percentage);
                        } elseif ($_SESSION['zentraduedatecritical_unit'] == 'hour') {
                            $less_crit_limit = $_SESSION['zentraduedatecritical_less'] * HOUR_TIMESTAMP;
                            $less_crit       = ($totaltime - $currenttime);
                        } elseif ($_SESSION['zentraduedatecritical_unit'] == 'day') {
                            $less_crit_limit = $_SESSION['zentraduedatecritical_less'] * DAY_TIMESTAMP;
                            $less_crit       = ($totaltime - $currenttime);
                        }

                        if ($color === null) {
                            $color = $_SESSION['zentraduedateok_color'];
                            if ($less_crit < $less_crit_limit) {
                                $color = $_SESSION['zentraduedatecritical_color'];
                            } elseif ($less_warn < $less_warn_limit) {
                                $color = $_SESSION['zentraduedatewarning_color'];
                            }
                        }
                        ...
                        switch ($table . "." . $field) {
                            ...
                            case "zentra_changes.time_to_resolve":
                                ...
                                $is_late = !empty($value)
                                    ...
                                    && (
                                        $solve_date > $value
                                        || ($solve_date == null && $value < $_SESSION['zentra_currenttime'])
                                    );
                                break;
                            case "zentra_tickets.time_to_own":
                            case "zentra_tickets.internal_time_to_own":
                                ...
                                $is_late = !empty($value)
                                    ...
                                    && (
                                        $tia_date > $value
                                        || ($tia_date == null && $value < $_SESSION['zentra_currenttime'])
                                    );
                        }
                        if ($is_late) {
                ...
                case "zentra_reservationitems.comment":
                    if (empty($data[$ID][0]['name'])) {
                        $text = __s('None');
                    } else {
                ...
                case 'zentra_crontasks.description':
                    $tmp = new CronTask();
                ...
                case 'zentra_projectstates.name':
                    $name = $data[$ID][0]['name'];
                ...
                case 'zentra_items_tickets.items_id':
                case 'zentra_items_problems.items_id':
                case 'zentra_changes_items.items_id':
                case 'zentra_certificates_items.items_id':
                case 'zentra_appliances_items.items_id':
                    if (!empty($data[$ID])) {
                        $items = [];
                ...
                case 'zentra_items_tickets.itemtype':
                case 'zentra_items_problems.itemtype':
                    if (!empty($data[$ID])) {
                        $itemtypes = [];
                        foreach ($data[$ID] as $key => $val) {
                            if (is_numeric($key)) {
                ...
                case 'zentra_tickets.name':
                case 'zentra_problems.name':
                case 'zentra_changes.name':
                    if (
                        isset($data[$ID][0]['id'])
                ...
                case 'zentra_ticketsatisfactions.satisfaction':
                    if ($html_output) {
                ...
                case 'zentra_changesatisfactions.satisfaction':
                    if ($html_output) {
                ...
                case 'zentra_projects._virtual_planned_duration':
                    return \htmlescape(
                ...
                case 'zentra_projects._virtual_effective_duration':
                    return \htmlescape(
                        Html::timestampToString(
                ...
                case 'zentra_cartridgeitems._virtual':
                ...
                case 'zentra_printers._virtual':
                    return Cartridge::getCountForPrinter(
                        $data["id"],
                        !$html_output
                    );

                case 'zentra_consumableitems._virtual':
                ...
                case 'zentra_reservationitems._virtual':
                ...
                case "zentra_projects.priority":
                    ...
                    $color = \htmlescape($_SESSION["zentrapriority_$index"]);
                ...
                case "zentra_knowbaseitems.name":
                    global $DB;
                    $result = $DB->request([
                        'SELECT' => [
                ...
                case "zentra_certificates.date_expiration":
                    if (
                        !in_array($orig_id, [151, 158, 181, 186])
                        && !empty($data[$ID][0]['name'])
                    ) {
                        $date = $data[$ID][0]['name'];
                        $before = Entity::getUsedConfig('send_certificates_alert_before_delay', $_SESSION['zentraactive_entity']);
                        $color = ($date < $_SESSION['zentra_currenttime']) ? '#cf9b9b' : null;
                        if ($before) {
                            $before = date('Y-m-d', strtotime($_SESSION['zentra_currenttime'] . "+$before days"));
                            $color = match (true) {
                                $date < $_SESSION['zentra_currenttime'] => '#d63939',
                                $date < $before => '#de5d06',
                                $date >= $before => '#a1cf66',
                                default => null
                            };
                    ...
                    break;
                case "zentra_domains.date_expiration":
                    if (!empty($data[$ID][0]['name'])
                        && ($data[$ID][0]['name'] < $_SESSION['zentra_currenttime'])
                    ) {
                        return "<div class='badge_block' style='border-color: #cf9b9b'>
        ...
        if (isset($table) && isset($field)) {
            if (preg_match("/^zentra_plugin_([a-z0-9]+)/", $table . '.' . $field, $matches)) {
                if (count($matches) == 2) {
                    $plug     = $matches[1];
                    $out = Plugin::doOneHook(
                        $plug,
        ...
        if (isset($so["datatype"])) {
            switch ($so["datatype"]) {
                case "itemlink":
                    ...
                    for ($k = 0; $k < $data[$ID]['count']; $k++) {
                        if (isset($data[$ID][$k]['id'])) {
                            ...
                            $name  = $data[$ID][$k]['name'];
                            if ($_SESSION["zentrais_ids_visible"] || empty($data[$ID][$k]['name'])) {
                ...
                case "text":
                    ...
                    for ($k = 0; $k < $data[$ID]['count']; $k++) {
                        if (trim((string) $data[$ID][$k]['name']) !== '') {
                            ...
                            if ($html_output && (Toolbox::strlen($plaintext) > $CFG_ZENTRA['cut'])) {
                                $rand = mt_rand();
                                $popup_params = [
                                    'display'       => false,
                    ...
                    for ($k = 0; $k < $data[$ID]['count']; $k++) {
                        if (trim((string) $data[$ID][$k]['name']) !== '') {
                            ...
                            if (
                                ...
                                $out .= Dropdown::getValueWithUnit($data[$ID][$k]['name'], $unit, $CFG_ZENTRA["decimal_number"]);
                            }
                        }
                    }
                    $out = "<span class='text-nowrap'>" . \htmlescape($out) . "</span>";
                ...
                case "language":
                    if (isset($data[$ID][0]['name'], $CFG_ZENTRA['languages'][$data[$ID][0]['name']])) {
                        return $CFG_ZENTRA['languages'][$data[$ID][0]['name']][0];
                    }
