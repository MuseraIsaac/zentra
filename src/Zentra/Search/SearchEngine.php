...
use Zentra\Search\Input\SearchInputInterface;
use Zentra\Search\Output\AbstractSearchOutput;
...
use Zentra\Search\Output\Ods;
use Zentra\Search\Output\Pdf;
use Zentra\Search\Output\Xlsx;
use Zentra\Search\Provider\SearchProviderInterface;
...
final class SearchEngine
{
    ...
    public static function getMetaItemtypeAvailable($itemtype): array
    {
        ...
        $linked = [];
        foreach ($CFG_ZENTRA as $key => $values) {
            if ($key === 'link_types') {
    ...
    public static function prepareDataForSearch($itemtype, array $params, array $forcedisplay = []): array
    {
        ...
        foreach ($params as $key => $val) {
            switch ($key) {
                case 'order':
                    if (!is_array($val)) {
                        // Backward compatibility with ZENTRA < 10.0 links
        ...
        if (!$CFG_ZENTRA['allow_search_all']) {
        ...
        if (!$CFG_ZENTRA['allow_search_view'] && !array_key_exists('globalsearch', $p)) {
            foreach ($p['criteria'] as $val) {
    ...
    {
        unset($_SESSION['zentrasearch']);
        $_SESSION['zentrasearch'] = [];
    }
    ...
    public static function show(string $itemtype, array $params = []): void
    {
        ...
        if (($params['init_session_data'] ?? false) && isset($params['criteria'])) {
            ...
            $_SESSION['zentrasearch'][$itemtype]['criteria'] = $params['criteria'];
        }
        ...
        echo "<div class='col search-container' data-zentra-search-container>";
        ...
        if ($_SESSION['zentrashow_search_form']) {
