    ...
    public static function getOptionsForItemtype($itemtype, $withplugins = true): array
    {
        ...
        switch ($itemtype) {
            case 'Internet':
                ...
                $search[$itemtype][31]['table']         = 'zentra_states';
            ...
            case AllAssets::getType():
                ...
                $search[$itemtype][70]['table']         = 'zentra_users';
                ...
                $search[$itemtype][71]['table']         = 'zentra_groups';
                ...
                $search[$itemtype][23]['table']         = 'zentra_manufacturers';
                $search[$itemtype][23]['field']         = 'name';
                $search[$itemtype][23]['name']          = Manufacturer::getTypeName(1);

                $search[$itemtype][24]['table']         = 'zentra_users';
                ...
                $search[$itemtype][49]['table']          = 'zentra_groups';
                ...
                $search[$itemtype][80]['table']         = 'zentra_entities';
        ...
        if (
            ...
            && in_array($itemtype, $CFG_ZENTRA["ticket_types"])
        ) {
        ...
        if (
            in_array($itemtype, $CFG_ZENTRA["networkport_types"])
        ...
        if (
            in_array($itemtype, $CFG_ZENTRA["contract_types"])
            || ($itemtype == AllAssets::getType())
        ) {
        ...
        if (
            in_array($itemtype, $CFG_ZENTRA["domain_types"])
            || ($itemtype == AllAssets::getType())
        ) {
        ...
        if (
            in_array($itemtype, $CFG_ZENTRA["appliance_types"])
            || ($itemtype == AllAssets::getType())
        ) {
            $fn_append_options(Appliance::getSearchOptionsToAdd($itemtype));
        }

        if (in_array($itemtype, $CFG_ZENTRA["link_types"])) {
        ...
        if (in_array($itemtype, $CFG_ZENTRA['socket_types'], true)) {
        ...
        if (!is_a($itemtype, CommonDBTM::class, true)) { // Special union type
            $itemtable = $CFG_ZENTRA['union_search_type'][$itemtype];
        } else {
                ...
                switch ($searchopt[$field_num]['datatype']) {
                    ...
                    case 'decimal':
                        ...
                        if (
                            ...
                            // https://github.com/zentra-project/zentra/issues/6917
    ...
    public static function getDefaultToView(string $itemtype, array $params = []): array
    {
        ...
        if (
            ...
            && (isset($CFG_ZENTRA["union_search_type"][$itemtype])
                ...
                || isset($_SESSION['zentraactiveentities']) && (count($_SESSION["zentraactiveentities"]) > 1))
        ) {
            if ($itemtype !== AllAssets::getType()) {
