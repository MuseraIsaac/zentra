...
final class TOTPManager
{
     ...
     */
    public static string $brand_label = 'ZENTRA';
    ...
    {
        global $CFG_ZENTRA;
        $label = $CFG_ZENTRA['app_name'] ?? self::$brand_label;

        $tfaSuffix = $CFG_ZENTRA['2fa_suffix'] ?? '';
        if (!empty($tfaSuffix)) {
            $label .= ' (' . $tfaSuffix . ')';
        }
    ...
    public function setSecretForUser(int $users_id, string $secret, ?string $algorithm = null): bool
    {
        ...
        $encrypted_secret = (new ZENTRAKey())->encrypt($secret);
        return $DB->update('zentra_users', [
            '2fa'   => json_encode([
                'algorithm' => 'totp',
    ...
    private function get2FAConfigForUser(int $users_id): ?array
    {
        ...
        try {
            ...
            if (!isset($config['secret'])) {
                ...
                $config['secret'] = (new ZENTRAKey())->decrypt($config['secret']);
                return $config;
            }
    ...
    public function get2FAEnforcement(int $users_id): int
    {
        ...
        $enforced = false;
        if ((int) $CFG_ZENTRA['2fa_enforced'] === 1) {
            $enforced = true;
        } else {
    ...
    public function getGracePeriodDaysLeft(): int
    {
        ...
        $grace_days = $CFG_ZENTRA['2fa_grace_days'] ?? 0;
        $grace_start_date = $CFG_ZENTRA['2fa_grace_date_start'] ?? null;
    ...
    public function showTOTPConfigForm(int $users_id, bool $force_setup = false, bool $regenerate_backup_codes = false): void
    {
        ...
        if (!$force_setup && $this->is2FAEnabled($users_id)) {
            ...
            if ($enforcement === self::ENFORCEMENT_MANDATORY_GRACE_PERIOD) {
                $grace_period_end = (new DateTimeImmutable($CFG_ZENTRA['2fa_grace_date_start']))
                    ->add(new DateInterval("P{$CFG_ZENTRA['2fa_grace_days']}D"));
            ...
            $qr = $tfa->getQRCodeImageAsDataUri($_SESSION['zentraname'], $secret);
    ...
    public function showTOTPSetupForm(int $users_id): void
    {
        ...
        } elseif (isset($_SESSION['zentraname'])) {
            $name = $_SESSION['zentraname'];
        }
