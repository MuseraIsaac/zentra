...
class Socket extends CommonDBChild
{
    ...
    public static function getSocketLinkTypes()
    {
        global $CFG_ZENTRA;
        $values = [];
        foreach ($CFG_ZENTRA["socket_types"] as $itemtype) {
    ...
    public function getTabNameForItem(CommonZENTRA $item, $withtemplate = 0)
    {
        ...
        if (!$withtemplate) {
            ...
            switch (get_class($item)) {
                ...
                default:
                    /** @var CommonDBTM $item */
                    if (in_array($item->getType(), $CFG_ZENTRA['socket_types'])) {
                        if ($_SESSION['zentrashow_count_on_tabs']) {
                            $nb =  countElementsInTable(
                                $this->getTable(),
    ...
    public static function displayTabContentForItem(CommonZENTRA $item, $tabnum = 1, $withtemplate = 0)
    {
        ...
        } elseif (in_array($item::class, $CFG_ZENTRA['socket_types'])) {
            /** @var CommonDBTM $item */
    ...
    public static function showForLocation(Location $item): bool
    {
        ...
        $rand = mt_rand();
        $number = countElementsInTable('zentra_sockets', ['locations_id' => $ID]);
    ...
    public static function getHTMLTableCellsForItem(
        ...
        $row->addCell(
            ...
            htmlescape(Dropdown::getDropdownName("zentra_sockets", $item->fields["sockets_id"])),
            $father
        );
    }
}
