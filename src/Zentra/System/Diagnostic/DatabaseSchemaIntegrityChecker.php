...
namespace Zentra\System\Diagnostic;
...
class DatabaseSchemaIntegrityChecker
{
    ...
    public function checkCompleteSchema(
        ...
        if ($include_unknown_tables) {
            ...
            if ($context === 'core') {
                $unknown_tables_criteria[] = [
                    'NOT' => [
                        'table_name' => ['LIKE', 'zentra\_plugin\_%'],
                    ],
                ];
            } elseif (preg_match('/^plugin:\w+$/', $context) === 1) {
                $unknown_tables_criteria[] = [
                    'table_name' => ['LIKE', sprintf('zentra\_plugin\_%s_%%', str_replace('plugin:', '', $context))],
                ];
            } else {
                trigger_error(sprintf('Invalid context "%s".', $context));
                $is_context_valid = false;
            }
            $unknown_table_iterator = $is_context_valid ? $this->db->listTables('zentra\_%', $unknown_tables_criteria) : [];
    ...
    protected function getNormalizedSql(string $create_table_sql): string
    {
        ...
        if (
            $table_name === 'zentra_notimportedemails'
            && $dbversion !== null
            && version_compare(VersionParser::getNormalizedVersion($dbversion), '10.0', '<')
        ) {
            // Before ZENTRA 10.0.0, COLLATE property was not explicitly defined for zentra_notimportedemails,
            ...
            // and, anyway, it will be fixed during ZENTRA 10.0.0 migration.
            $column_replacements['/( CHARACTER SET \w+)? COLLATE \w+/i'] = '';
        }
        if (
            $table_name === 'zentra_impactcontexts'
            && $dbversion !== null
            && version_compare(VersionParser::getNormalizedVersion($dbversion), '10.0.1', '<')
        ) {
            // Remove default value on zentra_impactcontexts.positions column.
            ...
            // this default value on ZENTRA firstly installed in version <= 9.5.3.
            // see https://github.com/zentra-project/zentra/pull/8415
            // see https://github.com/zentra-project/zentra/pull/11662
    ...
    final public function hasTables(string $context = 'core'): bool
    {
        if (preg_match('/^plugin:(?<plugin_key>\w+)$/', $context, $plugin_matches) === 1) {
            $tables_pattern = 'zentra\_plugin\_' . $plugin_matches['plugin_key'] . '\_%';
            $condition      = [];
        } else {
            $tables_pattern = 'zentra\_%';
            $condition = [
                'NOT' => [
                    'table_name' => ['LIKE', 'zentra\_plugin\_%'],
                ],
    ...
    final public function canCheckIntegrity(?string $schema_version = null, string $context = 'core'): bool
    {
        if ($context === 'core') {
            ...
            $is_latest_version     = $schema_version === ZENTRA_SCHEMA_VERSION;
    ...
    /**
     * Returns ZENTRA database version.
     *
     * @return string|null
     */
    private function getDbVersion(): ?string
    {
        if ($this->db_version === null) {
            if ($this->db->tableExists('zentra_configs') && $this->db->fieldExists('zentra_configs', 'context')) {
            ...
            } elseif ($this->db->tableExists('zentra_configs') && $this->db->fieldExists('zentra_configs', 'version')) {
                // ZENTRA < 0.85
