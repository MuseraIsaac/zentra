...
class SourceCodeIntegrityChecker
{
    ...
    /**
     * ZENTRA source code root directory.
     */
    ...
    /**
     * Download the ZENTRA release.
     *
     * @param array $errors
     * @return string|null Release file path.
     */
    private function getZENTRARelease(array &$errors = []): ?string
    {
        global $CFG_ZENTRA;

        $version_to_get = VersionParser::getNormalizedVersion(ZENTRA_VERSION);
        $gh_releases_endpoint = 'https://api.github.com/repos/zentra-project/zentra/releases/tags/' . $version_to_get;
        ...
        try {
            ...
            foreach ($release['assets'] as $asset) {
                if (str_starts_with($asset['name'], 'zentra-' . $version_to_get)) {
                    $dest = ZENTRA_TMP_DIR . '/' . $asset['name'];
    ...
    public function getDiff(bool $allow_download = false, array &$errors = []): ?string
    {
        ...
        $release_path = ZENTRA_TMP_DIR . '/zentra-' . VersionParser::getNormalizedVersion(ZENTRA_VERSION) . '.tgz';
        if (!file_exists($release_path)) {
            if ($allow_download) {
                $release_path = $this->getZENTRARelease($errors);
        ...
        foreach ($summary as $file => $status) {
            ...
            $current_content = '';
            $extra_header = '';
            if ($status === self::STATUS_ADDED || !file_exists('phar://' . $release_path . '/zentra/' . $file)) {
                ...
                try {
                    $original_content = file_get_contents('phar://' . $release_path . '/zentra/' . $file);
                } catch (Throwable $e) {
            ...
            if ($status === self::STATUS_MISSING || !file_exists($this->root_dir . '/' . $file)) {
                ...
                $file_perms   = @substr(sprintf('%o', fileperms('phar://' . $release_path . '/zentra/' . $file)), -4) ?: '0644';
