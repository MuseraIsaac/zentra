...
final class LogParser extends CommonZENTRA
{
    /**
     * Log directory.
     *
    ...
    public function __construct(string $directory = ZENTRA_LOG_DIR)
    {
        if (!is_dir($directory)) {
    ...
    public function parseLogFile(string $filepath, ?int $max_nb_lines = null): ?array
    {
        ...
        if ($max_nb_lines === null) {
            $max_nb_lines = $_SESSION['zentralist_limit'] ?? $CFG_ZENTRA['list_limit'];
        }
