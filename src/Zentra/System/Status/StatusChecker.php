...
namespace Zentra\System\Status;
...
final class StatusChecker
{
    ...
    /**
     * Calculate the overall ZENTRA status or the overall service status based on all child status checks
    ...
    public static function getServiceStatus(?string $service, $public_only = true): array
    {
        ...
        if ($service === 'all' || $service === null) {
            ...
            $status['zentra']['status'] = self::calculateGlobalStatus($status);
    ...
    public static function getDBStatus(bool $public_only = true): array
    {
        if (!isset(self::$cached_status['db'])) {
                ...
                foreach (array_keys($hosts) as $num) {
                    ...
                    if (abs($diff) > 1000000000) {
                        $status['replicas']['servers'][$num] = [
                            ...
                            'replication_delay'  => '-1',
                            'status_msg'           => _x('zentra_status', 'Replication delay is too high'),
                        ];
            ...
            if (!@DBConnection::establishDBConnection(false, true)) {
                $status['main'] = [
                    'status' => self::STATUS_PROBLEM,
                    'status_msg' => _x('zentra_status', 'Unable to connect to the main database'),
                ];
                $status['status'] = self::STATUS_PROBLEM;
            }
    ...
    public static function getLDAPStatus(bool $public_only = true): array
    {
        if (!isset(self::$cached_status['ldap'])) {
            ...
            if (self::isDBAvailable()) {
                ...
                $ldap_methods = getAllDataFromTable('zentra_authldaps', ['is_active' => 1]);
                ...
                if ($total_servers > 0) {
                    ...
                    foreach ($ldap_methods as $method) {
                        $ldap = null;
                        $display_name = $public_only ? 'ZENTRA_LDAP_' . $method['id'] : $method['name'];
                        try {
                            if (
                                @AuthLDAP::tryToConnectToServer(
                                    $method,
                                    $method['rootdn'],
                                    (new ZENTRAKey())->decrypt($method['rootdn_passwd'])
                                )
                            ) {
                                ...
                                $status['servers'][$display_name] = [
                                    'status' => self::STATUS_PROBLEM,
                                    'status_msg' => _x('zentra_status', 'Unable to connect to the LDAP server'),
                                ];
                                $total_error++;
                                $global_status = self::STATUS_PROBLEM;
                            }
                    ...
                    if ($global_status !== self::STATUS_OK) {
                        $message = sprintf(_x('zentra_status', 'OK: %d, WARNING: %d, PROBLEM: %d, TOTAL: %d'), $total_servers - $total_error, 0, $total_error, $total_servers);
                    }
                }
    ...
    public static function getIMAPStatus(bool $public_only = true): array
    {
        if (!isset(self::$cached_status['imap'])) {
            ...
            if (self::isDBAvailable()) {
                ...
                $imap_methods = getAllDataFromTable('zentra_authmails', ['is_active' => 1]);
                ...
                if ($total_servers > 0) {
                    ...
                    foreach ($imap_methods as $method) {
                        ...
                        $display_name = $public_only ? 'ZENTRA_IMAP_' . $method['id'] : $method['name'];
                        if ($param['ssl'] === true) {
                        ...
                        if ($fp = @fsockopen($host, $param['port'], $errno, $errstr, 1)) { // @phpstan-ignore theCodingMachineSafe.function
                            ...
                            $status['servers'][$display_name] = [
                                'status' => self::STATUS_PROBLEM,
                                'status_msg' => _x('zentra_status', 'Unable to connect to the IMAP server'),
                            ];
                            $total_error++;
                            $global_status = self::STATUS_PROBLEM;
                        }
    ...
    public static function getCASStatus(bool $public_only = true): array
    {
        ...
        if (!isset(self::$cached_status['cas'])) {
            ...
            if (!empty($CFG_ZENTRA['cas_host'])) {
                ...
                $url = 'https://' . $CFG_ZENTRA['cas_host'];
                if (!empty($CFG_ZENTRA['cas_port'])) {
                    $url .= ':' . (int) $CFG_ZENTRA['cas_port'];
                }
                $url .= '/' . $CFG_ZENTRA['cas_uri'];
    ...
    public static function getMailCollectorStatus(bool $public_only = true): array
    {
        if (!isset(self::$cached_status['mail_collectors'])) {
            ...
            if (self::isDBAvailable()) {
                $mailcollectors = getAllDataFromTable('zentra_mailcollectors', ['is_active' => 1]);
                ...
                if ($total_servers > 0) {
                    ...
                    foreach ($mailcollectors as $mc) {
                        if ($mailcol->getFromDB($mc['id'])) {
                            $display_name = $public_only ? 'ZENTRA_COLLECTOR_' . $mc['id'] : $mc['name'];
                            try {
                                $mailcol->connect();
                                $status['servers'][$display_name] = [
                                    'status' => self::STATUS_OK,
    ...
    public static function getCronTaskStatus(bool $public_only = true): array
    {
        if (!isset(self::$cached_status['crontasks'])) {
            ...
            if (self::isDBAvailable()) {
                ...
                $crontasks = getAllDataFromTable('zentra_crontasks');
                ...
                $status['status_msg'] = sprintf(_x('zentra_status', 'RUNNING: %d, STUCK: %d, TOTAL: %d'), $running, count($stuck_crontasks), count($crontasks));
            }
    ...
    public static function getFilesystemStatus(bool $public_only = true): array
    {
        if (!isset(self::$cached_status['filesystem'])) {
            ...
            if ($session_handler !== false && strtolower($session_handler) === 'files') {
                ...
                if (!is_dir(ZENTRA_SESSION_DIR)) {
                    $status['session_dir'] = [
                        'status' => self::STATUS_PROBLEM,
                        'status_msg'   => sprintf(_x('zentra_status', '%s variable is not a directory'), 'ZENTRA_SESSION_DIR'),
                    ];
                    $status['status'] = self::STATUS_PROBLEM;
                } elseif (!is_writable(ZENTRA_SESSION_DIR)) {
                    $status['session_dir'] = [
                        'status' => self::STATUS_PROBLEM,
                        'status_msg'   => sprintf(_x('zentra_status', '%s variable is not writable'), 'ZENTRA_SESSION_DIR'),
                    ];
                    $status['status'] = self::STATUS_PROBLEM;
                }
            } else {
                $status['session_dir']['status_msg'] = _x('zentra_status', 'PHP is not configured to use the "files" session save handler');
            }
