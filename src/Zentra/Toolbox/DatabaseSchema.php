...
class DatabaseSchema
{
    ...
    public static function getEmptySchemaPath(string $version): ?string
    {
        ...
        $latest_version     = VersionParser::getNormalizedVersion(ZENTRA_VERSION, false);

        $schema_path = $normalized_version === $latest_version
            ? sprintf('%s/install/mysql/zentra-empty.sql', realpath(ZENTRA_ROOT))
            : sprintf('%s/install/mysql/zentra-%s-empty.sql', realpath(ZENTRA_ROOT), $normalized_version);
