...
final class URL
{
    ...
    /**
     * Checks whether an URL can be considered as a valid ZENTRA relative URL.
     *
     * @param string $url
     *
     * @return bool
     */
    public static function isZENTRARelativeUrl(string $url): bool
    {
        if ($url === '') {
            return false;
        }
        ...
        if (
            ...
            // ZENTRA URLs are not supposed to contain special chars.
            || preg_match('#[^a-z0-9_/\.-]#i', $parsed_url['path']) === 1
            // // The path refers to an hidden resource (name starts with `/.`), or contains a `/..` that may lead outside the ZENTRA tree.
     ...
     * - '/front/namespace/itemtype.form.php' will yield 'zentra\namespace\itemtype'
     *
    ...
    private static function extractCoreItemtypeFromUrlPath(string $path): ?string
    {
        ...
        if (self::extractedPathContainsNamespace($extracted_path)) {
            return 'zentra\\' . str_replace("/", "\\", $extracted_path);
        } else {
    ...
    private static function extractPluginItemtypeFromUrlPath(string $path): ?string
    {
        ...
        if (self::extractedPathContainsNamespace($extracted_path)) {
            return 'zentraplugin\\' . $plugin_name . '\\' . str_replace(
                "/",
                "\\",
                $extracted_path
            );
