...
class NotificationSettingInstanceTest extends DbTestCase
{
    ...
    {
        $this->assertSame('zentra_configs', \NotificationSetting::getTable());
    }
    ...
    public function testDisableAll()
    {
        ...
        $this->assertEquals(0, $CFG_ZENTRA['use_notifications']);
        $this->assertEquals(0, $CFG_ZENTRA['notifications_mailing']);
        $this->assertArrayNotHasKey('notifications_xyz', $CFG_ZENTRA);

        $CFG_ZENTRA['use_notifications'] = 1;
        $CFG_ZENTRA['notifications_mailing'] = 1;
        $CFG_ZENTRA['notifications_xyz'] = 1;

        \NotificationSetting::disableAll();

        $this->assertEquals(0, $CFG_ZENTRA['use_notifications']);
        $this->assertEquals(0, $CFG_ZENTRA['notifications_mailing']);
        $this->assertEquals(0, $CFG_ZENTRA['notifications_xyz']);
    }
}
