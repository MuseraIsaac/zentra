...
class ZENTRAMailerTest extends DbTestCase
{
    ...
    public function testBuildDsn()
    {
        ...
        $bkp_mode = $CFG_ZENTRA['smtp_mode'];
        $bkp_host = $CFG_ZENTRA['smtp_host'];
        $bkp_port = $CFG_ZENTRA['smtp_port'];
        $bkp_user = $CFG_ZENTRA['smtp_username'];
        $bkp_pass = $CFG_ZENTRA['smtp_passwd'];
        $bkp_check_certif = $CFG_ZENTRA['smtp_check_certificate'];

        $mailer = new \ZENTRAMailer();
        $this->assertSame('native://default', $mailer::buildDsn(true));
        $this->assertSame('native://default', $mailer::buildDsn(false));

        $CFG_ZENTRA['smtp_mode'] = MAIL_SMTP;
        $CFG_ZENTRA['smtp_port'] = 123;
        $CFG_ZENTRA['smtp_host'] = 'myhost.com';
        $CFG_ZENTRA['smtp_username'] = 'myuser';
        $CFG_ZENTRA['smtp_passwd'] = (new \ZENTRAKey())->encrypt('mypass');
        ...
        //reset values
        $CFG_ZENTRA['smtp_mode'] = $bkp_mode;
        $CFG_ZENTRA['smtp_host'] = $bkp_host;
        $CFG_ZENTRA['smtp_port'] = $bkp_port;
        $CFG_ZENTRA['smtp_username'] = $bkp_user;
        $CFG_ZENTRA['smtp_passwd'] = $bkp_pass;
        $CFG_ZENTRA['smtp_check_certificate'] = $bkp_check_certif;
    }
}
