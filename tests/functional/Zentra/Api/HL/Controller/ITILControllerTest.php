...
namespace tests\units\Zentra\Api\HL\Controller;
...
use Zentra\Tests\HLAPITestCase;
use Problem;
use Ticket;
        ...
        foreach ($itil_types as $itil_type => $schema_name) {
            $this->api->autoTestCRUDNoRights(
                ...
                deny_read: static function () use ($itil_type) {
                    ...
                    $_SESSION['zentraactiveprofile'][TicketValidation::$rightname] = 0;
                    $_SESSION['zentraactiveprofile'][ChangeValidation::$rightname] = 0;
                    $_SESSION['zentraactiveprofile'][$itil_type::$rightname] = 0;
                }
            );
        }
    }
        ...
        foreach ([Change::class, Problem::class, Ticket::class] as $itil_type) {
            $this->createItems($itil_type, [
                ...
                [
                    ...
                    'entities_id' => getItemByTypeName('Entity', '_test_root_entity', true),
                    'users_id_recipient' => 1, // Some other user,
                    '_users_id_requester' => $_SESSION['zentraID'],
                    '_skip_auto_assign' => 1,
                ],
                [
                    'name' => $func_name . '4',
            ...
            $_SESSION['zentragroups'][] = getItemByTypeName('Group', '_test_group_1', true); // Group of the ticket
            $request = new Request('GET', '/Assistance/' . $itil_type);
            $request->setParameter('filter', ['name=ilike=' . $func_name . '*']);

            if ($itil_type !== Ticket::class) {
                $_SESSION['zentraactiveprofile'][$itil_type::$rightname] = CommonITILObject::READMY;
                ...
                $_SESSION['zentraactiveprofile'][$itil_type::$rightname] = Ticket::READGROUP;
                $this->api->call($request, function ($call) use ($func_name) {
                    /** @var \HLAPICallAsserter $call */
                    $call->response
                        ->isOK()
                ...
                $_SESSION['zentraactiveprofile'][$itil_type::$rightname] = Ticket::OWN;
                $this->api->call($request, function ($call) use ($func_name) {
                    /** @var \HLAPICallAsserter $call */
                    $call->response
                        ->isOK()
                ...
                $_SESSION['zentraactiveprofile'][$itil_type::$rightname] = Ticket::READASSIGN;
                $this->api->call($request, function ($call) use ($func_name) {
                    /** @var \HLAPICallAsserter $call */
                    $call->response
                        ->isOK()
                ...
                $_SESSION['zentraactiveprofile'][$itil_type::$rightname] = Ticket::READNEWTICKET;
                $this->api->call($request, function ($call) use ($func_name) {
                    /** @var \HLAPICallAsserter $call */
                    $call->response
                        ->isOK()
