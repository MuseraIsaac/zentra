...
class ProjectControllerTest extends HLAPITestCase
{
    ...
    public function testRestrictedProjectRead()
    {
        ...
        $_SESSION['zentraactiveprofile'][Project::$rightname] = 0;
        ...
        $_SESSION['zentraactiveprofile'][Project::$rightname] = Project::READMY;
        $this->api->call($request, function ($call) {
            /** @var \HLAPICallAsserter $call */
            $call->response->isOK();
        });
        ...
        $this->assertTrue($project->update([
            'id' => $project->getID(),
            'users_id' => $_SESSION['zentraID'] + 1, // Created by another user
        ]));
        $this->api->call($request, function ($call) {
            /** @var \HLAPICallAsserter $call */
            $call->response->isNotFoundError();
