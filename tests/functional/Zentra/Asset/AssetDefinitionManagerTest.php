...
use ZentraPlugin\Tester\Asset\Capacity\HasFooCapacity;
...
class AssetDefinitionManagerTest extends DbTestCase
{
    public function testLoadConcreteClass(): void
    {
        ...
        for ($i = 0; $i < 5; $i++) {
            ...
            $mapping['Zentra\\CustomAsset\\' . $system_name . 'Asset'] = $this->initAssetDefinition($system_name);
        }
    ...
    public function testAutoloader(): void
    {
        ...
        $this->assertTrue(\class_exists('Zentra\CustomAsset\Test_Item123Asset'));
        $this->assertTrue(\class_exists('Zentra\CustomAsset\Test_Item123AssetModel'));
        $this->assertTrue(\class_exists('Zentra\CustomAsset\Test_Item123AssetType'));
        $this->assertTrue(\class_exists('Zentra\CustomAsset\RuleDictionaryTest_Item123AssetModelCollection'));
        $this->assertTrue(\class_exists('Zentra\CustomAsset\RuleDictionaryTest_Item123AssetModel'));
        $this->assertTrue(\class_exists('Zentra\CustomAsset\RuleDictionaryTest_Item123AssetTypeCollection'));
        $this->assertTrue(\class_exists('Zentra\CustomAsset\RuleDictionaryTest_Item123AssetType'));
    }
    ...
    public function testTicketTypeConfigRegistration(): void
    {
        ...
        // Itemtype should be registered in $CFG_ZENTRA["ticket_types"]
        $this->assertContains($class, $CFG_ZENTRA["ticket_types"]);
    }
    ...
    protected function testCommonITILTabRegistrationProvider(): iterable
    {
        ...
        // profile
        $profile = Profile::getById($_SESSION['zentraactiveprofile']['id']);
    ...
    public function testRegisteredTesterPluginCapacity(): void
    {
        ...
        $this->assertEquals(
            [
                'table'     => 'zentra_plugin_tester_assets_foos',
                'field'     => 'name',
                'name'      => 'Name',
                'datatype'  => 'itemlink',
            ],
