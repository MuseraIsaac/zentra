...
class HasDevicesCapacityTest extends DbTestCase
{
    public function testCapacityActivation(): void
    {
        ...
        foreach ($has_capacity_mapping as $classname => $has_capacity) {
            ...
            if ($has_capacity) {
                $this->assertContains($classname, $CFG_ZENTRA['itemdevices_types']);
                $this->assertContains($classname, $CFG_ZENTRA['itemdevices_itemaffinity']);
            } else {
                $this->assertNotContains($classname, $CFG_ZENTRA['itemdevices_types']);
                $this->assertNotContains($classname, $CFG_ZENTRA['itemdevices_itemaffinity']);
            }
            foreach (array_keys($CFG_ZENTRA) as $config_key) {
                if (preg_match('/^itemdevice[a-z+]_types$/', $config_key)) {
                    if ($has_capacity) {
                        $this->assertContains($classname, $CFG_ZENTRA[$config_key]);
                    } else {
                        $this->assertNotContains($classname, $CFG_ZENTRA[$config_key]);
                    }
                }
            }
            if ($has_capacity) {
    ...
    public function testCapacityDeactivation(): void
    {
        ...
        $this->assertContains($classname_1, $CFG_ZENTRA['itemdevices_types']);
        $this->assertContains($classname_2, $CFG_ZENTRA['itemdevices_types']);
        ...
        $this->assertNotContains($classname_1, $CFG_ZENTRA['itemdevices_types']);
