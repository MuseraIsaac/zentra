...
use Zentra\Asset\Capacity\HasSocketCapacity;
use Zentra\Socket;
...
class HasSocketCapacityTest extends DbTestCase
{
    ...
    public function testCapacityActivation(): void
    {
        ...
        foreach ($has_capacity_mapping as $classname => $has_capacity) {
            ...
            if ($has_capacity) {
                $this->assertContains($classname, $CFG_ZENTRA['socket_types']);
            } else {
                $this->assertNotContains($classname, $CFG_ZENTRA['socket_types']);
            }
            ...
            if ($has_capacity) {
                $this->assertArrayHasKey('Zentra\Socket$1', $item->defineAllTabs());
            } else {
                $this->assertArrayNotHasKey('Zentra\Socket$1', $item->defineAllTabs());
            }
    ...
    public function testCapacityDeactivation(): void
    {
        ...
        $this->assertContains($classname_1, $CFG_ZENTRA['socket_types']);
        $this->assertContains($classname_2, $CFG_ZENTRA['socket_types']);
        ...
        $this->assertNotContains($classname_1, $CFG_ZENTRA['socket_types']);
