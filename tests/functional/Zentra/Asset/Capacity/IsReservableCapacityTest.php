...
use Zentra\Tests\Zentra\Asset\CapacityUsageTestTrait;
use Log;
use ReservationItem;
...
class IsReservableCapacityTest extends DbTestCase
{
    ...
    public function testCapacityActivation(): void
    {
        ...
        foreach ($has_capacity_mapping as $classname => $has_capacity) {
            ...
            if ($has_capacity) {
                $this->assertContains($classname, $CFG_ZENTRA['reservation_types']);
                $this->assertEquals(
                    ['fullcalendar', 'reservations'],
                    $CFG_ZENTRA['javascript']['assets'][strtolower($classname)]
                );
            } else {
                $this->assertNotContains($classname, $CFG_ZENTRA['reservation_types']);
                $this->assertArrayNotHasKey(
                    strtolower($classname),
                    $CFG_ZENTRA['javascript']['assets']
                );
            }
    ...
    public function testCapacityDeactivation(): void
    {
        ...
        $this->assertContains($classname_1, $CFG_ZENTRA['reservation_types']);
        $this->assertContains($classname_2, $CFG_ZENTRA['reservation_types']);
        ...
        $this->assertNotContains($classname_1, $CFG_ZENTRA['reservation_types']);
