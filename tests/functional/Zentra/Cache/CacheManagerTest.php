...
namespace tests\units\Zentra\Cache;
...
class CacheManagerTest extends ZENTRATestCase
{
    ...
    {
        vfsStream::setup('zentra', null, ['config' => [], 'files' => ['_cache' => []]]);

        $instance = new CacheManager(
            vfsStream::url('zentra/config'),
            vfsStream::url('zentra/files/_cache')
        );
    ...
    public static function configurationProvider(): iterable
    {
        foreach (['core', 'plugin:tester'] as $context) {
            ...
            yield [
                'context'            => $context,
                'dsn'                => ['redis://cache1.zentra-project.org', 'redis://cache2.zentra-project.org'],
                'options'            => [],
                'expected_set_error' => 'Invalid DSN: ["redis://cache1.zentra-project.org","redis://cache2.zentra-project.org"].',
                'expected_get_error' => [
                    'level' => LogLevel::WARNING,
                    'message' => sprintf('Invalid configuration for cache context "%s".', $context),
                ],
            ...
            if (extension_loaded('memcached')) {
                ...
                yield [
                    'context'            => $context,
                    'dsn'                => 'memcached://cache.zentra-project.org',
                    'options'            => [
                ...
                yield [
                    'context'          => $context,
                    'dsn'              => ['memcached://cache1.zentra-project.org', 'memcached://cache2.zentra-project.org'],
                    'options'          => [],
                    'expected_set_error' => null,
                    'expected_get_error' => null,
                    'expected_adapter' => MemcachedAdapter::class,
            ...
            if (extension_loaded('redis')) {
                ...
                yield [
                    'context'          => $context,
                    'dsn'              => 'redis://cache.zentra-project.org',
                    'options'          => [
                        'lazy'       => true,
                        'persistent' => 1,
                    ],
    ...
    public function testSetConfiguration(
        ...
        $config_file = vfsStream::url('zentra/config/' . CacheManager::CONFIG_FILENAME);
        $expected_config = [
            'contexts' => [
                $context => [
                    'dsn'       => $dsn,
    ...
    public function testUnsetConfiguration(): void
    {
        ...
        $expected_config = [
            'contexts' => [
                ...
                'plugin:tester' => [
                    'dsn'       => 'redis://cache.zentra-project.org/zentra',
                    'options'   => ['lazy' => true],
                ],
                'plugin:another' => [
                    'dsn'       => 'redis://cache.zentra-project.org/zentra',
        ...
        $config_file = vfsStream::url('zentra/config/' . $config_filename);
    ...
    public static function dsnProvider(): iterable
    {
        ...
        yield [
            'dsn'      => ['redis:///tmp/cache', 'redis://localhost/zentra'], // invalid multiple DSN
            'is_valid' => false,
            'scheme'   => null,
        ];
    }
    ...
    public function testGetKnownContexts()
    {
        ...
        $vfs_structure = vfsStream::setup('zentra', null, ['config' => [], 'files' => ['_cache' => []]]);
