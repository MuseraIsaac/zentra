...
class ErrorUtilsTest extends DbTestCase
{
    ...
    {
        assert(is_string(\ZENTRA_ROOT) && strlen(\ZENTRA_ROOT) > 0);
        $this->assertEquals($input, ErrorUtils::cleanPaths($input));
    }

    #[DataProvider('unsafeContentsProvider')]
    public function testcleanPathsOnUnsafeContentsRemovesZENTRA_ROOT($input, $expected)
    {
        assert(is_string(\ZENTRA_ROOT) && strlen(\ZENTRA_ROOT) > 0);
    ...
    {
        return [
            ['input' => \ZENTRA_ROOT . '/files/ is not writtable.', 'expected' => './files/ is not writtable.'],
            ['input' => 'Base dir is writtable, fix rights in ' . \ZENTRA_ROOT, 'expected' => 'Base dir is writtable, fix rights in .'],
            ['input' => \ZENTRA_ROOT, 'expected' => '.'],
            ['input' => 'error in ' . \ZENTRA_ROOT . '/path : content not readable', 'expected' => 'error in ./path : content not readable'],
        ];
    }
