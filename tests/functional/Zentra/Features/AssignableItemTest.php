 ...
 */
    ...
    public function testGetAssignableVisiblityCriteria(string $class): void
    {
        ...
        $_SESSION["zentracronuserrunning"] = 1;
        ...
        if (!in_array($class, $CFG_ZENTRA['ticket_types'])) {
            return;
        }
        ...
        $DB->update(
            'zentra_profiles',
            ['helpdesk_item_type' => '[]'],
            ['id' => $_SESSION['zentraactiveprofile']['id']]
        );
        $this->assertEquals(
        ...
        $this->login();
        $_SESSION['zentraactiveprofile'][$class::$rightname] = 0;
        $this->assertEquals(
            [[new QueryExpression('0')]],
            array_values($class::getAssignableVisiblityCriteria())
        );
        $_SESSION['zentraactiveprofile'][$class::$rightname] = READ;
        $this->assertEquals(
            [[new QueryExpression('1')]],
            array_values($class::getAssignableVisiblityCriteria())
        );
        $_SESSION['zentraactiveprofile'][$class::$rightname] = READ_ASSIGNED;
        $this->assertNotEquals(
            [[new QueryExpression('0')]],
            array_values($class::getAssignableVisiblityCriteria())
        );
        ...
        $_SESSION['zentraactiveprofile'][$class::$rightname] = READ_OWNED;
        $this->assertNotEquals(
            [[new QueryExpression('0')]],
            array_values($class::getAssignableVisiblityCriteria())
        );
