...
namespace tests\units\Zentra\Form\Export;
...
use Zentra\Form\Destination\CommonITILField\SLMFieldConfig;
...
use Zentra\Form\Export\Result\ImportError;
...
use Zentra\Form\QuestionType\QuestionTypeActorsDefaultValueConfig;
...
use Zentra\Form\QuestionType\QuestionTypeItemDefaultValueConfig;
...
use Zentra\Form\RenderLayout;
use Zentra\Form\Section;
use Zentra\Form\Tag\AnswerTagProvider;
use Zentra\Form\Tag\CommentDescriptionTagProvider;
use Zentra\Form\Tag\CommentTitleTagProvider;
use Zentra\Form\Tag\FormTagProvider;
use Zentra\Form\Tag\QuestionTagProvider;
use Zentra\Form\Tag\SectionTagProvider;
...
use ZentraPlugin\Tester\Form\DayOfTheWeekPolicyConfig;
...
final class FormSerializerTest extends DbTestCase
{
    ...
    public function testSingleFormExportFileName(): void
    {
        ...
        // Act: export form
        $_SESSION['zentra_currenttime'] = "2024-09-09 08:38:28";
    ...
    public function testExportAndImportWithCustomIcon(): void
    {
        ...
        $custom_icon_source = ZENTRA_ROOT . "/tests/fixtures/uploads/foo.png";
        $file_name = Uuid::uuid4() . "-foo.png";
        $tmp_file_path = ZENTRA_TMP_DIR . "/$file_name";

        copy($custom_icon_source, $tmp_file_path);
        $illustration_manager->saveCustomIllustration($file_name, $tmp_file_path);

        $saved_icon_path = ZENTRA_PICTURE_DIR . "/illustrations/" . $file_name;
        ...
        // where we would import the form into a ZENTRA where the icon doesn't exist
        $json = $this->exportForm($form);
    ...
    public function testTagsAreUpdatedWhenImporting(): void
    {
        ...
        $this->assertStringContainsString(
            "data-form-tag-value=\"{$imported_form->getID()}\" data-form-tag-provider=\"Zentra\Form\Tag\FormTagProvider\"",
            $imported_destination->getConfig()[TitleField::getKey()]['value'],
        );
        $this->assertStringContainsString(
            "data-form-tag-value=\"{$imported_section->getID()}\" data-form-tag-provider=\"Zentra\Form\Tag\SectionTagProvider\"",
            $imported_destination->getConfig()[ContentField::getKey()]['value'],
        );
        $this->assertStringContainsString(
            "data-form-tag-value=\"{$imported_question->getID()}\" data-form-tag-provider=\"Zentra\Form\Tag\QuestionTagProvider\"",
            $imported_destination->getConfig()[ContentField::getKey()]['value'],
        );
        $this->assertStringContainsString(
            "data-form-tag-value=\"{$imported_question->getID()}\" data-form-tag-provider=\"Zentra\Form\Tag\AnswerTagProvider\"",
            $imported_destination->getConfig()[ContentField::getKey()]['value'],
        );
        $this->assertStringContainsString(
            "data-form-tag-value=\"{$imported_comment_1->getID()}\" data-form-tag-provider=\"Zentra\Form\Tag\CommentTitleTagProvider\"",
            $imported_destination->getConfig()[ContentField::getKey()]['value'],
        );
        $this->assertStringContainsString(
            "data-form-tag-value=\"{$imported_comment_2->getID()}\" data-form-tag-provider=\"Zentra\Form\Tag\CommentTitleTagProvider\"",
            $imported_destination->getConfig()[ContentField::getKey()]['value'],
        );
        $this->assertStringContainsString(
            "data-form-tag-value=\"{$imported_comment_1->getID()}\" data-form-tag-provider=\"Zentra\Form\Tag\CommentDescriptionTagProvider\"",
            $imported_destination->getConfig()[ContentField::getKey()]['value'],
        );
        $this->assertStringContainsString(
            "data-form-tag-value=\"{$imported_comment_2->getID()}\" data-form-tag-provider=\"Zentra\Form\Tag\CommentDescriptionTagProvider\"",
            $imported_destination->getConfig()[ContentField::getKey()]['value'],
        );
    }
    ...
    public function testCustomAssetsAreAddedAsRequirements(): void
    {
        ...
        $data = json_decode($json, associative: true);
        $this->assertEquals([
            ['itemtype' => "Zentra\\CustomAsset\\LawnmowerAsset"],
    ...
    public function testImportWithCustomAsset(): void
    {
        ...
        $this->assertTrue(class_exists("Zentra\\CustomAsset\\HammerAsset"));
        $json = $this->getFormJson('form-with-hammer-asset.json');
    ...
    public function testExportThatUsesPluginItemsInQuestionsConfig(): void
    {
        ...
        // Arrange: create a form with a type from a plugin
        $CFG_ZENTRA['asset_types'][] = Foo::class;
        $builder = new FormBuilder();
        $builder->setShouldInitDestinations(false);
        $builder->addQuestion(
            name: "My plugin question",
    ...
    private function compareValuesForRelations(
        ...
        $relation_table_names_to_ignore = [
            '_zentra_helpdesks_tiles_formtiles', // Do not export/import tiles
            '_zentra_forms_answerssets', // Do not export/import answers sets
        ];

        $field_keys_to_ignore_for_specific_tables = [
            '_zentra_forms_sections'                      => ['uuid', 'conditions'],
            '_zentra_forms_questions'                     => ['uuid', 'forms_sections_uuid', 'conditions', 'extra_data'],
            '_zentra_forms_comments'                      => ['uuid', 'forms_sections_uuid', 'conditions'],
            '_zentra_forms_destinations_formdestinations' => ['config', 'conditions'],
        ];
        ...
        foreach ($relations as $table_name => $field_keys) {
            ...
            foreach ($field_keys as $field_key) {
                ...
                foreach ($default_values as $default_value) {
                    ...
                    if ($table_name === '_zentra_forms_questions') {
                    ...
                    if ($table_name === '_zentra_forms_destinations_formdestinations') {
                        ...
                        unset(
                            $default_config['zentra-form-destination-commonitilfield-titlefield'],
                            $default_config['zentra-form-destination-commonitilfield-contentfield'],
                            $imported_config['zentra-form-destination-commonitilfield-titlefield'],
                            $imported_config['zentra-form-destination-commonitilfield-contentfield']
                        );
