...
class FormTest extends DbTestCase
{
    ...
    public function testPrepareInputForUpdate(): void
    {
        ...
        $old_time = $_SESSION['zentra_currenttime'];
        $_SESSION['zentra_currenttime'] = date('Y-m-d H:i:s', strtotime('-1 minute', strtotime($old_time)));
        $form = $this->createItem(Form::class, [
            'name'        => 'Form with first section',
            'entities_id' => $entity->getID(),
        ]);
        $_SESSION['zentra_currenttime'] = $old_time;
     ...
     *
     * Update to the form itself and its sections are already handled by ZENTRA's
