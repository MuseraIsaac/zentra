...
namespace tests\units\Zentra\Kernel\Listener\RequestListener;
...
class LegacyRouterListenerTest extends ZENTRATestCase
{
    public static function fileProvider(): iterable
    {
        ...
        yield '/front/whatever.php/ZentraPlugin%5CNamespace%5CUnexemple/' => [
            'structure'       => $structure,
            'path'            => '/front/whatever.php/ZentraPlugin%5CNamespace%5CUnexemple/',
            'target_path'     => '/front/whatever.php',
            'target_pathinfo' => '/ZentraPlugin\Namespace\Unexemple/',
            'included'        => true,
        ];
    ...
    public function testRunLegacyRouterResponse(
        ...
        $instance = new LegacyRouterListener(
            vfsStream::url('zentra'),
            [vfsStream::url('zentra/marketplace'), vfsStream::url('zentra/plugins')]
        );
        ...
        if ($included === false) {
            ...
            $this->assertNull($event->getRequest()->attributes->get('_zentra_file_to_load'));
        } else {
            $this->assertEquals(LegacyFileLoadController::class, $event->getRequest()->attributes->get('_controller'));
            $this->assertEquals(vfsStream::url('zentra') . $target_path, $event->getRequest()->attributes->get('_zentra_file_to_load'));
        }
    }

    public static function pathProvider(): iterable
    {
        $zentra_exposed_php_files = [
            '/ajax/script.php',
            '/front/page.php',
            '/install/install.php',
            '/install/update.php',
        ];
        $zentra_protected_php_files = [
            '/ajax-script.php',
            '/apirest.php',
            '/caldav.php',
        ...
        $zentra_static_files = [
            ...
            '/install/mysql/zentra-empty.sql',
            '/js/common.js',
            '/locales/en_GB.po',
            '/locales/en_GB.mo',
            '/locales/zentra.pot',
        ...
        foreach ($zentra_exposed_php_files as $file) {
            // exposed ZENTRA PHP files should be served
            yield $file => [
                'url_path'  => $file,
                'file_path' => $file,
                'is_served' => true,
        ...
        foreach ($zentra_protected_php_files as $file) {
            // protected ZENTRA PHP files should NOT be served
            yield $file => [
                'url_path'  => $file,
                'file_path' => $file,
                'is_served' => false,
        ...
        $not_served_files = \array_merge(
            $zentra_static_files, // ZENTRA static files should NOT be served by this listener
            $zentra_hidden_files // ZENTRA hidden files should never be served
        );
    ...
    public function testRunLegacyRouterFirewall(string $url_path, string $file_path, bool $is_served): void
    {
        ...
        if ($is_served === false) {
            ...
            $this->assertEquals(LegacyFileLoadController::class, $event->getRequest()->attributes->get('_controller'));
            $this->assertEquals(vfsStream::url('zentra') . $file_path, $event->getRequest()->attributes->get('_zentra_file_to_load'));
        }
    }
    ...
    public function testRunLegacyRouterFromMarketplaceDirWithPluginPath(): void
    {
        ...
        $this->assertEquals(LegacyFileLoadController::class, $event->getRequest()->attributes->get('_controller'));
        $this->assertEquals(vfsStream::url('zentra/marketplace/tester/front/test.php'), $event->getRequest()->attributes->get('_zentra_file_to_load'));
    }
    ...
    public function testRunLegacyRouterFromDeprecatedPublicPath(): void
    {
        ...
        $this->assertEquals(LegacyFileLoadController::class, $event->getRequest()->attributes->get('_controller'));
        $this->assertEquals(vfsStream::url('zentra/plugins/tester/public/test.php'), $event->getRequest()->attributes->get('_zentra_file_to_load'));
    }
    ...
    public function testRunLegacyRouterFromPluginInMultipleDirectories(): void
    {
        ...
        $instance = new LegacyRouterListener(
            vfsStream::url('zentra'),
            [vfsStream::url('zentra/plugins'), vfsStream::url('zentra/marketplace')]
        );
        ...
        $this->assertEquals(LegacyFileLoadController::class, $event->getRequest()->attributes->get('_controller'));
        $this->assertEquals(vfsStream::url('zentra/plugins/tester/front/test.php'), $event->getRequest()->attributes->get('_zentra_file_to_load'));
    }
