...
namespace tests\units\Zentra\Security;
...
class TOTPManagerTest extends DbTestCase
{
    ...
    public function testSetSecretForUser()
    {
        ...
        $this->assertEquals('G3QWAUUBIOM7GUU3EHC76WGMV5FIO3FB', (new \ZENTRAKey())->decrypt($tfa_config['secret']));
    ...
    public function testIs2FAEnabled()
    {
        ...
        $DB->update('zentra_users', [
            '2fa' => json_encode([
                'secret' => (new \ZENTRAKey())->encrypt('G3QWAUUBIOM7GUU3EHC76WGMV5FIO3FB'),
            ]),
        ], ['id' => $users_id]);
        $this->assertTrue($tfa->is2FAEnabled($users_id));

        $DB->update('zentra_users', ['2fa' => json_encode([])], ['id' => $users_id]);
        $this->assertFalse($tfa->is2FAEnabled($users_id));

        $DB->update('zentra_users', ['2fa' => null], ['id' => $users_id]);
        $this->assertFalse($tfa->is2FAEnabled($users_id));
    }
    ...
    public function testGet2FAEnforcement()
    {
        ...
        $CFG_ZENTRA['2fa_enforced'] = 1;
        $this->assertEquals($tfa::ENFORCEMENT_MANDATORY, $tfa->get2FAEnforcement($users_id));
        $DB->update('zentra_users', ['2fa_unenforced' => 1], ['id' => $users_id]);
        $this->assertEquals($tfa::ENFORCEMENT_OPTIONAL, $tfa->get2FAEnforcement($users_id));
        $DB->update('zentra_users', ['2fa_unenforced' => 0], ['id' => $users_id]);

        $CFG_ZENTRA['2fa_enforced'] = 1;
        $CFG_ZENTRA['2fa_grace_date_start'] = date('Y-m-d H:i:s', strtotime('-1 day'));
        $CFG_ZENTRA['2fa_grace_days'] = 3;
        $this->assertEquals($tfa::ENFORCEMENT_MANDATORY_GRACE_PERIOD, $tfa->get2FAEnforcement($users_id));

        $CFG_ZENTRA['2fa_grace_date_start'] = date('Y-m-d H:i:s', strtotime('-5 day'));
        $CFG_ZENTRA['2fa_grace_days'] = 2;
        $this->assertEquals($tfa::ENFORCEMENT_MANDATORY, $tfa->get2FAEnforcement($users_id));

        $CFG_ZENTRA['2fa_enforced'] = 0;
        $CFG_ZENTRA['2fa_grace_days'] = 0;
        $DB->update('zentra_entities', ['2fa_enforcement_strategy' => 1], ['id' => $entities_id]);
        $this->assertEquals($tfa::ENFORCEMENT_MANDATORY, $tfa->get2FAEnforcement($users_id));
        $DB->update('zentra_entities', ['2fa_enforcement_strategy' => 0], ['id' => $entities_id]);

        $DB->update('zentra_profiles', ['2fa_enforced' => 1], ['id' => 4]);
        $this->assertEquals($tfa::ENFORCEMENT_MANDATORY, $tfa->get2FAEnforcement($users_id));
        $DB->update('zentra_profiles', ['2fa_enforced' => 0], ['id' => 4]);
        ...
        $DB->update('zentra_groups', ['2fa_enforced' => 1], ['id' => $groups_id]);
        $this->assertEquals($tfa::ENFORCEMENT_MANDATORY, $tfa->get2FAEnforcement($users_id));
    }
    ...
    public function testGet2faIssuer()
    {
        ...
        // No custom suffix
        $this->assertEquals('ZENTRA', $tfa->getIssuer());

        $CFG_ZENTRA['2fa_suffix'] = 'test';
        $this->assertEquals('ZENTRA (test)', $tfa->getIssuer());
    }
}
