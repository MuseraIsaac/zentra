...
namespace tests\units\Zentra\System\Diagnostic;
...
class DatabaseSchemaIntegrityCheckerTest extends ZENTRATestCase
{
...
DIFF,
    ...
    public function testExtractSchemaFromFile(
        ...
        $this->assertEquals($raw_tables, $checker->extractSchemaFromFile(vfsStream::url('zentra/install/schema.sql')));
    }
    ...
    public function testCheckCompleteSchema(
        ...
        $this->assertEquals($expected_differences, $checker->checkCompleteSchema(vfsStream::url('zentra/install/schema.sql')));
    }
...
CREATE TABLE `zentra_{$table_prefix}existingtable` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `zentra_{$table_prefix}missingtable` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `name` varchar(255) NOT NULL,
  `description` text,
  PRIMARY KEY (`id`)
...
CREATE TABLE `zentra_{$table_prefix}unknowntable` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `name` varchar(255) NOT NULL,
  `description` text,
  PRIMARY KEY (`id`)
...
DROP TABLE IF EXISTS `zentra_somethings`;
...
SQL,
            ...
            $checker = new DatabaseSchemaIntegrityChecker($this->getDbMock([
                ...
                '_mock_tableExists' => function ($table_name) use ($table_prefix) {
                    return $table_name !== "zentra_{$table_prefix}missingtable";
                },
                '_mock_listTables' => [['TABLE_NAME' => "zentra_{$table_prefix}unknowntable"]], // $DB->listTables() is used to list unknown tables
                '_mock_doQuery' => function ($query) use ($table_prefix, $existingtable_sql, $unknowntable_sql) {
                    $table_name = preg_replace('/SHOW CREATE TABLE `([^`]+)`/', '$1', $query);
                    $result = null;
                    switch ($table_name) {
                        case "zentra_{$table_prefix}existingtable":
                            $result = ['Create Table' => $existingtable_sql];
                            break;
                        case "zentra_{$table_prefix}unknowntable":
            ...
            $expected = [
                "zentra_{$table_prefix}missingtable" => [
                    'type' => 'missing_table',
                    'diff' => <<<DIFF
--- Expected database schema
+++ Current database schema
...
DIFF,
                ...
                "zentra_{$table_prefix}unknowntable" => [
                    'type' => 'unknown_table',
                    'diff' => <<<DIFF
--- Expected database schema
+++ Current database schema
...
DIFF,
            ...
            $this->assertEquals($expected, $checker->checkCompleteSchema(vfsStream::url('zentra/install/schema.sql'), true, $context));

            // Check without unknown tables detection
            unset($expected["zentra_{$table_prefix}unknowntable"]);
            $this->assertEquals($expected, $checker->checkCompleteSchema(vfsStream::url('zentra/install/schema.sql'), false));
        }
    }

    public function testImpactContextsPositionsDefaultValueHack()
    {
        // Schema contained in ZENTRA 9.5.0 database installation file
...
SQL;
        ...
        // Schema contained in ZENTRA 10.0.1 database installation file
...
SQL;
        ...
        foreach ($dbversions as $dbversion) {
            $checker = new DatabaseSchemaIntegrityChecker($this->getDbMock([
                // Case 1: "DEFAULT ''" not returned by MySQL should not be detected as a difference for ZENTRA < 10.0.1
...
SQL,
            ...
            $this->assertEmpty($checker->getDiff('zentra_impactcontexts', $schema_sql_from_950));
        }
        ...
        foreach ($dbversions as $dbversion) {
            $checker = new DatabaseSchemaIntegrityChecker($this->getDbMock([
                'use_utf8mb4' => true,
                // Case 2: "DEFAULT ''" returned by MariaDB should be detected as a difference for ZENTRA >= 10.0.1
                '_mock_doQuery' => function ($query) {
                    if (preg_match('/^SHOW CREATE TABLE/', $query) === 1) {
                        return $this->getMysqliResultMock([
                            'Create Table' => <<<SQL
...
DIFF,
                $checker->getDiff('zentra_impactcontexts', $schema_sql_from_1001)
            );
        }
    }
...
SQL,
            ...
            $this->assertEmpty($checker->getDiff('zentra_notimportedemails', $schema_sql_from_955));
        }
...
SQL,
            ...
            $this->assertEmpty($checker->getDiff('zentra_notimportedemails', $schema_sql_from_1000_ok));
            $this->assertEquals(
                <<<DIFF
--- Expected database schema
+++ Current database schema
...
DIFF,
                $checker->getDiff('zentra_notimportedemails', $schema_sql_from_1000_ko)
            );
        }
    }
    ...
    {
        $root = realpath(ZENTRA_ROOT);

        // Current version -> can check
        yield [
            'version'              => ZENTRA_SCHEMA_VERSION,
            'context'              => 'core',
            'expected_can_check'   => true,
            'expected_schema_path' => sprintf(
                '%s/install/mysql/zentra-empty.sql',
                $root
            ),
        ];
        ...
        yield [
            ...
            'context'              => 'core',
            'expected_can_check'   => true,
            'expected_schema_path' => sprintf('%s/install/mysql/zentra-10.0.2-empty.sql', $root),
        ];
    ...
    public function testHasTables()
    {
        ...
        $checker = new DatabaseSchemaIntegrityChecker($this->getDbMock([
            '_mock_listTables' => new ArrayIterator([
                ['TABLE_NAME' => 'zentra_configs'],
                ['TABLE_NAME' => 'zentra_entities'],
                // ...
            ]),
        ]));
        ...
        $checker = new DatabaseSchemaIntegrityChecker($this->getDbMock([
            '_mock_listTables' => new ArrayIterator([
                ['TABLE_NAME' => 'zentra_plugin_myplugin_bars'],
                ['TABLE_NAME' => 'zentra_plugin_myplugin_foos'],
            ]),
        ]));
