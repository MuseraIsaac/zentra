...
class SourceCodeIntegrityCheckerTest extends ZENTRATestCase
{
    /**
...
EOL,
    ...
    public function testGetDiff()
    {
        ...
        $version = VersionParser::getNormalizedVersion(ZENTRA_VERSION, false);
        $version_full = VersionParser::getNormalizedVersion(ZENTRA_VERSION);
        ...
        if (file_exists(ZENTRA_TMP_DIR . '/zentra-' . $version_full . '.tar.gz')) {
            unlink(ZENTRA_TMP_DIR . '/zentra-' . $version_full . '.tar.gz');
        }
        $phar = new \PharData(ZENTRA_TMP_DIR . '/zentra-' . $version_full . '.tgz');
        ...
        foreach ($iterator as $file) {
            ...
            $phar->addFromString('zentra/' . $path, file_get_contents($file->getPathname()));
        }
        $phar->compress(\Phar::GZ);
        $this->assertTrue(rename(ZENTRA_TMP_DIR . '/zentra-' . $version_full . '.tar.gz', ZENTRA_TMP_DIR . '/zentra-' . $version_full . '.tgz'));
