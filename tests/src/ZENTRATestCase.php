...
use Zentra\Tests\Log\TestHandler;
...
// Main ZENTRA test case. All tests should extend this class.

class ZENTRATestCase extends TestCase
{
    private $int;
    private $str;
    ...
    protected function resetSession()
    {
        ...
        foreach ($CFG_ZENTRA['user_pref_field'] as $field) {
            if (!isset($_SESSION["zentra$field"]) && isset($CFG_ZENTRA[$field])) {
                $_SESSION["zentra$field"] = $CFG_ZENTRA[$field];
            }
        }
    }
    ...
    private function resetGlobalsAndStaticValues(): void
    {
        ...
        // Globals
        global $CFG_ZENTRA, $FOOTER_LOADED, $HEADER_LOADED;
        $CFG_ZENTRA = $this->config_copy;
        $FOOTER_LOADED = false;
        $HEADER_LOADED = false;
    ...
    protected function modifyCurrentTime(string $modification): void
    {
        ...
        $_SESSION['zentra_currenttime'] = $date->format("Y-m-d H:i:s");
    }

    /**
     * Set $_SESSION['zentra_currenttime'] and return the related DateTimeImmutable object.
     *
    ...
    protected function setCurrentTime(string $datetime): DateTimeImmutable
    {
        ...
        $_SESSION['zentra_currenttime'] = $datetime;

        return DateTimeImmutable::createFromFormat('Y-m-d H:i:s', $_SESSION['zentra_currenttime']);
    }
